...
//   1. Scaffolds a real gemini using the host adapter
...
async function ensureScaffoldLoaded() {
  ...
  if (!existsSync(join(distDir, 'index.js'))) throw new Error('build first: cd packages/create-agent-gemini && npm run build');
...
describe('cross-host invariants (iter 126)', () => {
  ...
  for (const host of HOSTS) {
    it(`${host}: .gemini/manifest.json records the host in the hosts[] array`, async () => {
