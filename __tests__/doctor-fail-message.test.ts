...
//
// iter 93 — when `gemini doctor` reports a failure, the message must
...
beforeAll(async () => {
  const distPath = resolve(REPO_ROOT, 'packages', 'create-agent-gemini', 'dist', 'subcommands.js');
  if (!existsSync(distPath)) {
    throw new Error(`create-agent-gemini dist missing — run npm run build first`);
  }
...
describe('doctor fail message recommends diag --bundle (iter 93)', () => {
  it('on FAIL, the message includes the diag --bundle suggestion', async () => {
    ...
    try {
      // Empty dir → doctor finds no package.json, no .gemini, etc.
      ...
      expect(out).toContain('github.com/cvsz/zagents-generator/issues');
