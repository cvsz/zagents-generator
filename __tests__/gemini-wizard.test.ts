...
let wiz: typeof import('../packages/create-agent-gemini/dist/wizard.js');

beforeAll(async () => {
  const distPath = resolve(REPO_ROOT, 'packages', 'create-agent-gemini', 'dist', 'wizard.js');
...
describe('runWizard (iter 100)', () => {
  ...
  it('template can also be picked by pasted id', async () => {
    ...
    expect(answers.description).toBe('edu-bot — an AI agent gemini');
  });
  ...
  it('answersToInvocation builds a copy-pasteable npx command', () => {
    ...
    expect(inv).toBe('npx create-agent-gemini my-bot --template vertical:coding --host codex --description "A coding pod"');
  });
  ...
  it('answersToInvocation omits defaults (minimal + claude-code)', () => {
    ...
    expect(inv).toBe('npx create-agent-gemini my-bot --description "A bot"');
  });
});
