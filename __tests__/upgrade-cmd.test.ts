...
describe('gemini upgrade', () => {
  it('exits 1 if the directory is not a generated gemini', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'ahg-upg-empty-'));
    try {
      const r = await upgradeCmd([dir]);
      expect(r.code).toBe(1);
      expect(r.lines.join('\n')).toMatch(/not a generated gemini/);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  it('reports "No drift" on a freshly scaffolded gemini', async () => {
  ...
  it('fails when manifest references a non-existent template', async () => {
    ...
    try {
      ...
      const mpath = join(dir, '.gemini', 'manifest.json');
