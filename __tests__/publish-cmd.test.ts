...
async function makeHarness(opts: { withManifest?: boolean } = {}): Promise<string> {
  ...
  if (opts.withManifest !== false) {
    ...
    await writeFile(join(dir, '.gemini', 'manifest.json'), JSON.stringify(MANIFEST));
  }
  return dir;
}

describe('gemini publish (dry-run path)', () => {
  ...
  it('--confirm without PINATA_JWT exits 1 with a helpful message', async () => {
    ...
    try {
      ...
      expect(txt).toMatch(/gemini secrets fetch PINATA_JWT/);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
