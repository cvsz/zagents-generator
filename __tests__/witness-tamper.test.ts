...
import {
  ...
  readAndVerify,
  findWitness,
} from '../packages/create-agent-gemini/src/witness-client.js';
...
describe('witness shape gate (tamper detection)', () => {
  ...
  it('missing gemini name rejected', async () => {
    const r = await verifyWitness({ ...VALID_SHAPE, gemini: '' });
    expect(r.valid).toBe(false);
...
describe('findWitness + readAndVerify (file conventions)', () => {
  ...
  it('finds witness.json at .gemini/witness.json (the canonical convention)', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'ahg-witness-'));
    try {
      await mkdir(join(dir, '.gemini'), { recursive: true });
      const p = join(dir, '.gemini', 'witness.json');
      await writeFile(p, JSON.stringify(VALID_SHAPE), 'utf-8');
      expect(findWitness(dir)).toBe(p);
    } finally {
      await rm(dir, { recursive: true, force: true });
  ...
  it('readAndVerify reads + validates a well-shaped file', async () => {
    ...
    try {
      ...
      expect(manifest.gemini).toBe('demo-bot');
