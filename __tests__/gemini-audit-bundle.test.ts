...
//
// iter 102 — `gemini audit --bundle` emits the full audit verdict
...
beforeAll(async () => {
  const distPath = resolve(REPO_ROOT, 'packages', 'create-agent-gemini', 'dist', 'audit-cmd.js');
  if (!existsSync(distPath)) throw new Error('build first');
  const mod = await import(`file://${distPath}`);
  auditCmd = mod.auditCmd;
});

describe('gemini audit --bundle (iter 102)', () => {
  ...
  it('non-bundle (default) mode still produces text output, not JSON', async () => {
    ...
    try {
      ...
      expect(txt).toMatch(/gemini audit —/);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
