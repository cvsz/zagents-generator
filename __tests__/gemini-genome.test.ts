...
//
// iter 110 — `gemini genome <repo>` (18th subcommand). The user's named
...
beforeAll(async () => {
  ...
  if (!existsSync(join(distDir, 'genome.js'))) throw new Error('build first: cd packages/create-agent-gemini && npm run build');
...
describe('gemini genome (iter 110)', () => {
