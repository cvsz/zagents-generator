...
//
// iter 97 — `gemini export-config` emits MCP servers + claims +
...
describe('gemini export-config (iter 97)', () => {
  it('emits a parseable JSON for a freshly scaffolded gemini', async () => {
  ...
  it('exits 2 + FAIL message when no manifest at path', async () => {
    ...
    try {
      ...
      expect(r.lines.join('\n')).toMatch(/FAIL no \.gemini\/manifest\.json/);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
