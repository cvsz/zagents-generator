...
describe('scripts/dev-toolkit.mjs', () => {
  ...
  it('default run exits 0 and lists all 4 sections', async () => {
    ...
    expect(r.stdout).toMatch(/gemini subcommands/);
    expect(r.stdout).toMatch(/CI matrix/);
  });

  it('lists every gemini subcommand (currently 16)', async () => {
    ...
    for (const s of subs) {
      expect(r.stdout, `missing subcommand: ${s}`).toContain(`gemini ${s}`);
    }
  });
  ...
  it('--filter=release narrows to release-related entries', async () => {
    ...
    expect(r.stdout).not.toMatch(/gemini mcp\b/);
  });
