...
//
// iter 117 — verifies the new zagents subcommand router. Per the user's
// directive: "Before generation: zagents. Inside generated gemini: gemini."
//
...
describe('zagents subcommand router (iter 117)', () => {
  ...
  it('analyze runs analyze-repo against a real local dir', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'zagents-analyze-'));
    try {
  ...
  it('genome runs the genome command against a real dir', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'zagents-genome-'));
    try {
      ...
      expect(r.out).toMatch(/gemini genome|Repo profile|Scorecard|exit/);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
  ...
  it('genome with missing path returns exit 2 with usage', async () => {
    ...
    expect(r.code).toBe(2);
    expect(r.out).toMatch(/Usage: gemini genome/);
  });
  ...
  it('bare name (back-compat) still works — falls through to legacy scaffold', async () => {
    ...
    expect(r.code).toBe(2);
    expect(r.out).toMatch(/Usage: npx zagents/);
  });
  ...
  it('unknown first-arg verb (not a subcommand, not a flag) falls through to legacy scaffold', async () => {
    ...
    const dir = await mkdtemp(join(tmpdir(), 'zagents-bare-'));
    ...
    try {
      ...
      expect(r.err).not.toMatch(/Usage: npx zagents from-repo/);
    } finally {
