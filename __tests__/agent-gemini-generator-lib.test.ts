...
//
// iter 116 — verifies the @cvsz/zagents-generator library wrapper
...
describe('@cvsz/zagents-generator (iter 116)', () => {
  it('re-exports the core scaffold API', async () => {
    const mod = await import('@cvsz/zagents-generator');
  ...
  it('validateHarnessName accepts/rejects expected names', async () => {
    const { validateHarnessName } = await import('@cvsz/zagents-generator');
  ...
  it('TEMPLATES length matches the canonical 20', async () => {
    const { TEMPLATES, loadCatalog } = await import('@cvsz/zagents-generator');
