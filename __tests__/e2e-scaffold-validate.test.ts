...
//   1. scaffold() a minimal gemini into a tmpdir
//   2. assert the file shape the rest of the system relies on
//   3. run `gemini validate` (umbrella) on the scaffolded result
...
describe('e2e: scaffold → validate', () => {
  it('minimal/claude-code scaffolds, then `gemini validate` reports HEALTHY', async () => {
    ...
    try {
      ...
      expect(existsSync(join(dir, '.gemini', 'manifest.json'))).toBe(true);
      expect(existsSync(join(dir, '.gemini', 'manifest.sha256'))).toBe(true);
  ...
  it('scaffolds for every host without throwing', async () => {
    ...
    for (const host of hosts) {
      ...
      try {
        ...
        expect(existsSync(join(dir, '.gemini', 'manifest.json')), `host=${host} missing manifest`).toBe(true);
      } finally {
        await rm(dir, { recursive: true, force: true });
      }
    }
  ...
  it('scaffold output passes path-guard (no hardcoded /tmp/, C:\\, /Users/)', async () => {
    ...
    try {
      ...
      const { validate } = await import('../packages/create-agent-gemini/src/validate.js');
      const v = await validate([dir, '--skip-gcp']);
      ...
      // paths, every user-generated gemini inherits that bug.
  ...
  it('validate umbrella reports HEALTHY for EVERY host (not just claude-code)', async () => {
    ...
    // doesn't throw. This checks that `gemini validate` accepts the
