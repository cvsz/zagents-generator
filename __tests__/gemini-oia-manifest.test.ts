...
//
// iter 121 — `gemini oia-manifest <path>` (21st subcommand). Implements
...
async function makeHarness(opts: { name?: string; mcp?: boolean; witness?: boolean; auditLog?: boolean } = {}): Promise<string> {
  ...
  await writeFile(join(dir, 'package.json'), JSON.stringify({
    name: opts.name ?? 'demo-gemini',
    version: '0.1.0',
  }), 'utf-8');
  await mkdir(join(dir, '.gemini'), { recursive: true });
  await writeFile(join(dir, '.gemini', 'manifest.json'), JSON.stringify({ name: opts.name ?? 'demo-gemini' }), 'utf-8');
  if (opts.mcp ?? true) {
    await writeFile(join(dir, '.gemini', 'mcp-policy.json'), JSON.stringify({
      defaultDeny: true,
  ...
  if (opts.witness) {
    await writeFile(join(dir, '.gemini', 'witness.json'), JSON.stringify({ sig: 'fake' }), 'utf-8');
  }
  return dir;
}

describe('gemini oia-manifest (iter 121 — ADR-034)', () => {
  it('emits .gemini/oia-manifest.json with the full ADR-034 §72 shape', async () => {
    const dir = await makeHarness({ name: 'test-gemini', mcp: true, auditLog: true });
    try {
      ...
      expect(m.harnessId).toBe('test-gemini@0.1.0');
      // 9 layers
  ...
  it('--check on missing manifest returns exit 2', async () => {
    ...
    try {
      ...
      expect(r.lines.join('\n')).toMatch(/no \.gemini\/oia-manifest\.json/);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
  ...
  it('--check on corrupted manifest returns exit 1 DRIFT with reasons', async () => {
    ...
    try {
      await mkdir(join(dir, '.gemini'), { recursive: true });
      await writeFile(join(dir, '.gemini', 'oia-manifest.json'), JSON.stringify({
        schema: 99,
        oiaVersion: 'wat',
      }), 'utf-8');
      const r = await oiaManifestCmd([dir, '--check']);
  ...
  it('--dry-run prints the manifest, does NOT write', async () => {
    ...
    try {
      ...
      // No file written
      expect(existsSync(join(dir, '.gemini', 'oia-manifest.json'))).toBe(false);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
  ...
  it('manifest reflects MCP state — mcpFull on, audit on, no witness', async () => {
    ...
    try {
      await oiaManifestCmd([dir]);
      const m = JSON.parse(readFileSync(join(dir, '.gemini', 'oia-manifest.json'), 'utf-8'));
      ...
      expect(m.adjacentStandards.mcp.policyPath).toBe('.gemini/mcp-policy.json');
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });

  it('manifest reflects MCP-off gemini — all MCP-tied spans partial', async () => {
  ...
  it('checkOiaManifest passes a freshly-built object', () => {
    const m = buildOiaManifest({
      ...
      hasMcp: true,
      mcpMode: 'local',
      mcpPolicyPath: '.gemini/mcp-policy.json',
      hasWitness: true,
      hasAuditLog: true,
    });
  ...
  it('missing args returns exit 2 with usage', async () => {
    ...
    expect(r.code).toBe(2);
    expect(r.lines.join('\n')).toMatch(/Usage: gemini oia-manifest/);
  });
});
