...
//   - It's the exact regression class create-agent-gemini@0.1.0 hit
...
describe('npm pack content invariants', () => {
  ...
  it('zagents (in packages/create-agent-gemini/) ships dist/ + templates/ + the bin entrypoints', async () => {
    ...
    expect(fileExistsInPack(packed, 'dist/gemini-bin.js'), 'missing dist/gemini-bin.js (gemini binary)').toBe(true);
  }, 30_000);
