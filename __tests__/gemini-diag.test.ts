...
//
// iter 66 — tests for the `gemini diag` subcommand.
//
// Covers:
//   - skewVerdict() pure-function contract
//   - formatDiagReport() output shape + exit codes
//   - end-to-end: scaffold a real gemini, run diagCmd, assert PASS
...
let diag: typeof import('../packages/create-agent-gemini/dist/diag.js');
...
describe('skewVerdict pure function', () => {
  ...
  it('ignores pre-release suffix when comparing', () => {
    ...
    // for the diag purposes (the gemini was scaffolded against a build
...
describe('formatDiagReport', () => {
  ...
  it('exits 1 + FAIL line + actionable on major-diff', () => {
    const r = diag.formatDiagReport({
      ...
      verdict: 'major-diff',
      actionable: 'Run: npm install @zagents/kernel@0.1.0 (major skew — APIs may break)',
    });
    expect(r.code).toBe(1);
    expect(r.lines.join('\n')).toMatch(/FAIL MAJOR skew/);
    expect(r.lines.join('\n')).toMatch(/npm install @zagents\/kernel@0\.1\.0/);
  });
  ...
  it('exits 1 on minor-diff', () => {
    const r = diag.formatDiagReport({
      ...
      verdict: 'minor-diff',
      actionable: 'Run: npm install @zagents/kernel@0.1.0',
    });
    expect(r.code).toBe(1);
  });
  ...
  it('exits 2 + FAIL on no manifest at path', () => {
    ...
    expect(r.lines.join('\n')).toMatch(/no \.gemini\/manifest\.json found/);
  });
});
...
describe('generator-version skew (iter 71)', () => {
  ...
  it('generator-skew is INFORMATIONAL — never changes exit code', () => {
    ...
    expect(out).toMatch(/gemini upgrade/);
  });
...
describe('--bundle output (iter 90 MILESTONE)', () => {
  it('emits a complete SupportBundle JSON for a fresh scaffold', async () => {
    ...
    try {
      ...
      // gemini block — only @zagents/* deps
      expect(b.gemini.packageName).toBe('bundle-bot');
      expect(Object.keys(b.gemini.rufloDeps)).toContain('@zagents/kernel');
      // manifest present + read
      expect(b.manifest.present).toBe(true);
      expect((b.manifest.content as any).meta.kernel_version).toBe('0.1.0');
      // harnessFiles enumerates .gemini/* (iter 90 fix — readdirSync
      // import works in ESM context, not require)
      expect(b.harnessFiles).toContain('.gemini/manifest.json');
      expect(b.harnessFiles).toContain('.gemini/manifest.sha256');
...
describe('--json output (iter 73)', () => {
  it('emits parseable JSON with the full DiagReport + exitCode', async () => {
    ...
    try {
      ...
      // against the locally-resolved @zagents/kernel — legitimately
      ...
      // (0.1.0, this test's input) against the LOCAL zagents version,
...
describe('diagCmd end-to-end', () => {
  it('reports PASS on a freshly scaffolded gemini (manifest kernel === local kernel)', async () => {
