...
describe('.codex/skills/*/skill.toml manifests', () => {
  ...
  it('create-gemini lists all 6 supported hosts', async () => {
    const raw = await readFile(join(SKILLS_DIR, 'create-gemini', 'skill.toml'), 'utf-8');
  ...
  it('the 4 new skills (create/publish/validate/gemini-secrets) are present', async () => {
    const skills = await readdir(SKILLS_DIR);
    expect(skills).toContain('create-gemini');
    expect(skills).toContain('publish-gemini');
    expect(skills).toContain('validate-gemini');
    expect(skills).toContain('gemini-secrets');
  });

  // iter 70 — every gemini-lifecycle skill that exists on disk now,
  // pinned so dropping one is visible in code review.
  it('all 7 codex skills are present (iter 70: + diag-gemini)', async () => {
    const skills = await readdir(SKILLS_DIR);
    const expected = [
      'create-gemini',      // iter 22
      'publish-gemini',     // iter 22
      'validate-gemini',    // iter 22
