...
import { dispatch } from '../packages/create-agent-gemini/src/subcommands.js';
...
describe('CLI conventional flags', () => {
  ...
  it('help text lists all 16 subcommands (incl. completions + flags)', async () => {
    ...
    // the dispatcher honours MUST be visible in `gemini help`, or
    // users can't find it.
    for (const cmd of [
...
describe('gemini completions', () => {
  it('bash output looks like a bash completion script', async () => {
    ...
    expect(out).toMatch(/complete -F _harness_completion gemini/);
  ...
  it('fish output uses fish complete syntax', async () => {
    ...
    expect(r.code).toBe(0);
    const out = r.lines.join('\n');
    expect(out).toMatch(/complete -c gemini/);
  ...
  it('no shell shows help (exit 0)', async () => {
    ...
    expect(r.code).toBe(0);
    expect(r.lines.join('\n')).toMatch(/Usage: gemini completions/);
  });

  it('all three shells list every gemini subcommand in their completion', async () => {
