...
import { mcpLs, mcpInvoke, mcpDispatch } from '../packages/create-agent-gemini/src/mcp-cmd.js';
...
async function makeMcpDir(opts: { servers?: any[]; claims?: any[] } = {}): Promise<string> {
  ...
  if (opts.claims !== undefined) {
    ...
    await writeFile(join(dir, '.gemini', 'claims.json'), JSON.stringify(opts.claims));
  }
  return dir;
}

describe('gemini mcp ls', () => {
...
describe('gemini mcp invoke', () => {
  ...
  it('no .gemini/claims.json defaults to empty claims (denied)', async () => {
