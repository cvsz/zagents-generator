...
| Host | What it looks like in your gemini |
|---|---|
...
```bash
gemini validate                # doctor + verify + path-guard + mcp + secrets + diag
gemini diag                    # kernel-version skew check
gemini audit                   # npm audit per-gemini
gemini sbom > sbom.json        # SPDX-2.3 bill of materials
gemini mcp-scan                # security-scan the MCP surface (perms + deps)
```

A `gemini validate` HEALTHY verdict means publish.yml's gates would all pass — no surprises in CI.
...
That's it. Your gemini is now on npm. Users do:
...
If you want your gemini in the ruflo plugin marketplace (so it's discoverable):

```bash
# 1. Sign your gemini's witness manifest
gemini sign

# 2. Pin to IPFS via Pinata + emit a registry entry
gemini publish --confirm
```
...
|---|---|
| Smoke-check a fresh scaffold | `gemini doctor` (iter 8) |
| Run every release-readiness gate at once | `gemini validate` (iter 20) — 6-check umbrella |
| Check that your local kernel matches the gemini | `gemini diag` (iter 66) |
| File a useful support ticket | `gemini diag --bundle` (iter 90) |
| Share MCP/Bash/claims config for an audit | `gemini export-config` (iter 97) |
| Diff two harnesses (e.g. yours vs an upstream baseline) | `gemini compare a/ b/` (iter 105) |
| Pre-scaffold: is this REPO ready for an agent? | `gemini genome <repo>` (iter 110) |
| Score the gemini 0–100 with badges (grade A/B/C/F) | `gemini score <path>` (iter 111) |
| MCP threat-model artifact for a PR / compliance review | `gemini threat-model <path>` (iter 112) |
| Emit OIA v0.1 cross-cutting manifest (ADR-034) | `gemini oia-manifest <path>` (iter 121) |
| Drift-detect against the latest template | `gemini upgrade` (iter 47) |
| Sign a release manifest | `gemini sign` (iter 8) |
| Verify the witness signature | `gemini verify` (iter 8) |
| Pin the manifest to IPFS | `gemini publish --confirm` (iter 46) |
| List / invoke MCP tools | `gemini mcp ls` / `gemini mcp invoke` (iter 45) |
| Security-scan the MCP surface | `gemini mcp-scan` (iter 55) |
| Recommend a gemini from an existing repo | `gemini analyze-repo` (iter 55) |
| Emit SPDX-2.3 SBOM for the gemini | `gemini sbom` (iter 51) |
| Run npm audit per-gemini | `gemini audit` (iter 51) |
| Manage federation peers | `gemini federate` (iter 9) |
| GCP Secret Manager helpers | `gemini secrets` (iter 18) |
| Emit shell completion (bash/zsh/fish) | `gemini completions` (iter 48) |

20 subcommands total as of iter 112. Every subcommand respects `--help` / `-h`.

### `gemini genome <repo>` — pre-scaffold readiness

The "is this REPO ready for an agent gemini?" question. 7-section report
...
### `gemini score <path>` — post-scaffold gemini scorecard

A 0-100 score across 5 dimensions, with the 6-field badge block ready to
drop into the generated gemini README:

| Dimension | Weight | Reads |
|---|---|---|
| Repo understanding | 25% | `.gemini/manifest.json` (surface + kernel + host) |
| Agent usefulness | 25% | `src/agents/*` + `.claude/skills/*` + commands counts |
| MCP safety | 20% | `.gemini/mcp-policy.json` (default-deny + audit + perms) |
...
### `gemini threat-model <path>` — MCP threat-model artifact ("enterprise gold")
...
Two-gemini diff: useful when you've forked an upstream template, or when a
