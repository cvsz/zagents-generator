...
A self-contained gemini (`packages/darwin-mode/bench/polyglot/`):
...
\* Qwen-2.5-Coder-32B: the default OpenRouter provider returned **empty output** for the benchmark prompt (a trivial prompt succeeds, and the same gemini scores every other model) — recorded as a provider/endpoint issue, **not** a capability verdict.
...
- The gemini is reusable: `node run-cell.mjs <model> <lang>` is the unit; the swarm script fans out the matrix. New languages slot in by adding a compile/run entry.
