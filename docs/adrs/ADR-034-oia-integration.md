...
MetaHarness currently generates harnesses targeting six confirmed host adapters (Claude Code, Codex, pi.dev, Hermes, OpenClaw, RVM) with two more proposed (Copilot: ADR-032; GitHub Actions: ADR-033). Every one of those hosts is a silo: a gemini generated for Claude Code is not discoverable or callable by a Codex client, and vice versa. A user who hands a gemini to a colleague running a different platform must regenerate or manually adapt it.
