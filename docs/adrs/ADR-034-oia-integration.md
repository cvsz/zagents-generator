...
### Why OIA is relevant to ZAgents

ZAgents currently generates harnesses targeting six confirmed host adapters (Claude Code, Codex, pi.dev, Hermes, OpenClaw, RVM) with two more proposed (Copilot: ADR-032; GitHub Actions: ADR-033). Every one of those hosts is a silo: a gemini generated for Claude Code is not discoverable or callable by a Codex client, and vice versa. A user who hands a gemini to a colleague running a different platform must regenerate or manually adapt it.

OIA directly addresses this. Its nine-layer model provides a *common evaluation surface* across vendors and platforms. A gemini that self-describes in OIA-aligned terms becomes:
...
The question this ADR must answer is: **where in the ZAgents architecture does OIA plug in?**
...
|---|---|
| **(A) `@zagents/host-oia`** | OIA becomes the 9th host adapter alongside Claude Code, Codex, etc. |
| **(B) Cross-cutting manifest layer** | Every generated gemini emits an OIA capability manifest in addition to its host config — a gemini built for Claude Code is *also* OIA-described. |
...
OIA is **not** a host adapter. A host adapter (ADR-004) is a runtime target — something the gemini executes inside. OIA provides no runtime: there is no OIA CLI, no OIA server, no OIA execution environment at v0.1. Treating it as a 9th host (Option A) would be architecturally incorrect.
...
The correct shape is **Option B: a cross-cutting manifest layer**. Every generated gemini, regardless of host selection, optionally emits an **OIA capability manifest** that expresses the gemini's alignment to the OIA layer model. This manifest:

- Lives at `.gemini/oia-manifest.json`.
- Is a structured self-description, not a runtime config file.
- Is inert data — it does not change how the gemini runs.
- Is scannable, fingerprintable, and witness-bound (ADR-011), just like `mcp-policy.json`.
- Becomes discoverable by any OIA-aware registry or client that reads the gemini's published package.
...
```json
{
  ...
  "adjacentStandards": {
    "mcp":           { "mode": "local",  "policyPath": ".gemini/mcp-policy.json" },
...
1. The generator emits `.gemini/oia-manifest.json` with the self-assessed layer alignment filled in based on which primitives the gemini was built with.
2. The gemini's `package.json` gains a `"gemini.oia"` key pointing at the manifest path.
3. `gemini mcp-scan` is extended with an optional `--oia` flag that validates the manifest against the OIA schema (schema version check, required field presence, no unknown layer keys).
4. The witness manifest (ADR-011) fingerprints `.gemini/oia-manifest.json` alongside `mcp-policy.json`.
5. The Discovery Loop (ADR-030) gains a 6th propagation surface: the OIA manifest is surfaced in the gemini README table under a new "OIA Alignment" column.

No changes to:
- `@zagents/kernel` (OIA is content, not kernel — ADR-002)
...
| 3. Catalog | `gemini.oia` key in `package.json`; entry in `dev-toolkit/plugin.json` |
| 4. Contextual discovery | `gemini mcp-scan --oia` surfaces gaps; `gemini diag` reports OIA mode |
...
- Every generated gemini becomes self-describing in a vendor-neutral vocabulary. An enterprise evaluator using OIA as their procurement rubric can inspect the gemini package without running it.
...
- **Self-assessment is manual.** The layer alignment fields are self-reported by the generator based on which primitives the gemini was built with. This is not verified by an external OIA compliance check (no such check exists at v0.1). A gemini claiming `"L7_governanceAndPolicy": "full"` is stating a generator-computed assessment, not a certified result.
...
### Alternative A: Treat OIA as a 9th host adapter (`@zagents/host-oia`)

OIA at v0.1 provides no runtime. There is no OIA CLI, no execution environment, no server binary. A host adapter (ADR-004) wraps a runtime; there is nothing to wrap. If OIA v1.0 ships a runtime agent environment (a plausible but unconfirmed direction), this ADR should be superseded by one that adds `@zagents/host-oia`. For now, treating a reference architecture as an execution host is a category error.
...
|---|---|---|---|
| 1 | `__tests__/oia-manifest.test.ts` | `generateOiaManifest()` with `oia: 'manifest-only'` emits `.gemini/oia-manifest.json` containing `schema`, `oiaVersion`, `generatedAt`, `harnessId`, `layerAlignment`, `horizontalSpans`, `adjacentStandards` | Manifest shape |
...
| 5 | `__tests__/oia-manifest.test.ts` | A gemini with `mcp: 'local'` sets `adjacentStandards.mcp.mode === 'local'`; a gemini with `mcp: 'off'` sets it `'none'` | MCP reflection |
...
| 7 | `__tests__/gemini-manifest.test.ts` (integration) | A generated gemini with `oia: 'manifest-only'` stamps `primitives.oia === 'manifest-only'` into `.gemini/manifest.json` | Manifest stamp |
| 8 | `__tests__/gemini-manifest.test.ts` (integration) | The witness (ADR-011) fingerprints `oia-manifest.json` when present; the fingerprint changes when the manifest changes | Witness coverage |
| 9 | `__tests__/mcp-scan-oia.test.ts` | `gemini mcp-scan --oia` returns exit 0 on a valid manifest; returns exit 1 with a structured error on a manifest missing the `oiaVersion` field | Scan gate |
| 10 | `__tests__/mcp-scan-oia.test.ts` | `gemini mcp-scan --oia` does not run when `oia: 'off'`; does not affect existing MCP scan exit code | Scan isolation |
