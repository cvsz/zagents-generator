...
gemini uses the Node.js built-in `fetch` API (available since Node 18, required at
...
```json
{
  ...
  "rules": [
    ...
    {
      "tool": "write_file",
      "comment": "Cache fetched sequences/abstracts to .gemini/cache/",
      "pathPrefix": ".gemini/cache/",
      "action": "allow"
    },
    {
      "tool": "audit_log",
      "comment": "Append every API call to .gemini/audit.jsonl",
      "action": "allow"
    }
  ]
}
...
`.gemini/cache/` — are denied by default.
...
- **No sandbox/test endpoint exists** — NCBI and Ensembl offer only production
  ...
  `tool` + `email` registration reducing block risk. The gemini defaults to
...
- `node-ncbi` is primarily PubMed-focused; Gene, Nucleotide, and Protein database
  queries use raw `fetch` against the E-utilities base URL. The gemini includes
  helper functions for these to avoid boilerplate in user-extended harnessses.
- `node-ncbi` is a CommonJS module; the gemini's `bin/scaffold.mjs` uses dynamic
