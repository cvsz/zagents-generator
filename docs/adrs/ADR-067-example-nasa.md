...
parameter on every request; if unset the gemini transparently falls back to
...
The scaffolded `.gemini/mcp-policy.json` grants exactly the following tools
...
`write_file` is granted only to the TLE cache path (`.gemini/cache/tle/`)
and the pass-log path (`.gemini/logs/passes.jsonl`). All network access is
restricted to the two whitelisted origins; MCP network tool calls to any other
host are denied. Every tool invocation is appended to `.gemini/audit.jsonl`.
...
- **Rate-limit guard**: the executor agent checks the `X-RateLimit-Remaining`
  ...
  if the value falls below 10. When `DEMO_KEY` is in use, the gemini enforces
...
- **TLE freshness warning**: if the cached TLE epoch is more than 48 hours old
  ...
  TLE age. The gemini does not auto-refresh large groups more than once per
...
- `satellite.js` v7 is ESM-only; consumers on older CommonJS toolchains must
  use a bundler or dynamic `import()`. The scaffolded gemini targets Node
...
- CelesTrak imposes a 2-hour cooldown per IP for bulk group downloads; the
  gemini caches TLE files locally to avoid repeated downloads within the
  cooldown window.
- SGP4 propagation accuracy degrades for objects in highly eccentric orbits
  or for very old TLEs. The gemini is illustrative, not suitable for safety-
