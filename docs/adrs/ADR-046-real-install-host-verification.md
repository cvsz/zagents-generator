...
> gemini defect. Run the gate from a plain shell for a clean 7/7.
...
3. **hermes** (fixed vs template): real hermes config is a nested schema
   ...
   `system_prompt`/`scrub_*`/`agents:` top-level keys. The gemini identity now
...
(`packages/host-*`), the CLI (`packages/create-agent-gemini/src/host-config.ts`),
