# ADR-026: CLI Repo Analyzer (`gemini analyze-repo`) with optional ruvllm embeddings
...
ADR-023 shipped Repo → Gemini in the browser; ADR-025 added optional in-browser MiniLM embeddings. The browser path is privacy-first and zero-install, but it is bounded by the GitHub API (rate limits, public-by-default) and by what is safe to fetch over the network. The "serious use" path the importer's own design called out — private repos, the full local tree, real byte parity — belongs on the CLI, where there is a filesystem and a Node runtime.
...
Add `gemini analyze-repo [path]` — a local, analysis-only repo analyzer that mirrors the browser importer's rule-based core, with `@ruvector/ruvllm` as an **opt-in** (`--embed`) deterministic embedding engine and a transparent lexical fallback.

### Same rules, Node-side

`analyze-repo.ts` ports the browser's pure core: `inventory()` (safe file walk), `analyzeFiles()`, the 8-archetype library, the auditable `0.45·semantic + 0.25·manifest + 0.15·ci + 0.10·structure + 0.05·intent` score, and `recommendPlan()`. Archetypes map to the same catalog templates and agent ids, so a CLI recommendation scaffolds a real gemini via the existing `scaffold()`. Outputs `repo-profile.json` + `gemini-plan.json`, and `--scaffold <name>` materialises the plan.
...
- `analyzeFiles` detects rust + `cargo build`/`cargo test` + CI; `recommendPlan` routes a Rust crate to `rust-crate-gemini` (lexical), commands `execution: disabled`.
...
- `analyzeRepoCmd` writes `repo-profile.json` + `gemini-plan.json`; `--scaffold` materialises the gemini into `--out` (never pollutes cwd).
