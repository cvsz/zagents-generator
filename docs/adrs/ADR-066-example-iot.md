...
hardware damage or injury. The gap between "demo that reads a topic" and "gemini
that can safely actuate" is exactly the design problem this example must address.

Two npm packages cover the realistic surface an agent gemini needs:
...
The generated `.gemini/mcp-policy.json` enables only the minimum surface:
...
- **Physical safety is out of scope.** The gemini enforces software-level safety
