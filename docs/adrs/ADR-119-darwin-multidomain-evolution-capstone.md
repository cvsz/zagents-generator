...
- The LLM fixes are real but **cached per bug** (constant per task), so this measures gemini evolution, not per-variant LLM variance. 5 calls total.
...
Gemini + result committed (`bench/experiments/swe-evolution.mjs`, `bench/results/swe-evolution.json`). 5 cached LLM calls. 350 tests unaffected.
