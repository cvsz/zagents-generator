...
This ADR pins down: which memory primitives ship in the kernel; how a generated gemini configures them; how the intelligence pipeline is parameterised per domain; how cross-host shared memory works; and how the learning loop applies at the gemini level (with the deeper exotic-mode wiring deferred to ADR-014).

## Decision

### The memory stack in the kernel

`@zagents/kernel/memory` ships seven layers. Each is independently configurable; together they form the default unified memory.
...
The kernel ships all seven. The gemini configures each via `gemini.config.json` `memory.*`.
...
The gemini's existing memory CLI surface (`npx <gemini> memory store/search/retrieve/list`) is generated against this contract. Every gemini gets the same CLI verbs.
...
| Self-evolving gemini mode (feature-flagged; see ADR-014) | `LearnedWeights` | Reserved for harnesses with `--features self-evolution`; the learned-weights inference helper feeds the self-evolution loop's decay-rate optimisation. |
...
A gemini configures decay per memory namespace in `gemini.config.json`:
...
`halfLifeHours: null` means no decay; the memory persists at full weight forever. Domain-specific defaults are shipped with vertical packs (ADR-013): the `@zagents/vertical-trading` pack ships short half-lives by default; `@zagents/vertical-legal` ships long ones.
...
The kernel exposes both int8 (the ruflo-measured 3.84× compression with cosine 0.99999 reconstruction) and RaBitQ (32× compression at 0.60 ms/query in a 14,760-vector index, per ruflo's existing intelligence-system audit). The gemini selects via `gemini.config.json` `memory.quantization`:
...
The composer warns when the user picks `none` for a `gemini.config.json` that suggests large memory needs (e.g. `--features federated-memory`).
...
Sparse BM25 (Lucene-style) plus dense HNSW plus cross-encoder reranking, exactly as ruflo's `hybrid-retrieval.ts` already implements. The kernel ships this verbatim; the gemini inherits.
...
JUDGE         Evaluate the candidate against current context.
              ...
              Pluggable: each gemini ships its own judge for its domain.
                 ↓
...
The pluggable nodes (JUDGE, DISTILL) are the path to vertical-specific intelligence. A trading gemini's JUDGE asks "did this strategy beat the benchmark this week?" A legal gemini's JUDGE asks "did this answer hold up under attorney review?" The kernel ships interfaces; the gemini ships implementations.

```ts
// @zagents/kernel/memory/intelligence.ts (simplified)
...
// Gemini wires in custom providers:
import { configureIntelligence } from '@zagents/kernel/memory';
...
The default JudgeProvider is a verdict heuristic on the trajectory's `status` field. The default DistillProvider is a LoRA-style pattern adapter. Both are good enough for the trivial-tier gemini; both are replaced for the exotic-tier.
...
A generated gemini's memory is **single-instance per gemini installation**. The same physical AgentDB lives at `${HARNESS_DATA_PATH}/memory/` (default: `./data/memory` per ruflo's `CLAUDE_FLOW_MEMORY_PATH` env var convention). It is shared across the host invocations of that gemini — a session under Claude Code and a session under Codex hit the same database.

This is the load-bearing payoff of the kernel design. The gemini's intelligence pipeline is host-agnostic. The learning a user gets from one host carries into another.

A user with multiple gemini installations (one per project) gets isolated memories per gemini. A user who wants cross-gemini sharing opts into federation (ADR-014) and shares via the federation transport.
...
A gemini adds its own namespaces in `gemini.config.json` `memory.namespaces`. The vertical packs ship their own.

### Memory bridge with Claude Code's auto-memory

Ruflo already bridges Claude Code's `~/.claude/projects/*/memory/*.md` into AgentDB (per the `CLAUDE.md` §Claude Code ↔ AgentDB Memory Bridge section, MCP tool `memory_import_claude`). The kernel ships this bridge. A generated gemini that selects the Claude Code adapter (ADR-004) gets the bridge wired automatically. The MCP tool name and behaviour are unchanged.

### Pretraining and pattern transfer

The kernel exports `hooks.pretrain(history)`, which seeds the intelligence pipeline from a history corpus (the ruflo `hooks pretrain` command, generalised). A new gemini installation typically does not start cold — the generator can seed it with patterns from:

- A vertical-pack corpus (each pack ships a small bootstrap pattern set; see ADR-013).
- A previous gemini's export (`npx <gemini> memory export | npx <other-gemini> memory import`).
...
A subset of memory entries are witness-attested. By default: entries in the `verifications` namespace. A gemini operator who wants strong attestation on more namespaces configures it in `gemini.config.json`. The witness scripts (ADR-011) treat attested memory as part of the manifest. The same Ed25519 signature covers the whole manifest. Adversarial mutation of attested memory invalidates the signature.

## Consequences

### What gets easier

- **Domain-specific intelligence without forking the kernel.** A trading gemini gets a trading JudgeProvider, the kernel is unchanged.
- **Cross-host continuity.** A user who hops between Claude Code and Codex on the same gemini sees the same memory.
...
- **Namespace explosion.** Without discipline, a gemini with 30 plugins has 50 namespaces. The composer surfaces namespace count and warns when crossing thresholds.
...
A more pluggable backend layer. Rejected for v1.0 because AgentDB + SQLite already covers the gemini use case (single-machine, embedded storage), and supporting Postgres / Redis introduces a network dependency the trivial-tier gemini should not have to think about. A future ADR can add a `MemoryBackend` interface if a real user pull emerges. For now, AgentDB is the one backend, configurable but not pluggable.
