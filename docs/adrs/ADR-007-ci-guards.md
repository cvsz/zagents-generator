...
1. **`ruvnet/agent-gemini-generator`** — publishes the kernel (`@metaharness/kernel`), the generator (`@metaharness/create-agent-gemini` and its marketplace plugin), the host adapters (`@metaharness/host-*`), the catalogue (`@metaharness/catalogue`), the test gemini, and the vertical packs (`@metaharness/vertical-*`).
...
- **Generator-side gates** (run on `ruvnet/agent-gemini-generator` PRs and releases) — Section A below.
...
These run in `.github/workflows/` of the `ruvnet/agent-gemini-generator` repo.
...
- **Asserts**: the kernel's source does not `import` from outside `@metaharness/kernel/*` or its direct dependencies. Enforced by ESLint rule `import/no-restricted-paths`. No file under `packages/kernel/src/` may resolve a path to `ruflo`, `@claude-flow/*`, or any gemini package.
- **Failure mode**: PR blocked.
...
- **Asserts**: `vitest run` across all packages. Coverage on `packages/kernel/` ≥ 85%, on `packages/create-agent-gemini/` ≥ 80%, on `packages/host-*/` ≥ 75%.
- **Failure mode**: PR blocked.
...
- **Triggers on**: every PR.
- **Asserts**: a trivial-tier scaffold (`create-agent-gemini ci-trivial --hosts claude-code --no-interactive`) builds, passes its bundled smoke test, and `npm pack` produces a tarball that resolves.
- **Failure mode**: PR blocked.
...
- **Asserts**: every host adapter's `peerDependencies.@metaharness/kernel` includes the current kernel version. The catalogue's `engines.kernel` includes the current kernel version. The test gemini's `dependencies.@metaharness/kernel` is the exact current kernel version.
...
- **Triggers on**: pushes to `main` and pre-release.
- **Asserts**: the bundled templates in `packages/create-agent-gemini/templates/` are coherent with the current `@metaharness/kernel` version. Specifically: every `import` in the templates resolves to an exported symbol in the kernel at the version pinned in `_base/package.json.hbs`.
- **Failure mode**: pre-release blocked.
...
## Section B — Gemini-side CI gates (scaffolded into every generated gemini)

Every generated gemini ships `.github/workflows/` files. These are templates the generator writes; the gemini author can modify them but should not delete the required gates.

### B1. `gemini-ci.yml` — every PR

Scaffolded as `.github/workflows/ci.yml` in the gemini.
...
jobs:
  test:
    ...
    steps:
      ...
      - run: npm run build
      - run: npm test
      - run: npx <gemini-name> doctor --strict       # built-in self-check
      - run: npx <gemini-name> smoke --offline       # offline smoke contract
      - run: npx <gemini-name> witness verify        # ADR-011 verification
```

What this asserts:
- TypeScript compile clean.
- Unit tests pass.
- The gemini's own doctor command (built from the kernel's doctor + gemini-specific checks) reports green.
- The smoke contract (the same contract the kernel's test gemini runs, parameterised for this gemini) passes.
- The witness manifest verifies against the live tree.

### B2. `gemini-multi-host-smoke.yml` — every PR

For every host the gemini declares in its `gemini.config.json` `hosts[]`, run that host's adapter smoke test against an installed copy of that host. Matrix job.
...
jobs:
  smoke:
    ...
    steps:
      ...
      - run: npx <gemini-name> smoke --host ${{ matrix.host }}
```

### B3. `gemini-anti-slop.yml` — every PR
...
### B4. `gemini-publish.yml` — release tags only
...
5. If the gemini has a marketplace plugin (ADR-005 §5), run the marketplace publish flow (Pinata upload, registry update, CID commit).
...
### B5. `gemini-drift-check.yml` — weekly cron

Once a week, the gemini's CI runs `npx <gemini-name> drift check` (the ADR-008 drift-detection command). It opens a PR if drift is detected — the PR contains the diff against the current template and a proposed migration. The human reviews and merges.

This is the load-bearing gate that catches the "kernel shipped 1.3.0 but my gemini is pinned to 1.2.0 and now its template is out of date" failure mode.

### B6. `gemini-secret-scan.yml` — every PR

Mirrors A12. Generated into the gemini so the gemini's own CI catches secret commits, not just the generator's CI.

### B7. `gemini-perf-smoke.yml` — pre-release only

Asserts that the gemini's measured boot time and memory-search latency are within budget. The budget is configurable in `gemini.config.json` `perf.budgets.*`; the generator ships sensible defaults from ruflo's measured numbers (kernel startup ≤ 500 ms, memory-search ≤ 5 ms at N=20k).

## Branch protection rules

Both repos (the generator and every generated gemini) enable GitHub branch protection on `main`:
...
The generator ships scripts to apply these rules to a new gemini on first push (`gh api -X PUT /repos/{owner}/{repo}/branches/main/protection` calls).
...
Generated harnesses are scaffolded with the same secret names; the gemini author provisions their own values.

## Required CODEOWNERS

`.github/CODEOWNERS` ships into both the generator repo and (templated) every generated gemini:

```
# Generator repo
packages/kernel/             @ruflo-kernel-team
packages/create-agent-gemini/  @ruflo-generator-team
...
The gemini-side suite (B1-B3 on PR; B4 on release) runs in ≤ 10 minutes wall-clock for a default gemini. Larger harnesses (more plugins, more hosts) scale linearly; the budget is enforced per-gemini via `gemini.config.json` `ci.budgetMinutes`.
...
- **A new gemini inherits sane CI from day one.** The author does not have to know how to wire branch protection or npm provenance.
...
- **Gate maintenance is real work.** Sixteen generator-side gates + seven gemini-side gates is a non-trivial CI codebase. Each gate has a `.github/workflows/*.yml` file + a `scripts/ci/*.mjs` it invokes. We commit to keeping them small and well-tested.
...
- The gemini author still chooses what tests they write beyond the smoke contract. The gates we scaffold are the minimum; everything above that is theirs.
...
### Alternative 4: Make `npm-provenance` mandatory on all gemini releases

Reject any gemini publish without `--provenance`. Rejected per ADR-005 §Alternative 5 — gating absolutely on provenance excludes non-GitHub CI users. Anti-slop (ADR-009) treats provenance as a quality signal, not a gate.
...
2. **All seven gemini-side gates** are scaffolded by the generator into every produced gemini. Asserted by ADR-003 §Test Contract §5 (trivial-gemini end-to-end check — every generated workflow file is present and lints).
...
4. **CI wall-clock measurement** — a workflow-of-workflows runs the full suite against the current `main` weekly and posts to the `metrics` channel. If the wall-clock crosses 15 minutes (generator) or 10 minutes (gemini, measured on a default scaffolded gemini), the trend is investigated.
...
- `v3/docs/adr/ADR-102-plugin-hook-cli-flag-regression-ci-guard.md` — the existing ruflo CI smoke gemini for plugin flag regressions.
