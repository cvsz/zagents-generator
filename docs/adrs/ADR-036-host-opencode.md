# ADR-036: OpenCode as a gemini host (proposed 8th host)
...
These are the integration shapes the gemini has to target. The user's iter-127 directive — `add new /adr for opencode` — turns this from a possible-future-host into a Proposed ADR matching the same template as ADR-032 (Copilot) and ADR-033 (GHA).
...
3. **Open-weights / open-source friendliness.** OpenCode users self-host their model layer; the gemini composes cleanly with whatever provider they bring. This is the same target audience as Hermes (ADR-004) and OpenClaw (ADR-011) — the user base that the existing hosts already serve.
...
### Manifest fields (additive to `.gemini/manifest.json`)
...
`opencode_min_version` is an advisory floor — if OpenCode renames the config keys in a future minor (analogous to the Claude Code config-path migration that happened in 2025), `gemini diag` (ADR-027) gains a new warning surface.
...
The `mcp.servers` map is a direct re-export of the gemini's `src/mcp/server.ts` registration table — no per-host transformation beyond the JSON wrapping. The `permissions` block is a copy of `.gemini/mcp-policy.json` projected to OpenCode's key naming.

### Default-deny composition

OpenCode evaluates `mcp.permissions.deny` BEFORE `mcp.permissions.allow`. The adapter MUST emit the deny rules from `.gemini/mcp-policy.json` verbatim — the gemini's default-deny posture (ADR-022) wins by being copied into OpenCode's own enforcement gate, not by trusting OpenCode to enforce a separate default.
...
2. Ship a `gemini diag --opencode-version-check` flag that surfaces the schema-drift case explicitly (mirrors the kernel-version-skew pattern from iter 66).
...
| `packages/create-agent-gemini/src/index.ts` | `HOSTS` array gains `'opencode'` (7 → 8 entries) |
...
- **Dual codex/opencode footprint.** Users may scaffold for both because they look similar. Acceptable — the gemini output is the same npm package, just the per-host config files differ.
...
| 8 | `.github/workflows/published-smoke.yml` | The CI guard scaffolds `--host opencode` and runs `gemini doctor` against it |
