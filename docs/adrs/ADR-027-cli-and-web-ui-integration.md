...
**Status**: Accepted
**Date**: 2026-06-13
**Related**: ADR-003 (generator architecture), ADR-007 (CI guards), ADR-010 (TDD test contracts), ADR-019 (release orchestration), ADR-020 (web generator UI), ADR-021 (client-side packaging + Pages deploy), ADR-022 (MCP primitive), ADR-023 (Repo→Gemini importer), ADR-024 (Studio + Verify), ADR-026 (CLI repo analyzer)
...
By iter 55, the project has TWO generator surfaces that can produce the same scaffolded gemini:

1. **CLI generator** (`packages/create-agent-gemini/`)
   - Node-only, npm-published
   - `npx create-agent-gemini <name>` or `node packages/create-agent-gemini/dist/bin.js`
   - 12 `gemini` subcommands for the post-scaffold lifecycle (sign / verify / validate / publish / upgrade / mcp / federate / secrets / doctor / completions / sbom / audit)
...
2. **Web-UI generator** (`apps/web-ui/` — PR #1)
   ...
   - Two modes: full-gemini scaffolding (byte-compatible .zip download) + skill/agent/command authoring (SKILL.md folder with YAML frontmatter)
...
The CLI's `packages/create-agent-gemini/src/renderer.ts` is the **canonical** generator. Its behaviour — the `{{var}}` templating rules, `validateHarnessName()` constraints, per-host file shapes, the manifest fingerprint algorithm — defines what a "correct" gemini looks like.

The web-UI's generator (`apps/web-ui/src/generator/`) is a **behaviour-port**. It does NOT import from `packages/create-agent-gemini/` (different runtime constraints: no Node fs, must work in a browser sandbox). But it implements the same surface, and the **parity contract** below pins them together.
...
`.gemini/manifest.json`) is the load-bearing detail; if the surfaces drift here,
`gemini validate`'s `doctor` check would reject web-UI output as hand-edited, so
...
Asymmetric features are explicitly allowed; the **shared subset** (full-gemini scaffolding) stays **behaviourally equivalent** (same file set + manifest semantics, not byte-identical — see the status correction above):

| Feature | CLI | Web-UI |
|---|---|---|
| Full-gemini scaffolding | yes | yes (behaviourally equivalent to CLI) |
...
When a `gemini validate` umbrella check would reject a downstream gemini scaffolded by the web-UI as "wrong manifest hash", the cause is almost always **kernel version skew** — the CLI uses kernel v0.X.Y, the deployed Pages instance still uses v0.X.(Y-1). The parity test (point 2 above) is prevention; the kernel version in `meta.kernel_version` of the manifest is the diagnostic.
...
| CLI e2e lifecycle | `__tests__/e2e-lifecycle.test.ts` (iter 52) | All 12 gemini subcommands work against scaffold output |
...
- The byte-parity contract gives users a guarantee: a gemini scaffolded in the browser is bit-for-bit indistinguishable from one scaffolded in the terminal. `gemini validate` reports the same verdict for both.
