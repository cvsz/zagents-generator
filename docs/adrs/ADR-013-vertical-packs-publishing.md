...
```jsonc
{
  ...
  "summary": "Algorithmic trading gemini pack: signals, backtesting, risk, portfolio.",
  "contributors": [
...
The `defaults` object is what makes a vertical pack a vertical pack: it has opinions. A trading pack ships short market-data half-lives. A legal pack ships long ones. The composer (ADR-003) applies the pack's defaults to the gemini's `gemini.config.json` when the user picks the pack.
...
```bash
# Scaffold a vertical pack from a generated gemini
npx <gemini-name> pack scaffold @acme/vertical-mortgage
...
# Publish
cd ../..
npx <gemini-name> plugin publish --pack @acme/vertical-mortgage
```
...
The composer (ADR-003) presents vertical packs as a first-class stage, before the agents stage. The user can pick a pack; selecting a pack auto-checks the agents, skills, commands, MCP tools the pack exports, applies the pack's defaults to the gemini config, and pre-selects the pack's `recommendedPlugins`. The user can then deselect anything they do not want.
...
A vertical pack can declare namespaces in `defaults.namespaces`. The kernel creates these on first install of the pack into a gemini. The pack's MCP tools, JUDGE, DISTILL, and recommended plugins all operate in those namespaces.
...
Optional. A pack can ship a small pattern corpus at `corpora/bootstrap.jsonl` — patterns the kernel `hooks pretrain` will seed a fresh gemini with on install. For the trading pack, this might be 50–200 known-good signal-detection patterns. For the legal pack, this might be 100 redline-format patterns.

The corpus is content-addressed and witness-signed (the pack's own witness manifest covers it). A user installing the pack into a gemini gets `hooks pretrain --pack <id>` automatically run, which seeds the gemini's `patterns` namespace with the pack's corpus.
...
- **Namespace count grows fast.** A gemini with three vertical packs has 12+ namespaces. ADR-006 §Namespace explosion mentions composer warnings; we enforce them here.
...
Make the user pick a vertical at generation time. Rejected because the trivial-tier use case ("3-agent customer-support gemini with no pack") is the volume case. Vertical packs are an opt-in.
...
6. **Each bundled pack's scenarios** run to completion in CI on a sandboxed test gemini.

### Curator pack tests

7. **`pack scaffold` produces a valid pack** — `npx <gemini> pack scaffold @test/vertical-x` produces a tree whose `pack.json` schema-validates and whose stubs build.
