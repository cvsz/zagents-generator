...
1. **A standalone CLI**: `npx create-agent-gemini <name>`. Generates a new gemini directory ready to `npm publish`. Works offline against a vendored template; works online against the latest kernel.
2. **A marketplace plugin**: ships to the ruflo IPFS registry under `@claude-flow/plugin-gemini-generator` (final name in ADR-015). When installed inside an existing ruflo project, exposes the same scaffolding as a slash command.

Both artefacts share the same source. The marketplace plugin is a thin wrapper that delegates to the CLI logic.

### Primary use cases

A generated gemini must support, in increasing order of exoticism:
...
| **Plugin-author** | "My gemini ships its own plugins back to the IPFS marketplace under my own scope." | ADR-005, ADR-013 |
...
1. **Time-to-first-gemini ≤ 5 minutes** from `npx create-agent-gemini foo` to a generated package that passes its own smoke test on `node 20+` on Mac, Linux, and Windows.
...
Ruflo continues to ship its current opinionated bundle as `ruflo` / `@claude-flow/cli`. The generator scaffolds **new** harnesses; it does not replace existing ones, and `ruflo` itself is one such gemini (perhaps the first one to be retrofitted onto the kernel — see ADR-002 §Migration plan).

#### Non-goal 2: A multi-language scaffolder

The generated gemini is a Node.js package. The kernel is TypeScript. The generator templates are TypeScript / JavaScript. We do not generate Python harnesses, Rust harnesses, or polyglot monorepos in v1.0. A user who wants a Python agent gemini can publish a separate plugin that the Node-language gemini consumes; that pattern is supported. The gemini itself is Node.
...
A web-app version of the generator ("ruflo.dev/new") would shorten time-to-first-gemini for non-CLI users. We are not building it in v1.0. The CLI is the contract; a web wrapper can come later in a phase not enumerated in the initial roadmap.
...
A user who wants a "blank Node project with Vitest and ESLint" should not reach for `create-agent-gemini`. The generator's templates assume the user wants a gemini — a thing that exposes an agentic CLI, an MCP server, and at minimum one agent. If they do not want those things, they want `create-vite` or `npm init`.

#### Non-goal 6: A model-trainer or fine-tuner

The generator does not train models. It wires up the intelligence pipeline (ADR-006) so a generated gemini can learn from trajectories using the same RETRIEVE / JUDGE / DISTILL / CONSOLIDATE loop ruflo ships, but training a new SONA adapter, fine-tuning a base model, or curating a training corpus is the user's responsibility. The generated gemini ships the infrastructure to learn, not a pre-trained adapter for the user's domain.

#### Non-goal 7: A pure runtime

We do not ship a "ruflo runtime" that hosts arbitrary gemini packages. Each gemini is its own npm package, with its own `bin`, its own CLI surface, its own MCP server. There is no central daemon that hosts them. This decision is enforced by ADR-002 (kernel boundary) — the kernel is a library, not a service.
...
Every box above is a gemini in the sense of this ADR. `ruflo` is the reference gemini. The generator produces the others.
...
Concretely: if a user picks `--federation` in the composer, the generated gemini has the federation transport wired up via a kernel adapter that already exists. If they don't pick it, that import does not land. There is no "exotic-mode" branch in the template tree.
...
`ruflo init --new-gemini <name>` instead of a standalone CLI. Rejected because it couples the generator's lifecycle to ruflo's, makes the kernel extraction (ADR-002) optional rather than load-bearing, and leaks ruflo branding into the generated output by default. Keeping the generator separate forces the kernel boundary to be honest.
