...
- The natural next rung (not done autonomously — token/scope) is wiring this evaluator into `evolve()` over a real multi-task corpus so the gemini self-improves measured by real-LLM-real-test pass-rate at suite scale.
