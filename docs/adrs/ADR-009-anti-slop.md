...
1. **Smoke contract** — a plugin / gemini / pack that does not pass its bundled smoke contract is not publishable.
...
Every plugin, gemini, vertical pack, and host adapter ships a `smoke/` test directory. The contract:

- **For plugins.** A test that installs the plugin in a fresh test gemini, asserts the plugin loads, asserts every declared export (agents, skills, MCP tools, hooks, commands per the `exports` field in ADR-005 §2) is reachable, and the plugin's `init` lifecycle returns without error.
- **For harnesses.** The gemini-level smoke from ADR-007 §B1.
...
- **`memory`** — read/write to the gemini's memory namespaces.
...
At install time, the gemini inspects the plugin's permissions and either (a) grants automatically (if they match the gemini's default allowlist), (b) prompts the user (CLI: print the permission list, ask y/N), or (c) refuses (if the permission is forbidden by `gemini.config.json` `plugins.deniedPermissions`).
...
- For independence-mode harnesses (ADR-015) that point at their own registry, the gemini operator is the one maintaining their advisory feed. They can subscribe to ruflo's feed and re-export.
