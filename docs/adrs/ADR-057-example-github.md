...
GitHub provides no "sandbox" API endpoint or test-key mechanism analogous to Stripe. Safe-by-default for a GitHub gemini therefore means: read-only by default, mutations require an explicit `--allow-writes` opt-in flag, and all agent actions against real repos are prefixed with a read-back verification step before any write is submitted. The conventional best practice in the Octokit ecosystem for safe testing is to point the gemini at a dedicated personal test repository (not the organisation's production repos) and rely on the `--dry-run` path until the operator removes the guard.
...
A `verify` step runs after each agent output: the verifier re-fetches the PR/issue/release from the API and checks that any proposed labels or comments match the live state before presenting output as done (read-back verification per ADR-050). If the gemini is operating in write mode, the verifier confirms the mutation was applied and matches the intent before marking the task complete.
...
The scaffolded `.gemini/mcp-policy.json` is default-deny with the following explicit grants:
...
All write-access grants are blocked unless the operator sets `ALLOW_WRITES=true` in their shell environment and explicitly re-runs with that flag. The MCP policy is scannable via `gemini mcp-scan` (static-only, flags any shell/network grants not on the allowlist).
...
4. **Rate-limit awareness.** The scaffold wires `@octokit/plugin-throttling` to automatically back off at GitHub's secondary rate limit threshold. This prevents the gemini from burning the operator's API quota during a large triage sweep.
5. **Token scope check.** On `gemini doctor`, the scaffold calls `octokit.rest.users.getAuthenticated()` to confirm the token is valid and logs the token's `X-OAuth-Scopes` header (for classic PATs) or a sentinel-accessible-endpoint check (for fine-grained PATs) so the operator knows their permissions before the gemini runs.
...
- `@octokit/rest` v22 dropped Node 18 support; the scaffold requires Node >=20, consistent with the gemini-wide engine floor.
