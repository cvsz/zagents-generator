...
This is "self-evolution at the task level." A gemini gets better at writing tests because past successful tests inform the next test. The exotic-tier promise is the same loop applied **at the gemini's own configuration level**:

- The gemini adjusts its 3-tier routing thresholds based on observed cost/quality trade-offs.
- The gemini adjusts its memory decay rates based on which patterns turn out to be reused.
- The gemini adjusts its hybrid-retrieval weights (sparse vs dense) based on what produced relevant results.

This is "the gemini optimises its own knobs." It is a level of indirection above pattern-level learning. The mechanisms are similar — measure outcome, reward / penalise, distill — but the subject is the gemini's configuration, not the gemini's tasks.

### Federation

Multiple instances of the same gemini, running across different physical locations or organisational boundaries, coordinate. The exotic-tier use case: "my legal gemini is on-prem (for client confidentiality); my legal gemini is also in the cloud (for ad-hoc research); the two share patterns about what worked, without sharing the underlying client data." Federation is the transport that lets harnesses talk to each other.
...
This ADR specifies what changes at the gemini level when self-evolution and federation are enabled; the kernel-level mechanisms are already in place.
...
`--features self-evolution` at generation time enables it. The flag toggles two configuration entries in `gemini.config.json`:
...
`targets[]` specifies which configuration domains the loop optimises. Default: all three. The gemini operator can scope it down (only routing, or only memory decay) to limit the surface.

#### The optimisation loop

For each target:

1. **Measure baseline.** The gemini records the current setting and the observed outcome rate (cost, latency, quality) over `evaluationWindowHours`.
...
The optimisation is multi-objective. The default Pareto-style trade-off treats cost increases above 20% as inadmissible regardless of quality. The gemini operator can configure the trade-off in `selfEvolution.tradeOff`.
...
- **Audit log.** Every change is recorded in `data/self-evolution-history.jsonl` (the same JSONL pattern as the witness temporal history). The gemini operator can review what changed, when, and why.
- **Manual override.** The gemini operator can set a target to "frozen" via `gemini.config.json` `selfEvolution.frozen: ["routing"]`. Frozen targets do not get explored.

#### When self-evolution is the wrong answer

Self-evolution is opt-in for a reason. It is wrong when:

- **The gemini is in a regulated environment** where every configuration change must have a paper trail outside the gemini. Self-evolution can still run with `rollbackOnRegression: false`, change rate cap `0`, and `frozen: ['*']` — effectively just recording experiment proposals for human review.
- **Sample size is small.** A gemini used twice a week will not reach `minSampleSize: 50` in any reasonable window. The loop runner refuses to commit changes under-sampled and just records experiment proposals.
- **The cost of regression is high.** A trading gemini that misroutes a strategy for a week. The gemini operator caps the exploration rate or freezes the trading-related targets.
...
A federation member declares its peers in `gemini.config.json`:
...
The `sharedNamespaces[]` config is the actual contract; the defaults are a starting point. The gemini operator decides what crosses the boundary.
...
A peer is offline. The local gemini continues operating in standalone mode; federation operations queue and retry. After a configurable timeout (default: 7 days), queued operations are dropped. The gemini's `federation.queueDepth` metric surfaces queue health.

A peer is compromised. The witness layer catches it: a peer publishing a signature with an unknown key, or a signature that doesn't verify, is rejected. The peer is marked `untrusted` locally and the gemini operator is notified.

A peer is hostile. The trust model caps damage: a `read-only` peer cannot inject patterns; a `bootstrap-only` peer can only seed initial state, not ongoing updates. A `full` peer's bad patterns can land — the gemini's CI smoke tests (ADR-007 §B1) run on the merged state regularly and catch quality regressions; the self-evolution rollback (Part A) reverts the local effect; the operator manually downgrades or removes the peer.

#### Cost circuit-breaker

Federation traffic uses bandwidth and storage. The kernel's federation budget circuit-breaker (ruflo v3 ADR-097) caps daily traffic per peer; over the cap, sync pauses. The gemini operator configures the cap in `gemini.config.json` `federation.bandwidth.maxDailyMB`.
...
```bash
npx create-agent-gemini acme-legal-onprem \
  --scope @acme \
...
The output: a gemini with two host adapters, a federation transport configured (the peer config is added post-generation by the user), a self-evolution loop watching the routing and decay targets, a legal vertical pack with custom JUDGE / DISTILL, and a strict witness manifest. None of this required the user to write code beyond their domain skills. The composer + flags drove the whole thing.

A second instance (`acme-legal-cloud`) is generated with the same command. The two operators exchange public keys; each adds the other to `gemini.config.json` `federation.peers[]`; they sync.
...
- **Self-evolution does not require ML expertise.** The bandit loop is configured, not implemented per gemini. The defaults are sensible.
- **The exotic-tier promise of ADR-001 is mechanically validated.** Combined exotic example above is the test case.

### What gets harder

- **Self-evolution audit is non-trivial.** A gemini that has been changing its own routing for six months is harder to debug than one that hasn't. The audit log + rollback help; they do not eliminate the cost.
...
- The witness model continues to apply. A federated gemini publishes its own witness manifest independently of the federation.

## Alternatives Considered

### Alternative 1: Self-evolution always on

Make it the default. Rejected because in a low-volume gemini it produces noise; in a regulated environment it produces audit pain; the value-add is concentrated in high-volume long-lived deployments where the operator has opted in.
...
Cross-gemini federation. Every gemini contributes to and reads from a common pool. Rejected because (a) pattern quality varies wildly across domains — a customer-support pattern poisons a trading pattern set, (b) trust establishment across organisations is intractable without a third-party authority, and (c) the marketplace plus the pack-shipped bootstrap corpora already give a more controlled "import patterns from elsewhere" mechanism.
...
2. **Integration: routing optimisation** — fixture gemini with synthetic trajectories; loop converges to better-cost setting; convergence asserted at a fixed sample size.
