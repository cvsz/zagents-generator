...
Ship a **rule-based, pure, deterministic** Repo → Gemini importer, with the embedding layer as a *future, optional refinement of one scoring term* — never the generator.
...
- The cold-start problem is solved: a repo becomes a recommended, editable, branded gemini in one paste.
...
- **LLM-generates-the-gemini.** Rejected: non-deterministic, unauditable, and breaks provenance — the opposite of the project's thesis.
...
- **Routing**: a Rust crate routes to `rust-crate-gemini` + `vertical:coding`; an MCP server routes to `mcp-server-gemini` with `mcp: remote`; suggested commands are all `execution: disabled`.
