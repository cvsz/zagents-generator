...
1. **Engine** (ADR-070…083, prior): frozen model + evolving gemini over 7 mutation surfaces; sandbox; immutable scorer; archive-as-tree; safety gate.
...
9. **The SWE-bench arc (117–141)** — from real multi-file reasoning to a self-optimizing gemini:
...
| Multi-package cross-package resolve-rate | 132 | **4/4** across kernel-js, create-agent-gemini, vertical-base, darwin-mode, $0.017, one runner |
| Evolve the gemini vs real SWE fitness | 133 | `(1+λ)` loop over 3 external pkgs; elite improves gen0→gen1 (3/3 at lower cost); converges to cheapest-sufficient |
