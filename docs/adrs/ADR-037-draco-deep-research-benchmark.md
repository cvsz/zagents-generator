...
gemini actually is.** Every other capability has a gate (CodeQL for security,
...
ultimate research gemini" are unfalsifiable without a benchmark.
...
2. is **reproducible** — same gemini + same questions → comparable score, and
3. exploits **model fusion via OpenRouter** — the research gemini routes each
...
research gemini with proof."* The operative word is **proof** — a measured,
re-runnable number, not a narrative.

## Decision

Build **DRACO** as a first-class benchmark in `packages/bench/`, driven by an
OpenRouter-backed fusion research gemini, gated in CI like every other metric.
...
### 2. The fusion gemini (OpenRouter)

The research gemini routes each pipeline stage to a different model via the
...
gemini records which model handled each stage (provenance) in the trajectory.
...
4. **Faithfulness** — an independent LLM-judge (a model NOT used in the gemini)
...
- A new gemini subcommand `gemini draco` runs DRACO against a scaffolded
  research gemini and emits the proof JSON (ADR-031 bundle shape).
...
**Positive**
- "Ultimate research gemini" becomes a *number with a per-domain breakdown*,
...
- **M5** — `gemini draco` subcommand + `bench-baseline` DRACO regression gate +
