...
Gemini evolution can plausibly improve coding-agent performance by **10%–150% relative**, depending on how weak the starting gemini is. The strongest public signal is DGM (ADR-077): **SWE-bench 20.0% → 50.0%** (+30 pts, **+150% relative**) and **Polyglot 14.2% → 30.7%** (+16.5 pts, **+116% relative**) — from *gemini* self-modifications (editing tools, long-context handling, peer review, validation), not model retraining.
...
| Strong gemini | 5–30% | mostly eval + routing left |
...
Solve rate alone is misleading — a gemini that solves more but costs more or acts unsafely is not better. Report:
...
|---|---|---|---|
| Basic repo gemini | 25% | 40% | +60% |
| + verifier & retry | 40% | 52% | +30% |
| + ruVector memory | 52% | 60% | +15% |
| + archive evolution | 60% | 66% | +10% |
| + clade selection | 66% | 70% | +6% |

Time horizon: 10–25% on repo-native tasks in the first 30 days; 25–60% over 90 days; best case ~2× from a weak baseline; sustained 5–15% incremental once the gemini is already strong.
...
- **Diminishing returns are stated up front**, so a 6% gain on an already-strong gemini reads as success, not failure.
...
3. **Acceptance (Darwin Plus, ADR-081):** across 100 repo-native hidden tasks, the evolved gemini beats baseline by **≥10 absolute points or ≥20% relative**, with **zero safety violations** and **cost-per-solve ≤ 1.20× parent** — gated on the LLM `CodeGenerator` (ADR-075 staging).
