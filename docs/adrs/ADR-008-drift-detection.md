...
A generated gemini diverges from its origin over time. The kernel ships new versions. Templates get fixed. Catalogue entries are upgraded. The gemini author edits files locally for legitimate reasons. After six months, the gemini is a tree of files; we want to know which files came from which source, whether the source has updated, whether the local edits conflict with upstream changes, and whether the divergence is safe to ignore or actionable.
...
5. **API surface drift** — a plugin the gemini installed declared `kernelEngines: ^1.0.0`, the gemini is now on `1.4.0`; should still work, but warrants a check.
6. **Witness drift** — the gemini's witness manifest claims fix `F12` is present at SHA `abc...`, but the file now hashes to `def...`. Either an attack or an honest local edit.
...
This ADR specifies all six, the CLI surface that exposes them (`npx <gemini-name> drift check`), the CI workflow (ADR-007 §B5) that runs the check weekly, and the auto-PR recovery path.

## Decision

### The gemini manifest is the source of truth

The manifest at `.gemini/manifest.json` (defined in ADR-003 §The gemini manifest) is the canonical record of what was generated, from what versions, with which choices. Drift detection is fundamentally "what is the delta between the manifest's assertions and the current state of the world?"

If the manifest disappears or is invalidated, drift detection cannot run. The witness manifest (ADR-011) attests the gemini manifest's checksum so silent corruption is caught.
...
- **Recovery.** Auto-applied for clean updates (config knob); offered as a 3-way merge UI for conflicts. The 3-way merge inputs are: the original template version's file, the new template version's file, the gemini's current file. This is the copier upgrade model.
...
- **Recovery.** `npx <gemini-name> drift accept-local <path>` updates the manifest's recorded SHA. `npx <gemini-name> drift restore <path>` reverts to generated state.
...
npx <gemini-name> drift check

# JSON output (for CI consumption).
npx <gemini-name> drift check --format json

# Restrict to one drift kind.
npx <gemini-name> drift check --kind kernel
npx <gemini-name> drift check --kind template
# (etc.)

# Recovery actions.
npx <gemini-name> drift accept-local <path>       # User edit, manifest records new SHA
npx <gemini-name> drift restore <path>            # Revert to generated state
npx <gemini-name> drift apply-template <path>     # Apply the upstream template's new version
npx <gemini-name> drift merge-template <path>     # Open 3-way merge for conflicts
```
...
```jsonc
{
  ...
  "manifestPath": ".gemini/manifest.json",
  "kinds": {
...
ADR-007 §B5 specifies `gemini-drift-check.yml`, which runs weekly. Concretely:
...
jobs:
  check:
    ...
    steps:
      ...
      - run: npm ci
      - id: drift
        run: npx <gemini-name> drift check --format json > drift.json
      ...
      - if: steps.drift.outputs.exit_code == '1'
        run: |
          npx <gemini-name> drift apply --safe-only
...
The gemini's CI (`gemini-ci.yml` from ADR-007 §B1) can optionally fail on drift. Configurable via `gemini.config.json` `ci.failOnDrift`:
...
A gemini inevitably contains files the generator never wrote — the user's own modules, their own tests, their own docs. These are absent from the manifest. The drift checker leaves them alone. A user who wants to add files under generator-control runs `npx <gemini-name> drift adopt <path>` which records the file in the manifest as `{ "fromUser": true }`. Adopted files participate in drift detection only for the local-edit kind (the manifest's recorded SHA becomes the canonical "user's intended version"), not for template / catalogue / kernel drift.

### Special case: the eject mode

A gemini that has been ejected (ADR-012 §Eject) has vendored the kernel. The kernel drift kind is replaced with "vendored-kernel drift" — a comparison against the vendoring tag rather than the npm registry. The template and catalogue drift kinds behave the same. The user is on their own for security updates; the drift command warns about this prominently when run on an ejected gemini.

## Consequences

### What gets easier

- **The "is my gemini up to date?" question has an answer.** One command. Multi-kind.
- **Weekly auto-PRs.** A gemini whose maintainer ignores it for a month gets a PR every week with safe updates applied. The gemini does not silently rot.
...
- **CI minutes for weekly drift checks across thousands of generated harnesses.** Each one is a tiny job (≤ 30 s) but it is a real volume. If the generator becomes popular, this is a real bill — the gemini pays, not the generator.
...
`npx create-agent-gemini --regenerate` would overwrite generated files with the latest template content. Rejected because it destroys local edits, and the cases where local edits are intentional are exactly the cases where the user is most engaged with the gemini. The 3-way merge approach lets local edits survive an upgrade.

### Alternative 4: One-way drift detection (read-only check, no fix)

Detect drift but do not provide recovery commands. Rejected because the response to "your gemini is drifting" cannot reasonably be "go fix it yourself, here are 47 files." The recovery commands are the load-bearing payoff.
...
1. **A fixture** that exercises the drift (e.g. a gemini manifest claiming `kernel@1.0.0` against a registry serving `1.4.0`).
...
9. **`scripts/check-gemini-compatibility.mjs`** — given a fixture of mock gemini manifests from "downstream consumers," asserts the compatibility matrix correctly identifies breakage.
