...
Ablation gemini + result committed (`bench/experiments/crossover-ablation.mjs`, `bench/results/crossover-ablation.json`). Zero LLM. Correction noted on ADR-114. 350 tests unaffected. Caveat: 2 seeds (the firm claim is the refutation — crossover-OFF clearly crosses).
