...
Together, ADR-133 + ADR-134 show the evolve loop responding correctly to the **fitness landscape**: it optimizes cost when capability is saturated, and selects the more capable gemini when the corpus discriminates. This is the "differentiator is robustness" thesis (ADR-098) demonstrated by an actual evolutionary loop on real code — search/replace is selected because it *resolves more per attempt*, not by preset.
