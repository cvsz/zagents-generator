...
Drives SDK calls: calls `ResourceManagementClient.resourceGroups.list()`, `BlobServiceClient.listContainers()`, and `AzureOpenAI.chat.completions.create()`. Holds the MCP tool grants from `.gemini/mcp-policy.json`. Applies the mutation gate: if `--allow-mutations` is absent, any ARM write or blob upload is replaced with a logged dry-run record. Uses frontier-tier model because it must reason about partial failures, pagination, and policy decisions.

**`agents/verifier.md` (Tier 2 — Haiku)**
Re-reads the output of executor against the live platform: lists the same resource groups a second time and cross-checks against the executor's report; for blobs, re-lists containers and confirms the reported count. Flags discrepancies before the gemini surfaces the result as done. This is the verification gate required by ADR-050.
...
`.gemini/mcp-policy.json` (default-deny, per ADR-022):

```json
{
  "version": 1,
  "default": "deny",
  "audit_log": ".gemini/mcp-audit.jsonl",
  "grants": [
...
All write tools require the `allow_mutations` flag at the gemini level. All tool calls are appended to `.gemini/mcp-audit.jsonl` with timestamp, tool name, caller agent, and flag state.
...
- Demonstrates `DefaultAzureCredential`'s environment-to-managed-identity graduation path: the same gemini code runs on a developer laptop (via `az login`) and in Azure (via managed identity) without modification.
...
- The mutation gate + Azurite defaults mean the gemini can be run in CI against a real Azure subscription (read-only) or fully locally (Azurite) without cost or risk.
...
- All MCP tool calls are audited to `.gemini/mcp-audit.jsonl` — satisfying enterprise compliance requirements for agent action logging.
...
- This example is not certified for HIPAA, PCI-DSS, FedRAMP, or any regulated workload. Azure compliance certifications apply to the Azure platform itself; the gemini is a development scaffold, not a production-hardened application.
