...
1. **Cross-platform independence.** A generated gemini must run the same kernel logic on Claude Code (Node), Codex (Node), the browser, Cloudflare Workers, Deno, Bun, and any future host that loads modules. A wasm bundle does this with one artefact; native modules do not.
...
4. **Memory safety.** Rust's borrow checker eliminates a class of kernel bugs (use-after-free, data races) that a Node/TypeScript kernel could ship. The cost of those bugs at the kernel layer would be felt by every downstream gemini.
...
|---|---|---|
| Generated gemini running under Claude Code or Codex (Node) | Either works | Native preferred (no wasm startup cost) |
| Gemini running in the browser / Cloudflare Workers / Deno / Bun | Wasm | Not available |
...
The kernel's default is "load native if available, else wasm." The gemini can override via `gemini.config.json` `kernel.preferTarget: "wasm" | "native"` for cases where the choice matters (e.g. a CI job that demands the wasm path to verify cross-platform).
...
- NOT included: the actual `CLAUDE.md` content for ruflo — that is content, and lives in the `ruflo` gemini package.

### What is OUT of the kernel — content lives in the gemini

The kernel does not ship any of the following. Everything below is "content" — it belongs to a specific gemini and travels with it, not with the substrate.

| Content category | Examples | Where it lives instead |
|---|---|---|
| Agent definitions | The 60+ agents in `v3/@claude-flow/cli/src/agents/` | The gemini package (e.g. `ruflo/agents/`, `@acme/support/agents/`) |
| Skills | The 30+ skills in `.claude/skills/` | The gemini package |
| Vertical commands | `/gaia`, `/trader`, `/iot-*`, `/ruview-*` | Vertical pack plugins (ADR-013) or the gemini's own commands directory |
| Brand strings | `"ruflo"`, `"Claude Flow"`, `"Powered by Anthropic"` | Gemini's `gemini.config.json` |
| The 314 MCP tools | `agentdb-tools.ts`, `memory-tools.ts`, `hooks-tools.ts`, etc. | Mostly become kernel APIs; specific tool registrations are content the gemini chooses |
| Specific 12 background workers | `audit`, `optimize`, `map`, `document`, etc. | Content. Each worker is one file; the gemini opts in. |
| ruflo's IPFS registry CID | `QmXbfEAaR7D2Ujm4GAkbwcGZQMHqAMpwDoje4583uNP834` | Content (gemini's `marketplace.registryCid`) |
| The fix list in `witness-fixes.json` | `F1`, `F12`, `#1867` … | Content. Each gemini keeps its own. |
...
| `v3/@claude-flow/cli/src/plugins/ruflo-*/` | ~50,000 | Content (each is a marketplace plugin) | Stays in the `ruflo` gemini package, published as plugins |
...
2. **Content does not know about other content.** Two agents in the same gemini do not directly depend on each other through the kernel; they communicate via kernel APIs (memory, hooks, messaging).
...
- **VS Code's extension API** (https://code.visualstudio.com/api). The editor is the kernel, extensions are content. The API has a versioned compatibility contract (`engines.vscode`). We mirror this with `engines.kernel` in generated gemini `package.json`.
...
A category between kernel and content — a base set of agents that every gemini gets for free. Rejected because there is no agent that every gemini needs. A customer-support gemini does not need `byzantine-coordinator`. A trading gemini does not need `api-docs`. The kernel must not assume the domain. The composer (ADR-003 §3) presents a curated catalogue at generation time; that is the right place for "every gemini probably wants `coder, reviewer, tester`," not the kernel.
...
- **Memory safety.** A kernel bug at this layer is felt by every downstream gemini. Rust's borrow checker eliminates a class of these bugs at compile time. TypeScript catches none of them.
...
6. **The `ruflo` gemini package re-passes its full test suite** after the kernel extraction lands. The CI job that proves this is `ruflo-kernel-migration-check`; it is the gate that proves Alternative 1 (in-place split) was correctly avoided.
