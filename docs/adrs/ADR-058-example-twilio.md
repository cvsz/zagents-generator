...
An agent gemini wired to Twilio can realistically drive three headline workflows:
...
### MCP policy — granted tools (`.gemini/mcp-policy.json`)
...
All other MCP tools (file write, shell exec, network fetch to non-Twilio hosts, memory mutation) are denied by default. The audit log writes one JSON line per tool call to `.gemini/audit.jsonl`.
