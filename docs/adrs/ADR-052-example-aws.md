...
The scaffolded `.gemini/mcp-policy.json` grants **only**:
...
Every tool call is appended to `.gemini/mcp-audit.jsonl` with timestamp, agent name, tool name, and a SHA-256 of the parameters. Mutating tools (`aws_s3_put_object`, `aws_ec2_run_instances`, `aws_lambda_invoke`, `aws_dynamodb_put_item`) are absent from the grants list by default and must be added by the operator after reviewing the policy file.
...
5. EC2/ECS/Lambda instance metadata (IMDS) — used when the gemini itself runs inside AWS.
...
- Turns the most-asked "can the gemini drive real AWS infra?" question into a one-command proof.
