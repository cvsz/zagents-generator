...
> frozen foundation model + self-modifying agent gemini + archive-based evolution + sandboxed empirical evaluation + statistical promotion gates.

In our framing: **the model stays fixed, the gemini evolves, the proof is in replay.**

The leaderboard of *ideas* (not vibes):

| Rank | Approach | Contribution | Build now? |
|---|---|---|---|
| 1 | Darwin Gödel Machine (ADR-077) | open-ended empirical gemini evolution + archive | Yes |
...
| 5 | Model-retraining loop | change weights, not gemini | No (impractical for most) |

SOTA vs hype:

| Claim | Status | Build now? |
|---|---|---|
| Agent improves its own gemini | real, practical | ✅ |
...
```
repo → baseline gemini → mutation generator → child gemini → sandbox evaluation
