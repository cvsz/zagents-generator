...
DGM (ADR-077) is credible precisely because each self-modification is a *response to an empirical result*: the agent reads how it failed and edits the gemini to fix that. Without the failure signal, Darwin Mode's LLM path was missing that feedback edge — it was random search wearing an LLM costume.
