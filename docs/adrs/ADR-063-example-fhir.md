...
Re-reads each cited resource by ID via `client.read({ resourceType, id })` and confirms the response matches what the executor reported (resourceType, id, status, key codes). Flags any discrepancy as an unresolved inconsistency. The gemini does not present output as done until this gate passes. Implements the ADR-050 verification-gated output pattern.
...
`.gemini/mcp-policy.json` ships with default-deny and grants exactly the tools this platform needs:
...
Network is granted (`allowNetwork: true`) because all FHIR operations are HTTP calls. Shell and file-write remain denied. All tool calls are appended to `.gemini/mcp-audit.jsonl`.
