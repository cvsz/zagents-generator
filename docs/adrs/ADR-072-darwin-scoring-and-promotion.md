...
`promotion_delta` is the anti-noise margin: a child must beat its parent by a real margin, not measurement jitter. The non-regression clause stops a variant trading test pass-rate for a cheaper, faster, but worse gemini.
