...
The first five host adapters (Claude Code, Codex, pi.dev, Hermes, OpenClaw) target **OS-level isolation**. The Node process running the generated gemini has whatever permissions the OS grants it. For local-dev and personal use, that's fine — the user runs `npx my-bot init` on their own laptop and trusts their own code.
...
The mapping is deterministic and lossless: if your gemini can express a constraint through kernel claims, RVM can enforce it.
...
- One gemini source → six deployment targets. No fork required.
...
- Witness-native syscalls compose with the ADR-011 Ed25519 witness manifest — the gemini's release-time signature + RVM's runtime hash-chain together give end-to-end provenance.
...
- **Replace `admit_message()` with RVM internally** — would force every gemini install to ship RVM, which is the wrong default. Kept the in-kernel implementation as the OS-level baseline; RVM is opt-in via `--host rvm`.
