...
- **LLM** — the real gemini loop (contextBuilder selects) asks the model for a **unified diff**; we apply it and report honestly.
...
For this model, **raw LLM unified diffs are an unreliable patch representation**; the whole-file output (ADR-123) is reliable. The recommended step-3 primitive is therefore: **model emits the whole corrected file → the gemini computes the diff itself via `git diff` → `git apply`**. This yields a real, guaranteed-appliable unified-diff artifact (the SWE-bench output format) while sidestepping LLM diff-corruption. A pure-diff path remains possible but needs a diff-validate-and-repair retry loop (and likely a larger token budget) — deferred to step 3.
