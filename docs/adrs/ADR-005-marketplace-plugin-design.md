...
3. **As a path for publishing.** A generated gemini can publish its own scoped plugins back into the same registry under its own scope (`@acme/`).
...
The schema below is the post-extraction version of ruflo's existing `PluginEntry` from `v3/@claude-flow/cli/src/plugins/store/types.ts`, generalised so it is gemini-agnostic.
...
- Exports: one command `/create-agent-gemini` (the slash-command thin wrapper) and one skill `gemini-generator-walkthrough`.

When a user has the plugin installed in their ruflo project, `/create-agent-gemini <name>` runs the same composer flow as the standalone CLI. The two share the same source under `packages/create-agent-gemini/src/`; the plugin is a thin bin-wrapper that calls into it.

### 5. Generated harnesses as marketplace publishers

Every gemini the generator produces can publish its own plugins to the marketplace. The generator scaffolds the publish-side flow:

- `.github/workflows/publish-plugin.yml` — the workflow that builds, signs, and publishes a plugin.
- `scripts/publish-plugin.mjs` — the script the workflow invokes. Reads `PINATA_API_JWT`, builds the plugin tarball, computes the checksum, signs the registry entry, uploads the updated registry to Pinata, and updates the gemini's pinned registry CID.
...
An independence-mode gemini (ADR-015) maintains its own registry; the publish flow updates the gemini's own CID. A powered-by-mode gemini (the default) publishes to ruflo's registry under the gemini's scope, with the gemini's author signing.
...
1. Read `gemini.config.json` `marketplace.registryCid`.
...
The gemini boots in degraded mode (no marketplace plugins) if the registry cannot be fetched. This is critical for air-gapped or offline use; the kernel does not depend on the registry being reachable.

### 8. Registry caching

The fetched registry is cached at `.gemini/cache/registry.json` with a 24-hour TTL. The CID is content-addressed, so a registry that has not changed re-uses the cache. A new CID (the gemini operator has updated their registry pointer) busts the cache.

### 9. Publishing a plugin: the user flow

```bash
# Inside a generated gemini
npx <gemini-name> plugin scaffold @acme/plugin-foo
...
npx <gemini-name> plugin publish @acme/plugin-foo
...
# updates gemini.config.json `marketplace.registryCid`, commits the change.
```

A CI version of the same flow runs from a GitHub Actions workflow scaffolded into the gemini.
...
5. **Offline / air-gapped boot** — boot a gemini with the registry unreachable. The gemini starts in degraded mode; no plugins load; the kernel logs the degradation.

### Publishing tests

6. **`plugin scaffold` produces a valid package** — `npx <gemini> plugin scaffold @test/plugin-foo` produces a tree that passes the gemini's own smoke test.
...
9. **Plugin-picker** — composer shows installed plugins from the registry. Trust-level filter works. Host-compatibility filter works (plugin without the gemini's selected host in `hostsSupported` is greyed out).
