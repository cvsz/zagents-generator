...
Advertising is one of the largest programmatic domains an agent gemini
...
a gemini that makes live mutations — budget changes, creative uploads, audience
...
- Demonstrates how an agent gemini handles *two* independent third-party
...
`.gemini/mcp-policy.json` grants exactly:
...
```json
{
  ...
  "audit_log": ".gemini/audit.jsonl"
}
```

Every tool call is written to `.gemini/audit.jsonl` regardless of grant
...
6. **Audit log**: every API call (read or mutation) is written to
   `.gemini/audit.jsonl` with timestamp, tool, arguments hash, and response
   status.
