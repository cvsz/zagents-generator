...
This is the same posture as ADR-022 (MCP is default-deny, one selectable primitive) applied to self-modification: the gemini's *capacity to change itself* is itself a default-deny surface.
...
5. **Signature stability** — a mutated surface still exports the same function signatures the gemini runtime imports.
