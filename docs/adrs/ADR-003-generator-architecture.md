...
The user runs `npx create-agent-gemini <name>`. Some seconds later, they have a directory containing a brand-new, npm-publishable agent gemini with their chosen name, scope, identity, hosts, agents, skills, and plugins wired up.
...
> **The generator is TypeScript; the kernel is Rust (wasm + NAPI-RS).** Per ADR-002 and ADR-002a, `@zagents/kernel` is published from a Rust workspace as a wasm bundle plus per-platform native peers. The generator (`create-agent-gemini`) is a conventional TypeScript npm package; it **embeds references to** `@zagents/kernel` in the templates it writes, and it **depends on** the kernel itself (for `init` / scaffolding helpers it shares with generated harnesses). It does NOT re-implement kernel primitives in TypeScript. Generated harnesses likewise depend on `@zagents/kernel`; the kernel they consume is the same Rust-source wasm/native build. The generator is the only piece of this project written purely in TypeScript.
...
```
packages/
  kernel/                    # @zagents/kernel (see ADR-002)
  create-agent-gemini/      # the CLI users run
    src/
  ...
  test-gemini/              # the canary gemini from ADR-002 §Test Contract
...
```bash
# Trivial (zero-question) — uses defaults, generates a Claude Code gemini.
npx create-agent-gemini <name>

# With scope
npx create-agent-gemini <name> --scope @acme
...
# From an existing project — eject ruflo into a generated gemini
npx create-agent-gemini <name> --from-existing ./my-ruflo-project

# Template choice (default: "standard"; others: "minimal", "vertical-trading", …)
npx create-agent-gemini <name> --template minimal

# Use a non-default kernel version
npx create-agent-gemini <name> --kernel ^1.2.0

# Use a custom template registry CID (independence mode)
npx create-agent-gemini <name> --registry-cid <ipfs-cid>
```

### Two run modes: offline and online

**Offline mode (default)**: every template the CLI needs is bundled into the published `create-agent-gemini` package. No network call required to scaffold. The bundled templates are pinned to a kernel version at publish time. Time-to-first-gemini is bounded by `npm install`.
...
Templates live under `packages/create-agent-gemini/templates/`. The structure:

```
_base/                       Always applied first. Universal gemini scaffold.
...
_features/                   Opt-in feature overlays. Applied on top of host overlays.
  ...
  self-evolution/            Adds the learning loop applied at gemini level
...
- **JSON / YAML / TOML** — parse with the appropriate format-aware parser, walk the tree, substitute placeholders only at known keys (`name`, `description`, `author`, `bin`, `engines.kernel`, MCP server keys). The schema for known keys is declared in `packages/create-agent-gemini/src/renamer/schemas/`.
...
- **Binary** — copied verbatim. If a binary asset needs renaming (e.g. a logo with the brand burned in), the gemini author must replace it post-generation; the generator does not paint logos.
...
### What the generated gemini looks like

The output of `npx create-agent-gemini acme-support --scope @acme --hosts claude-code` is approximately:
...
acme-support/
  ...
  src/
    index.ts                Imports from @zagents/kernel; wires the host adapter
    config.ts               Gemini config (hosts, features, kernel version, ...)
    agents/                 Selected agents (copied from catalogue)
    skills/                 Selected skills (copied)
    commands/               Selected commands
  .claude/
    settings.json           Claude Code hooks wiring (points at this gemini's hooks)
  ...
  .gemini/
    manifest.json           Records what was generated (see "The gemini manifest" below)
...
If the user picked Codex too, the same tree has `AGENTS.md`, `.codex/config.toml`, and the Codex host adapter wired in. If they picked `--features federation`, `src/index.ts` imports `@zagents/kernel/hosts/federation` and the federation transport is configured in `src/config.ts`.

### The gemini manifest

A new file the generator writes: `.gemini/manifest.json`. This is the single canonical record of "what we generated, against what versions, with what choices." It is the source-of-truth for drift detection (ADR-008) and the upgrade story (ADR-012).
...
1. **"What did I get?"** — every file in the gemini is enumerated with its template-of-origin or catalogue-of-origin, and a SHA-256 of the generated content.
2. **"What kernel version was I built against?"** — `kernel.version` is canonical.
3. **"What choices did I make?"** — `choice` reproduces the composer state. Re-running the generator with the same `choice` produces a bit-identical gemini (up to timestamp fields).
...
The composer offers a curated catalogue. The catalogue is content, not kernel — it lives in its own package, `@zagents/catalogue`, which the generator depends on. The package contains:
...
The catalogue is versioned independently of the kernel and the generator. A new catalogue version (more agents, updated skills) does not require regenerating harnesses; harnesses pull catalogue entries at install time via the manifest's `agents` and `skills` lists. ADR-008 detects when a catalogue entry the gemini depends on has been updated, and surfaces the diff.
...
4. Invoke `npx <gemini-name> --help` — exits 0.
5. Print the gemini's `.gemini/manifest.json` digest.
...
This is the canary that catches a kernel-template-version mismatch before the user is left holding a broken scaffold. It also serves as the input contract for ADR-009 (anti-slop): a gemini that does not pass its own smoke test cannot be published.

### Performance budget

- **Time from `npx create-agent-gemini foo` to a generated, smoke-passing gemini on a warm npm cache**: ≤ 90 seconds.
- **Time from `npx ...` to the composer first paint**: ≤ 5 seconds (this is mostly `npm` resolving the package).
- **Disk size of a default gemini**: ≤ 10 MB before `npm install`, ≤ 200 MB after.
...
- **Catalogue maintenance is real work.** Curating `@zagents/catalogue` is its own project. It is owned by the ruflo maintainers initially; ADR-013 covers how vertical-pack owners contribute.
...
- The kernel does not know about the generator. The generator is content tooling that consumes the kernel like any other gemini would.
...
Yeoman (https://yeoman.io/) is the canonical Node generator framework. Its sub-generator composition model is genuinely good. Rejected because (a) Yeoman has not had a major release in years and the ecosystem has moved on (create-vite, create-next-app, create-cra deliberately do not use it), (b) Yeoman's prompt system is more elaborate than we need and brings in a transitive-dependency tail we want to avoid on the publish path, and (c) we want zero generator-framework dependencies — every supply-chain risk on `npx create-agent-gemini` is a risk on every user's first install.

### Alternative 2: Use Cookiecutter / Copier (Python tooling under the hood)

`cookiecutter` and `copier` are both excellent. Rejected because shipping a Node CLI that depends on a Python tool means every user needs a working Python install, and Windows users have an even worse time. We re-implement copier's manifest-and-update model in Node (see "The gemini manifest" above) but do not shell out to copier.
...
5. **Trivial-gemini end-to-end** — `npx create-agent-gemini test-trivial --hosts claude-code --no-interactive` produces a directory that passes `npm install && npm run build && npm test`.
6. **Exotic-gemini end-to-end** — the ADR-001 §Test Contract "exotic" canary: federation + multi-host + custom DISTILL — built via the same CLI, passes the same gates.
7. **Re-runnable determinism** — same `HarnessChoice` produces bit-identical output (up to the `generatedAt` timestamp).
8. **`--from-existing` migration** — given a vendored fixture of a ruflo project, the eject mode produces a gemini that passes its smoke test. The fixture is committed alongside the test.

### Performance tests

9. **Time-to-first-gemini budget** — CI measures the trivial-gemini end-to-end run time. Hard ceiling: 120 seconds on the CI runners (warm cache).
...
- `v3/@claude-flow/cli/src/plugins/store/discovery.ts` — the IPFS registry consumer the online mode delegates to via `@zagents/kernel/marketplace`.
...
- `copier` (https://github.com/copier-org/copier) — the `_copier-answers.yml` manifest, the source of our `.gemini/manifest.json` idea. The most directly influential prior-art.
