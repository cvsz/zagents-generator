...
Nothing proves a generated gemini's *content* (system prompt, agent roster,
...
adapter layer, so a gemini can declare agents/prompt/permissions and have the
host config never reflect them.

## Decision

1. **Add a live-LLM verification tier** — `scripts/verify-gemini-live.mjs` —
   that extracts each gemini's capability surface from its emitted config and
...
- **⁸ opencode permissions wired to the wrong field.** `opencodeJson()` reads
  ...
  gemini's allow/deny posture is silently never emitted — it always writes
...
- **¹⁰ rvm capability table is always empty.** `generateConfig` calls
  ...
  capability-token security model emits nothing regardless of the gemini's
...
the emission, and (c) `verify-gemini-live.mjs` confirms a real model sees the
