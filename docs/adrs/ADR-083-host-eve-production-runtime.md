...
| `skills/` (Markdown domain knowledge) | gemini skills (ADR-003) |
...
| `eve/<name>/instructions.md` | system prompt / persona | the gemini identity |
| `eve/<name>/tools/*.ts` | gated MCP tools (ADR-022) | `defineTool()` + Zod; **`needsApproval: true` for any tool the policy marks high-risk** |
| `eve/<name>/skills/*.md` | gemini skills | verbatim Markdown |
...
- Schema drift: Eve is new (2026); `defineAgent`/`defineTool` signatures may move. Mitigated by pinning a snapshot + a `gemini diag --eve-version-check` flag (same pattern as ADR-036).
...
| 8 | CI smoke | `--host eve` scaffolds and `gemini doctor` passes; (where feasible) `eve dev` boots the emitted agent |
