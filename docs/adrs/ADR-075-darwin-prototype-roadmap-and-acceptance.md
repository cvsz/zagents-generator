...
### Prototype 3 — self-improving full gemini (only after 1 and 2)
...
**The demo passes when at least one repo shows a reproducible improvement, zero unsafe actions, and a clear winning-gemini lineage.**
...
- **Per-repo baseline tuning.** "≥10%" is relative to each repo's baseline; the promotion delta must be calibrated per repo, which is extra setup the acceptance gemini must script.
...
1. **Build the full gemini first.** Rejected — un-attributable failures and an unsafe first demo. Staging is the whole point.
