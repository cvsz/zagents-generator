...
Gemini + result committed (`bench/experiments/real-substrate-deception.mjs`, `bench/results/real-substrate-deception.json`). Zero LLM calls (deterministic agent loop). 350 tests unaffected. A strong-deception variant (window>50 ∧ retry>4) was crossed by neither strategy (consistent with ADR-105's strong-deception ceiling).
