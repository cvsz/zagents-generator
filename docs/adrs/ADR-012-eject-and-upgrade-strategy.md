...
A generated gemini has a long life. The kernel does not stand still. Six months after generation, the gemini operator wants the security fixes, the performance improvements, the new MCP tools — without rewriting their gemini. They want a route from "outdated" to "current" that is mechanical, reviewable, and reversible.
...
This ADR pins down both: the upgrade flow that keeps a gemini on the upstream kernel, and the eject flow for organisations that need independence — and what each gives up.
...
The peer + dependency dual-declaration mirrors VS Code extensions (which declare `engines.vscode` and resolve VS Code as a peer). The peer dependency advertises the gemini's compatibility range to consumers (e.g. a plugin loading inside the gemini). The regular dependency makes the gemini self-installable for users who run it standalone.

Upgrade is the ADR-008 drift-detection command:

```bash
npx <gemini-name> drift check               # any kernel drift?
npx <gemini-name> drift apply kernel        # apply the kernel update
npx <gemini-name> drift apply-template      # apply any consequent template updates
```
...
3. `drift apply kernel` bumps the version, runs `npm install`, runs the gemini's own test suite.
...
5. The user commits the result. The next release of the gemini rolls forward to the new kernel.
...
- **Major** (`1.x → 2.0.0`) is breaking. The peer range `^1.2.0` does not match. The gemini must explicitly accept the upgrade.
...
3. Configuration in `gemini.config.json` is migrated to the new schema (if any).
4. A `MIGRATION-NOTES-1-to-2.md` is written at the gemini root describing what changed.
...
The eject is one-way in the sense that there is no `unesect` command. The gemini can manually re-adopt the kernel by undoing the vendoring (essentially regenerating from the current template), but the route is "regenerate," not "unsetect." We do not pretend otherwise.
...
- It does not change the gemini's marketplace participation. The gemini is still a marketplace publisher / consumer.
...
An ejected gemini is on its own for security and feature updates. The kernel team has zero obligation. We do, however, document a recipe for a manual upgrade:

1. Generate a new gemini from the current generator (`npx create-agent-gemini fresh-copy --hosts ...`).
2. Diff the fresh-copy's `vendor/kernel/` against the ejected gemini's `vendor/kernel/`.
...
The gemini's CI (ADR-007 §B) still runs on an ejected gemini. The differences:
...
The composer (ADR-003) does **not** offer eject at generation time. Eject is a downstream choice. A user who knows they want independence from day one can run `create-agent-gemini foo` then `npx foo eject` as their first commit, but the default path is peer-dep.
...
Plugins declare `kernelEngines` (ADR-005 §2). When the kernel upgrades, the gemini checks every installed plugin's range against the new version. Three outcomes:
...
Force every gemini to stay on upstream. Rejected because the regulated-environment case is real, and forcing organisations to fork the entire generator to escape upstream is worse than offering eject.

### Alternative 2: Vendor by default (no peer-dep mode)

The opposite extreme: every generated gemini vendors the kernel. Rejected because it makes the upgrade path worse for the 95% case and explicitly throws away the marketplace's "you can install plugins compatible with kernel X" promise.

### Alternative 3: Live patching via hot-loaded JS

Allow the kernel to be updated at runtime via a network fetch. Rejected because (a) it makes provenance verification incoherent ("which kernel did this signature attest to?") and (b) it introduces a new attack surface (a hostile registry can push a kernel update that compromises every gemini).

### Alternative 4: Skip codemods; let users migrate manually

Ship breaking changes without migration scripts. Rejected because the cost of every breaking change falls on every downstream gemini simultaneously; codemods compress that work into the kernel team writing it once. CRA's lack of codemods is one of several reasons users left.

### Alternative 5: A `unesect` command

Let an ejected gemini return to peer-dep mode. Rejected for v1.0 because the implementation cost is large (we have to detect every customisation made while ejected, decide which to keep, which to revert) and the user pull is unclear. A user who ejected is unlikely to want to un-eject; if they do, regenerating from a fresh template is the right path.
...
1. **Patch upgrade** — fixture gemini at `1.4.0`; kernel ships `1.4.1`; `drift apply kernel` bumps, installs, tests pass.
2. **Minor upgrade with template change** — fixture gemini at `1.4.0`; kernel ships `1.5.0` with a new wiring template; `drift apply` bumps; `drift apply-template` 3-way-merges; tests pass.
3. **Major upgrade with codemod** — fixture gemini at `1.5.0`; codemod `1-to-2` exists; user runs `upgrade kernel --major --to 2.0.0`; codemod rewrites imports, migrates config; tests pass.

### Eject flow tests

4. **Eject end-to-end** — fixture gemini; run `eject`; verify imports rewritten, kernel vendored, `package.json` updated, `.gemini/ejected.json` present, smoke contract still passes.
5. **Eject + ejecting includes adapters** — `eject --include-adapters` also vendors adapters; same smoke contract passes.
6. **Ejected drift check** — fixture ejected gemini; drift detection runs in ejected mode (per ADR-008 §Special case).
