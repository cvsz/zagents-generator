...
| Candidate generation | **beam search** over gemini templates | template catalog + walker | — |
...
| Execution primitive | **ReAct** (thought → tool → observe → update) | generated gemini runtime | — |
...
| Scoring | deterministic evals + the scorecard | DRACO scorer + `score-gemini` | — |
...
- The differentiator becomes the **scorecard** (gemini fit, compile confidence,
...
Scorecard — <repo>  (best-fit archetype: …)
  Gemini fit:        N/100   (recommended-archetype match confidence)
