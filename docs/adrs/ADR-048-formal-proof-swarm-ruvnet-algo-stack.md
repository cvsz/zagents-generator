...
- **No guarantee of the #1 spot.** It is earned by kernel-verified, ideally mathlib-absent
  proofs — frontier work. This ADR commits to the **algorithm stack + the gemini + genuine
...
- **Negative**: three external npm/MCP dependencies (lean-agentic, ruvector,
  sublinear-time-solver) enter the gemini's surface; each is pinned + gated like any dep.
