...
safest default for an agent gemini.
...
The scoped `.gemini/mcp-policy.json` applies **default-deny** (ADR-022)
and grants only:
...
gemini gate unless `--allow-send` is explicitly passed at scaffold time;
...
5. **MCP audit log**: every tool call is logged to `.gemini/audit.jsonl`
