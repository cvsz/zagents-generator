...
- **The ruflo team.** Open-source license already permits forks; reputation does not. If a generated gemini uses "powered by ruflo" while shipping low-quality output, the marketplace's perception of ruflo suffers.
- **The gemini author.** Wants their own brand. Does not want their customer to think they are using ruflo specifically; wants a clean visual identity.
...
The composer (ADR-003 §Identity stage) asks the user to pick one of two modes at generation time. The choice is recorded in `gemini.config.json` `branding.mode` and enforced by both the composer and the publish workflow.
...
- Ships under the gemini's chosen scope (`@acme/foo`); the package's `package.json` `keywords` includes `"ruflo-gemini"` and `"ruflo-marketplace"`.
- Uses ruflo's default IPFS marketplace registry (`marketplace.registryCid` defaults to ruflo's current CID).
- Inherits ruflo's plugin-trust signal set; trust tier derivation (ADR-009 §3) places the gemini in the ruflo trust namespace.

In return:

- The kernel is a peer dep, upgradeable via `drift apply kernel` (ADR-012 §Default).
- The gemini is eligible for inclusion in the bundled catalogue's "harnesses we know about" list (curated; not automatic).
...
- Ships under the gemini's chosen scope, and `package.json` `keywords` does **not** include `"ruflo-gemini"` (this is enforced at publish time; see §Publish-time enforcement below).
- May point at its own IPFS registry (`marketplace.registryCid` is set to the gemini operator's own CID) or remain on ruflo's. The composer prompts.
- Operates its own plugin-trust signal namespace.

In return:

- The gemini is a peer-dep consumer of the kernel by default (so kernel upgrades still work), with `eject` available if the operator wants to vendor.
- The gemini's marketplace listing carries no `"ruflo-marketplace"` tag and does not appear in any "powered by ruflo" curation surfaces.
...
A gemini that uses ruflo's name informally ("Acme — built on the ruflo framework") in a context where the user has chosen independence mode is a policy violation. The publish-time gate (below) blocks it. A gemini that fails to acknowledge the kernel dependency at all while in powered-by mode is also a violation (the powered-by tag is required, not optional).
...
- `@zagents/*` — the kernel, generator, host adapters, catalogue, bundled vertical packs, marketplace tooling.
...
The gemini author picks any non-reserved npm scope:
...
A vertical pack (ADR-013) follows the same rule. The bundled packs live under `@zagents/vertical-*`; curator-published packs live under the curator's scope (`@acme/vertical-mortgage`).
...
- `"ruflo-official"` — set automatically on bundled packages (`@zagents/*`); refused on third-party publishes.

The tags are renderable in the marketplace UI and feed the search-result curation. A "ruflo-marketplace" tag is informational ("this gemini participates in the ruflo ecosystem"); a "ruflo-official" tag is editorial ("this is shipped by the ruflo team"). The distinction is enforced.

Other tags are free-form (the gemini author picks `"customer-support"`, `"legal"`, `"trading"`, etc. as appropriate). The reserved-namespace rule applies only to the two tags above.
...
- The marketplace listing's `displayName` field is the gemini's own name; it may not begin with "Ruflo."
...
1. Loads `gemini.config.json` `branding.mode`.
2. If `powered-by`:
   - Asserts `package.json` `keywords` contains `"ruflo-gemini"`.
   - Asserts the CLI banner (output of `npx <gemini> --version` or `--help`) contains "powered by ruflo" or "built with ruflo" or equivalent.
   - Asserts the marketplace listing's `displayName` does not begin with `"Ruflo"`.
3. If `independence`:
   - Asserts `package.json` `keywords` does **not** contain `"ruflo-gemini"` or `"ruflo-marketplace"`.
   ...
   - The `package.json` `dependencies` may reference `@zagents/kernel` — that is the only allowed reference.

Failure refuses to publish. The script's check lives at `scripts/branding-policy-check.mjs` in every gemini and is generated by the composer.
...
| Marketplace `description` opening | "An Acme gemini on the ruflo framework." | "Acme's customer support agent." |
| `package.json` `keywords` | `["ruflo-gemini", "support"]` | `["support"]` |
...
A powered-by gemini can switch to independence post-generation. Run `npx <gemini> branding switch-to-independence`. The tool:

- Removes the powered-by strings.
- Updates `gemini.config.json` `branding.mode`.
- Drops `"ruflo-gemini"` from `package.json` `keywords`.
...
Powered-by harnesses may use the ruflo logo per the trademark guidelines published at `https://ruflo.dev/brand` (placeholder URL; the actual asset bundle ships with the kernel under `@zagents/kernel/brand/`). The constraints: minimum size, clear space, color-on-color rules. The kernel's brand assets directory carries SVG + PNG; harnesses include them by reference.
...
A gemini author who picks a name that conflicts with an established product (e.g. `@acme/anthropic-clone`) is out of scope for this ADR. The gemini author is responsible for their own name's legal cleanliness. The composer does not run a trademark search; that is the user's job.
...
- **The publish-time check is one more script to maintain.** We generate it; we test it; we ship it with every gemini. The cost is bounded.
...
- The kernel itself does not care about branding mode. The branding rules are entirely in the gemini layer.
...
Reject independence; every gemini must acknowledge ruflo. Rejected because the regulated-brand and white-label cases are real. Forcing acknowledgment makes generation a no-go for many organisations.
...
Reserve `@ruflo`, `@gemini`, `@agent`, etc. Rejected because reserving names that other people might legitimately want is impolite. We reserve only what we publish.
...
1. **Reserved scope rejection** — `create-agent-gemini foo --scope @ruflo` refuses with a clear error.
2. **Mode default** — interactive composer with default presses produces a powered-by gemini.
3. **Independence requires confirmation** — selecting independence requires a typed rationale.

### Publish-time enforcement tests

4. **Powered-by gate** — a powered-by gemini with the keyword stripped fails the gate.
5. **Independence gate** — an independence gemini with "ruflo" in the README fails the gate.
6. **Banner enforcement** — generated CLI banner contains the expected attribution in powered-by; does not in independence.

### Migration tests

7. **`branding switch-to-independence`** — converts a powered-by gemini to independence; subsequent publish passes the independence gate.
...
- ADR-002 (Kernel boundary) — the `@zagents/kernel` scope choice.
