# ADR-024: Agent Gemini Studio + in-browser Verify
...
The web UI started as a two-mode generator (full gemini / single artifact). Three things pulled it toward a product surface rather than a tool page: the repo importer (ADR-023) needs a home, MCP policy (ADR-022) needs a trust-check, and the project's positioning is "the agent gemini supply chain — idea → gemini → package → install → audit," not "a scaffold picker." The supply chain has a missing link in-browser: **trust**. A user can generate a gemini but has no zero-install way to confirm it is well-formed and its MCP policy is safe before they ship it.

## Decision

Reframe the app as **Agent Gemini Studio** with four tabs, and add a **Verify** capability that closes the trust link entirely client-side.

### Four tabs

- **Repo → Gemini** — paste a URL, get a recommended editable plan (ADR-023).
- **Create gemini** — the full branded-runtime builder (templates, primitives, MCP, policy). Accepts a `seed` config so a repo plan flows straight in via "Open in builder".
...
- The supply chain is now demonstrable end-to-end in one page: repo → plan → branded gemini → **verify** → download.
...
- `verifyFileMap` passes a freshly generated secure gemini (`ok: true`); flags a tampered `package.json` (HIGH); flags a policy weakened to `allowShell`/non-default-deny (HIGH); treats an MCP-off gemini as clean info. (`apps/web-ui/src/generator/__tests__/verify.test.ts`.)
- e2e: the four tabs render; Repo→Gemini rejects a non-GitHub URL; Verify shows the dropzone + checklist. (`apps/web-ui/e2e/generator.spec.ts`.)
