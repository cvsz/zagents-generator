...
- The trivial gemini ("3 agents, Claude Code, no plugins").
- The exotic gemini ("federation + multi-host + custom DISTILL").
...
|---|---|---|
| Phase 0 — Kernel extraction | Static checks (import-boundary, API-surface freeze); London-school units for every kernel subsystem; the test gemini as integration canary | `packages/kernel/test/`, `packages/test-gemini/` |
| Phase 1 — Generator MVP | Composer state machine units; renamer round-trips; trivial-gemini end-to-end; smoke contract | `packages/create-agent-gemini/test/`, `tests/e2e/trivial-gemini/` |
...
| Phase 3 — Composer | Catalogue-picker integration; `--from-existing` end-to-end | `packages/create-agent-gemini/test/composer/`, `tests/e2e/from-existing/` |
...
2. **`fast-check` available** in the packages that use property-based tests (kernel/memory, create-agent-gemini/renamer).
