...
The ADR-044 review verified live that a CLI-scaffolded gemini works against a
real model (`scripts/verify-gemini-live.mjs --dir demo-bot` → PASS, OpenRouter,
$0.000286). While scaffolding the verification fixture, a distinct gap surfaced.

`npx zagents <name> --host opencode` (or any non-`claude-code` host):
...
(`packages/create-agent-gemini/src/index.ts` → `walkTemplate`) renders from
the claude-shaped `templates/<vertical>/` directories and **never invokes the
`@zagents/host-*` adapter packages**. So the host adapters — now fully
spec-complete and tested (ADR-044, 153 unit tests) — are not actually reached by
the primary `npx zagents` scaffold path for 8 of 9 hosts.

This means there are **three** host-config codegen paths today, only one of
which is exercised end-to-end by `npx zagents`:
...
| `packages/create-agent-gemini/templates/` | `npx zagents <name> --host X` | ❌ claude-shaped only |
...
9/9; and `scripts/verify-gemini-live.mjs --all` live-verifies all 9 against a
real model via OpenRouter (9/9 PASS, ~$0.0027 total). +10 CLI unit tests.

### Why dependency-free instead of importing the adapters

The canonical adapters live in `@zagents/host-*`. Importing them would add 9
deps to the standalone `zagents` CLI and couple publish ordering. The
...
- **Risk if not done**: a user running `npx zagents mybot --host opencode`
  gets a claude-shaped gemini, not an OpenCode one — the headline multi-host
  promise is only half-delivered on the CLI.
- **Interim**: the web-UI (Studio) path and the adapters themselves are correct
  and live-verified; only the `npx zagents --host` wiring lags.
