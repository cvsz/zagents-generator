...
- The agent loop and tasks are still **synthetic** (a deterministic file-location task), not a real LLM solving a real repo issue — but the *gemini code under evaluation is real and executed*, which is the Tier-1 → Tier-2 leap. The real-LLM-on-real-SWE-tasks substrate (ADR-098) is the remaining step.
