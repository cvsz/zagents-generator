...
Deterministic ablation gemini committed (`bench/ablation/poincare-vs-euclidean.mjs`); raw numbers in `bench/results/poincare-vs-euclidean-ablation.json`. No production code path changed; `euclideanNiche`/`poincareNicheOf`/`euclideanNicheOf` added as the comparator + shared binning helpers (used by `behavioralNiche`), 326 tests unchanged.
