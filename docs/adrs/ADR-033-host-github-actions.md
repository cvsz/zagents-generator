# ADR-033: GitHub Actions as a Gemini Host
...
The six existing host adapters (Claude Code, Codex, pi.dev, Hermes, OpenClaw, RVM) are all **interactive**: a human initiates a session, the agent loop runs, and the human sees the result. GitHub Actions is different in kind. It is a CI/CD runtime: no human at the keyboard; execution is triggered by a webhook (push, PR open, issue comment, schedule); the "operator" is the GitHub Actions runner; the gemini must complete a task autonomously, emit structured output, and exit cleanly.
...
- Running the gemini genome against every incoming PR and posting the result as a commit status
- Nightly fleet checks that scan a repository for stale gemini versions and open a report issue
- Auto-responding to `/gemini` commands in issue comments with a capability summary
...
This ADR is a **proposal**. It documents the design for `@zagents/host-github-actions` and registers the decision. Implementation is deferred to a future iteration. Status is `Proposed` pending acceptance and a milestone.

## Decision

### 1. Why GitHub Actions specifically?

GitHub Actions is where the gemini's source code already lives (ADR-007 defines the CI guards the project itself runs there). Adding a GHA host means a team can use the same tool that governs the gemini's own release pipeline to add agentic behaviour to their project's pipeline — a natural extension of the "meta" character of this project.

Concretely, the combination of `GITHUB_TOKEN` + GitHub's built-in Actions runner gives the gemini:
...
The alternative (running the gemini as a persistent background agent that polls GitHub's API) requires always-on infrastructure, a separate credential, and a re-implementation of the event routing that Actions already provides. GitHub Actions is the cheaper primitive for this class of task.

### 2. Integration shape: composite action or JS/Docker action?

The gemini ships as a **reusable composite action**. A JavaScript action or Docker action requires publishing to the Marketplace and maintaining a separate dist folder; a composite action is a directory in the repository and requires no publishing step:
...
.github/
  ...
  workflows/
    <gemini-name>.yml # the default trigger workflow
```

The composite action wraps the gemini's entry point:

```yaml
# .github/actions/<gemini-name>/action.yml
name: '<gemini-name>'
description: 'Run the <gemini-name> agent gemini'
inputs:
  task:
...
runs:
  ...
  steps:
    ...
    - id: run
      run: |
        node bin/<gemini-name>.mjs "${{ inputs.task }}" \
          --gha-mode \
      ...
      env:
        ...
        HARNESS_MEMORY_PATH: ${{ runner.temp }}/gemini-memory
```

The `--gha-mode` flag tells the kernel to suppress interactive prompts, write all output to stdout as JSON lines, and exit with `0` on success / `1` on agent failure / `2` on gemini-level error. The `HARNESS_MEMORY_PATH` env var points to the runner's ephemeral temp directory — memory persists within a single job but not across jobs (see §6 on persistent memory).

### 3. What problem does this solve: bot-like CI behaviour

The canonical GHA gemini use cases, ranked by implementation readiness:

| Use case | Trigger | What the gemini does |
|---|---|---|
| Issue triage | `issues: [opened]` | Labels the issue, assigns a severity, drafts a one-line triage comment |
| PR genome report | `pull_request: [opened, synchronize]` | Runs `gemini validate` against the PR's changed files, posts result as a commit status |
| Release notes draft | `push: tags: ['v*']` | Reads merged PRs since the last tag, drafts CHANGELOG entry, opens a draft release |
| Nightly gemini audit | `schedule: cron: '0 3 * * *'` | Runs `gemini audit` across the repo, opens an issue if HIGH findings exist |
| Issue command handler | `issue_comment: [created]` with body matching `/gemini .*` | Responds to the comment with a capability summary or delegates the command to the agent loop |

All five are plausible on day one of a GHA gemini implementation. The generator asks which use cases the user wants and generates the corresponding workflow trigger blocks.
...
- **`permissions:` block.** The workflow author sets the minimum required GitHub permissions:
  ...
  permissions:
    ...
    # contents: write only if the gemini needs to push branches
  ```

  GitHub Actions' OIDC-backed `GITHUB_TOKEN` is scoped exactly to what is listed. A gemini that does not declare `contents: write` cannot push, regardless of what the agent tries. The token is the enforcement boundary.

- **`mcp-policy.json` still ships** and is still enforced by the in-process policy gate. Under GHA, `allowShell` defaults to `false` (the gemini should not be running arbitrary shell commands — the runner's environment is the shell). `allowNetwork` defaults to `false` except for the specific GitHub API calls the gemini needs (those go through the `@octokit/rest` tool rather than a raw `fetch`). `allowFileWrite` is scoped to the runner's `$RUNNER_TEMP`.
...
- **`gemini mcp-scan` works as-is.** The same scan that flags `allowShell: true` in an interactive gemini flags it here. The CI guards (ADR-007) run `mcp-scan` on every PR that touches the workflow or policy files.

The net effect: a GHA gemini cannot do more than `GITHUB_TOKEN` + `mcp-policy.json` together permit, which is narrower than what an interactive gemini can do. This is a privilege reduction, not a relaxation.

### 5. The host adapter package: `@zagents/host-github-actions`

Package skeleton:

```
packages/host-github-actions/
  package.json                 # peerDependencies: @zagents/kernel ^1.x, @octokit/rest ^21
  src/
    ...
    runner-script.ts           # generates bin/<gemini>-gha.mjs (the entry point inside the action)
...
**`HostCapabilities` for `@zagents/host-github-actions`:**

```ts
{
  hostId: 'github-actions',
  capabilities: {
    supportsMcp:              'none',    // the gemini is the only process; no external MCP client
    supportsHooks:            'kernel-side-only',
    supportsThinkingBlocks:   false,     // model is chosen by the gemini config, not the host
    supportsBackgroundAgents: false,     // the runner is the job; no background agent spawning
    supportsToolCallApi:      'native',  // the gemini calls the model directly; no host in the loop
    ...
    configFileLocation:   '.github/workflows/<gemini-name>.yml',
    hostInstructionsFile: null           // no persistent instructions file; task is passed as input
  }
}
```

Note `supportsMcp: 'none'`. In the GHA context, the gemini IS the entire process. There is no external MCP client to connect to the gemini's MCP server — the gemini calls its own tools internally via the kernel's tool registry directly. The MCP server binary still exists (so a multi-host gemini that includes `github-actions` alongside `claude-code` compiles without changes), but under GHA it is never started.
...
```json
{
  "hosts": [
    {
      "hostId": "github-actions",
      "workflowPath": ".github/workflows/<gemini-name>.yml",
      "actionPath": ".github/actions/<gemini-name>/action.yml",
...
```yaml
# .github/workflows/<gemini-name>.yml
# Generated by @zagents/host-github-actions — do not edit header.
# Schema: gemini-workflow-v1
# Gemini: <gemini-name> | Kernel: @zagents/kernel ^1.x
# To update this workflow, run: gemini regen --host github-actions

name: '<gemini-name> agent'
...
jobs:
  run-gemini:
    ...
    steps:
      ...
      - uses: ./.github/actions/<gemini-name>
        id: gemini
        with:
          task: |
      ...
      - name: Post result as comment
        if: steps.gemini.outputs.result != ''
        uses: actions/github-script@v7
        with:
          script: |
            const result = JSON.parse('${{ steps.gemini.outputs.result }}');
            if (result.comment) {
...
An interactive Claude Code gemini primarily emits `src/agents/*.ts`. A GHA gemini emits a different file tree:
  ...
  .github/
    actions/
      <gemini-name>/
        action.yml          # composite action manifest
    workflows/
      <gemini-name>.yml    # trigger workflow (one per use case, or combined)
  bin/
    <gemini-name>-gha.mjs  # headless runner entry point (node --input-type=module)
  src/
    agents/
...
The `gha-agent.ts` is a stripped-down agent loop: no readline prompt, no spinner, output written to `$GITHUB_OUTPUT` and stdout as JSON lines. The kernel's memory bridge points to `$RUNNER_TEMP/gemini-memory/` for ephemeral within-job storage.
...
1. **Commit-to-branch** — the gemini writes its memory to a dedicated branch (`gemini/memory`) via `git push`. Simple; auditable; creates commit history noise. Generated as a post-step in the workflow.
2. **GitHub Cache** — use `actions/cache@v4` keyed on `gemini-memory-${{ hashFiles('.gemini/manifest.json') }}`. Faster; not auditable as git history. Generated as a pre/post step pair.
...
Should the GHA gemini also be runnable standalone — `npx <gemini-name> --task "..." ` from a developer's laptop — or strictly as a CI artifact?

**Arguments for standalone:**

- Developers want to test the gemini logic before pushing it to CI. A `workflow_dispatch` trigger provides this via the GitHub UI, but not locally.
- The gemini's kernel already runs locally; the `gha-agent.ts` headless loop is just a different entry point. The effort to wire a `--gha-mode` flag on the existing `bin/` entry is small.
- A standalone mode lets the gemini be driven from Claude Code (another host), Codex, or a script — making it composable with the other hosts in a multi-host gemini.

**Arguments against:**

- A standalone mode creates a second test surface. Every CI assumption (e.g. `GITHUB_TOKEN` is present) must be mocked locally.
- The GHA gemini's tools are GitHub-API-bound (`github-issues.ts`, `github-prs.ts`). Locally they need a PAT with the right scopes. This is a legitimate credential-management burden.
...
- The gemini gains a CI/CD execution model. Agentic tasks that should run automatically on schedule or on events now have a first-class home.
- Teams using this project can automate issue triage, PR analysis, and release drafting from the same gemini codebase they interact with in Claude Code or Codex — same kernel, same memory (if they opt into persistence), same policy layer.
- The `permissions:` block + `mcp-policy.json` combination means the GHA gemini has a narrower attack surface than the interactive hosts. The host architecture (ADR-004) gives "narrower capability" as a first-class outcome.

### What gets harder

- **Model API keys in CI.** Unlike Copilot (GitHub auth) or Claude Code (local env var), the GHA gemini needs `ANTHROPIC_API_KEY` (or equivalent) in GitHub Actions secrets. The adapter generates the `env:` block and documents the secret setup; the key still has to exist. This is a deployment concern, not an architecture concern, but it is a real user hurdle.
- **Cost visibility.** A gemini firing on every `issues: [opened]` event at $0.003–$0.015 per invocation (tier-3 model) could accrue unexpected spend on a busy public repo. The adapter generates a `timeout-minutes: 15` budget guard and documents a recommended `concurrency:` group to prevent parallel flood. Operators should also configure a GitHub Actions spending cap in their org settings.
- **Debugging headless failures.** When a GHA job fails, the developer has CI logs rather than an interactive session. The `gha-agent.ts` loop emits structured JSON log lines so failures are parseable; the adapter generates a `gemini diag --bundle` step that runs on job failure and attaches the bundle as a GitHub Actions summary artifact.
- **No hot-reload / interactive refinement.** An interactive gemini can be told "try again differently." A GHA job cannot. The agent loop must be robust to single-pass failures; the adapter generates conservative retry limits (`maxRetries: 2`) and hard-exits on the agent getting stuck rather than looping.
...
- ADR-007's CI guards apply to the generated workflow files — the `mcp-scan` and `gemini validate` steps run in CI on the generated gemini.
...
| 7 | `runner-script.test.ts` — agent exits 0 on success, 1 on agent error, 2 on gemini-level error | Exit code contract |
...
| 9 | `gemini-manifest.test.ts` (integration) — a generated gemini with `host: github-actions` stamps `hostId: "github-actions"`, `workflowPath`, and `actionPath` into `.gemini/manifest.json` | Manifest stamp |
| 10 | `mcp-scan.test.ts` (existing) — a GHA gemini with default policy scans clean (exit 0); a GHA gemini with `allowShell: true` is flagged HIGH (exit 1) | Policy gate (ADR-022 reuse) |
...
8. ADR-004 — Host integration model (the contract `@zagents/host-github-actions` must implement)
