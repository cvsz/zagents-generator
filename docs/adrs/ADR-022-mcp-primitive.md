...
**Status**: Accepted
**Date**: 2026-06-14
**Related**: ADR-004 (host integration model), ADR-005 (marketplace plugin), ADR-011 (witness + provenance), ADR-020 (web generator UI), ADR-023 (repo-to-gemini importer)

## Context

MCP (the Model Context Protocol) is an open standard for connecting AI applications to external systems over JSON-RPC with stdio and Streamable HTTP transports. For this project it is not optional: it is the interop layer that lets a *generated* gemini be driven by Claude Code, Codex, Hermes, OpenClaw, and other hosts without bespoke glue per host.

But MCP must not become the whole product. The meta-gemini's claim is broader than "another MCP scaffold": the gemini is the operating layer *around* the protocol — memory, routing, policy, validation, signing, release gates, multi-host execution. **MCP standardizes access; the gemini governs execution.**
...
.gemini/mcp-policy.json       the policy as inert, scannable data
.gemini/mcp-capabilities.json a capability manifest hosts can inspect
```
...
- **`gemini mcp-scan [path]`** — "npm audit for agent tools." Static-only (never executes anything) inspection of the policy + host permissions + `package.json` that flags: no policy, not-default-deny, shell/network/file-write grants, missing approval gate, missing audit log, missing timeout, missing call budget, wildcard tool permissions, risky `Bash(rm|curl|sudo …)` allow-rules, unguarded secret reads, and unpinned dependency ranges. Exit 1 on any HIGH.
- **Witness (ADR-011)** — because the policy and capability manifest are files in the tree, they are fingerprinted into `.gemini/manifest.json` and signed, so a release's tool surface + policy are provenance-bound.
...
- A generated gemini is interoperable across every supported host through one standard, but ships *governed* by default — the opposite of the rough MCP servers the measurement studies flag.
- `mcp-scan` turns the security posture into a CI gate, not a hope.
- The capability manifest lets hosts (and a future registry) inspect exactly what a gemini exposes before installing it.
...
- `off` is a first-class mode — the meta-gemini is bigger than MCP, and a memory-only or CLI-only gemini is valid.
...
- **A runtime MCP-proxy that enforces policy out-of-process.** Heavier and host-specific; the in-gemini `policy.ts` gate is portable and travels with the signed artifact.
- **No scanner; rely on review.** Rejected: the whole value is making the risk machine-checkable. A static scanner that never executes repo/gemini code is the safe, CI-able form.
...
- **Scanner (CLI).** A safe default-deny gemini scans clean (`worst: info`, exit 0); a server with no policy is HIGH (exit 1); shell + not-default-deny + wildcard perms are all flagged; missing audit/timeout/secret-guard/unpinned-deps are flagged at their severities. (`packages/create-agent-gemini/__tests__/mcp-scan.test.ts`.)

**Acceptance test (manual):** generate a gemini with `mcp: local`, install it into Claude Code, call the `ping` tool, and verify the witness manifest records the tool schema, version, policy, and generated-file hashes.
