...
### MCP policy (`.gemini/mcp-policy.json`) — granted tools only
...
```json
{
  ...
  "grants": [
    ...
    { "tool": "write_file",     "reason": "write simulation results and circuit artefacts to .gemini/output/" }
  ]
}
```

All MCP tool invocations are written to `.gemini/mcp-audit.jsonl` with timestamp, tool name, circuit ID (content-derived, ≤256 bytes as required by the ruqu-mcp server spec), and result summary.
...
No credentials are required for local simulation mode. When `QISKIT_IBM_TOKEN` is absent, the gemini automatically falls back to `ruqu_simulate` + `FakeManilaV2` local-only mode and will not attempt any IBM network call.
