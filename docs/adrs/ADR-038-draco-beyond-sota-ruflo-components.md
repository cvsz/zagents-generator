# ADR-038: DRACO beyond-SOTA — optimizing the research gemini with ruflo intelligence components
...
three-way ablation — **vanilla < gemini < fusion+gemini** — scored on five
dimensions (grounding, coverage, balance, cleanliness, faithfulness). The
benchmark exists; the open question is **can the gemini beat SOTA, and if not,
what makes it beat SOTA?**

Measured so far (honest, not gamed):

- **Cheap tier (n=20)** — haiku-4.5 / gpt-5-mini / gemini-2.5-flash: the thesis
  did **not** hold. Ordering inverted: vanilla 0.7788 > gemini 0.7611 > fusion
...
ruflo components to get there.** "SOTA" for DRACO = the fusion+gemini arm
strictly beating both vanilla and the single-model gemini at frontier tier,
with margin, on the same corpus + scorer.

A single-model pipeline with extra steps is not enough — the cheap-tier result
proves structure alone can hurt. The gemini needs components that add *real*
...
Improve the DRACO research gemini by applying **ruflo intelligence components**
...
2. If fusion does not beat vanilla AND the single-model gemini with margin,
...
| Tier | vanilla | gemini | fusion+gemini | thesis | notes |
|------|---------|---------|----------------|--------|-------|
| cheap n=20 | 0.7788 | 0.7611 | 0.7594 | NO | structure hurt on weak models |
| frontier n=20 (baseline) | 0.7143 | 0.6126 | 0.6472 | NO | gemini DEGRADES −0.10 vs vanilla; fusion recovers +0.035 but still < vanilla. ordering: gemini < fusion < vanilla |
...
dossier loses grounding* — the gemini rebuild loses it, the prune loses it —
...
no gemini can ever win. What IS proven is the *mechanism* that makes winning
hard — see below.)
...
|-----|-------------------|-------------------------------|
| gemini (6-stage) | −0.10 | no — a real LOSS |
...
scorer at frontier tier, within observed variance no tested gemini arm exceeded
...
- **transform** (6-stage gemini, verify→prune): loses live URLs → grounding −0.10 / −0.03.
...
working: it falsified the gemini-beats-vanilla thesis with a mechanism, rather
than rubber-stamping it. The fusion arm's one real, measured contribution
(independent verify recovers ~+0.035 vs the single-model gemini) stands, but
...
The gemini loses to vanilla at BOTH tiers, and at frontier the gap is large
...
**Revised optimization target:** the gemini must AUGMENT the strong direct
...
The gemini loses on `grounding` (= live cited URLs / total cited URLs) because it
...
composite win only when the gemini's dead citations are REDUNDANT* (co-located
...
removed content). So the grounding penalty is recoverable **iff** the gemini
...
**Result (7 unit tests, offline, no run):** on a synthetic gemini output whose
...
verify-then-cite gemini can honestly out-score vanilla: **maintain a live mirror
per claim in the retrieval pool**. It is the actionable spec for the next live
frontier run (supply the gemini a liveness-checked source pool and enforce live
...
## Arms 5+6 WIRED into the gemini — `--grounded` (runnable)

Arms 5 (grounding gate) and 6 (live-citation enforcer) were standalone library
stages. They are now wired into the actual fusion gemini (`fusion.ts`
...
The key honest move: the grounding pass uses the gemini's **own retrieved
sources** as the live-mirror pool. The gemini already runs `search` + `grade`
...
source the gemini ALREADY retrieved that supports the same claim; only claims
with no live mirror are dropped. This replaces the gemini's existing LLM *guess*
...
gemini already graded — so grounding rises toward 1.0 while coverage is held.
This is the falsifiable Self-RAG mechanism, now an executable arm.

Validated offline (`draco-grounded-gemini.test.ts`, 4 tests + 145 bench total):
