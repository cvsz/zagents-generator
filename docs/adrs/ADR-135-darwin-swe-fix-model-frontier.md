...
The same 3 external-package corpus (kernel-js, create-agent-gemini, vertical-base) and a fixed gemini config (search/replace, maxAttempts=2, k=6) are run across four OpenRouter models; only the **model** varies. Resolve-rate and cost are measured per model. (`bench/experiments/swe-model-frontier.mjs`.)
...
- Concrete optimization: the SWE runner should default to `deepseek-chat` (and the fitness loop of ADR-133/134 could evolve the **model** as another gene). The "model" axis is now part of the gemini optimization surface, with measured evidence.
