...
| 3. Tool surfaced in the matching CLI's `--help` text | User running `gemini --help` sees it, but only if they already know to ask |
...
Stage 4 is where iter 93 + iter 94 landed: `gemini doctor` and `gemini validate` umbrellas both recommend `gemini diag --bundle` on FAIL.
...
Step 2: Surface     — Add to README day-to-day commands table + gemini help text + codex skill
...
| Step | Iter | Surface |
|---|---|---|
| 1 — Build | 90 | `diag.ts` + 3 new tests in `gemini-diag.test.ts` |
| 2 — Surface | 91 | README day-to-day row + `.codex/skills/diag-gemini/README.md` Equivalent CLI block |
| 3 — Catalog | 91 | `.claude-plugin/plugin.json` command description + `dev-toolkit.mjs` summary |
| 4 — Discover | 93 | `gemini doctor` FAIL message — "Next: gemini diag … --bundle" |
| 4 — Discover | 94 | `gemini validate` umbrella FAIL message — same suggestion, same redaction reassurance |
...
1. **Copy-pasteable command using the user's actual context.** iter-93's test pins this: `gemini doctor /some/path` produces a suggestion that uses `/some/path`, not `cwd`. Users on Windows with spaces in paths shouldn't have to mentally substitute.
...
|---|---|
| User-facing CLI tool that solves a problem users will independently encounter (`gemini diag --bundle`, `gemini audit`, `gemini sbom`) | Full 5 steps |
...
| Skip step 3 (catalog) | Tab-completion drift (iter 67-69 caught this for the gemini subcommand surface). Users find the feature when they encounter the failure but don't tab-discover it. |
...
| 3 | `packages/create-agent-gemini/__tests__/validate.test.ts` "umbrella FAIL message recommends diag --bundle (iter 94)" | Step 4 (validate) |
| 4 | `packages/create-agent-gemini/__tests__/validate.test.ts` "HEALTHY result has no bundle suggestion noise" | Step 4 — only on FAIL invariant |
| 5 | `__tests__/gemini-diag.test.ts` "sanitises secret-like keys in the manifest" (iter 90) | Step 4 reassurance — sanitisation is real |
...
- iter 90 — `gemini diag --bundle` shipped (step 1 + tests)
...
- iter 93 — `gemini doctor` FAIL message recommends `--bundle` (step 4 + tests)
- iter 94 — `gemini validate` umbrella FAIL message recommends `--bundle` (step 4 + step 5)
