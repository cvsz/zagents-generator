...
The scoped `.gemini/mcp-policy.json` follows ADR-022 default-deny. Only the following tools are granted:
...
```json
{
  ...
  "audit": { "enabled": true, "path": ".gemini/audit.jsonl" }
}
```

`Bash` is denied by default to prevent shell-escape during unattended runs. If the user opts in to write operations via `--allow-write`, the policy emits a second file `.gemini/mcp-policy.write.json` that additionally grants the `uploadFile` / `createRepo` hub functions, logged to the same audit trail.
...
4. **Verification gate (ADR-050).** The `verifier` agent re-runs a lightweight read-back check (re-classify or re-embed a subset of the inference output) before the gemini marks the task done.
