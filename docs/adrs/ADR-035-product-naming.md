...
| Iter | Name | Outcome |
|---|---|---|
| pre-108 | `create-agent-gemini` | Squatted by `neohum/create-agent-gemini` on npm (unrelated project). Could not publish under it. |
...
| 118 | `openharness` (rebrand to **OpenHarness**) | User pivoted the brand to "OpenHarness" (with the in-app heading "Open Gemini Studio"). Full mechanical rename across packages, tests, docs, Web UI. |
| 124 | `metaharness` (rebrand to **MetaHarness**) | npm rejected `openharness` at publish time: *"Package name too similar to existing package open-gemini"* (npm anti-typosquatting). User suggested `metaharness`; it published cleanly. |
...
1. **`metaharness` is etymologically anchored.** ADR-001 introduced the project as "the **meta-gemini** for AI agents" in the first sentence of the first ADR. Every subsequent ADR — including ADR-018 (RVM as deployment target), ADR-023 (repo-to-gemini importer), ADR-031 (Bundle JSON Pattern) — uses "meta-gemini" in the running text. The published npm name now matches the term the codebase has used since day one.

2. **Defensive name protection emerged for free.** Once `metaharness` was published, npm's similarity rule began rejecting publish attempts for `meta-gemini` (dashed) by anyone else. The same rule that blocked us from claiming `openharness` (because `open-gemini` already existed) now protects `metaharness` from being typo-squatted in the dashed form.

3. **The CLI invocation reads as a verb of art.** `npx metaharness` parses as "run the meta-gemini" — coherent with the project's positioning as "a factory for agent frameworks" (the user's iter-107 directive). `npx mintagent` and `npx openharness` both worked, but neither anchored on a concept the codebase already taught.

The repository slug `ruvnet/agent-gemini-generator` stays unchanged. It's the **category** ("agent gemini generator"). The npm name is the **brand** ("MetaHarness"). This dual-handle pattern matches the iter-117 user directive that introduced the MintAgent/OpenHarness distinction in the first place.
...
| Layer | Name | Notes |
|---|---|---|
| GitHub repo (category, SEO) | `ruvnet/agent-gemini-generator` | Unchanged. Repository description, README, and badges keep this as the discoverable category. |
| Published CLI (npm) | `metaharness` | One word, all lowercase. `npx metaharness ...` is the user-facing invocation. |
| Published library (npm) | `@ruvnet/agent-gemini-generator` | The thin re-export wrapper from iter 116 stays scoped (`@ruvnet/...`) under the category name. |
| Product/brand display | **MetaHarness** | Used in README H1, Web UI heading ("Open Gemini Studio" → "MetaHarness Studio" follow-up), marketing copy. |
| Tagline | "Mint a custom AI agent gemini from any repo." | Inherited from the iter-107 OpenHarness rebrand; the action verb ("mint") still fits MetaHarness. |
| Repo description | "MetaHarness — the meta-gemini for AI agents. Mint a custom agent gemini from any repo." | One sentence; brand + category + outcome. |

The CLI surface inside the gemini stays `gemini <subcommand>` (21 subcommands at iter 122). Only the wrapper binary that scaffolds *new* harnesses changes name. Per the iter-117 rule:

> **Before generation: `metaharness`. Inside generated gemini: `gemini`.**

### npm naming details

- **`metaharness`** — published. The unscoped CLI binary. Bin entries: `metaharness` (the scaffolder) and `gemini` (the per-gemini toolkit).
...
- `npx create-agent-gemini ...` (the old name in early docs) — was never reachable, since the npm name was squatted by neohum. Documentation should not advertise it.
...
2. **Wrapper dependency**: `@ruvnet/agent-gemini-generator` re-exports from `metaharness` (was `openharness` after iter 118, was `mintagent` before iter 118).
3. **Web UI in-app heading**: "Open Gemini Studio" → "MetaHarness Studio". The iter-118 rename to "Open Gemini Studio" was made specifically because the user wrote it in lowercase ("the web ui is Open Gemini Studio") — that directive is now superseded by this ADR.
...
- **Conceptual integrity.** Every ADR from #001 to #034 already says "meta-gemini" in the running text. The npm name finally matches.
- **Defensive moat from npm's own rules.** No one can typo-squat us with `meta-gemini` (dashed).
...
2. **Pick another available unscoped name** (e.g. `harnessgen`, `agentforge`, `mintforge`). Rejected: every candidate either had the same similarity-rule risk or carried less conceptual weight than `metaharness`. The repo has been talking about itself as a *meta-gemini* for two months; choosing anything else would have been a brand reset, not a brand consolidation.

3. **Sunset the published CLI entirely, ship only the library `@ruvnet/agent-gemini-generator`.** Rejected: the user's iter-107 directive ("Paste any GitHub repo. Get a custom agent gemini.") and the entire Studio onboarding flow are built around an `npx`-runnable CLI. The library wrapper exists for embedding; the CLI exists for hands.
...
| 2 | `__tests__/agent-gemini-generator-lib.test.ts` | `@ruvnet/agent-gemini-generator` re-exports `scaffold`, `HOSTS`, `TEMPLATES` (no asserting on what package they live in — re-exports are opaque to the consumer). |
...
- ADR-001 — *Goals and non-goals* (introduces the term "meta-gemini").
...
- iter-117 commit `76914f5` — MintAgent → "Open Gemini Studio" Web UI rename + Getting Started action launcher.
...
- iter-117 "Suggested command model" user directive (in-conversation, not in any committed ADR before this one) — "Before generation: mintagent. Inside generated gemini: gemini."
