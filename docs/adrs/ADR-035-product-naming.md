# ADR-035: Product naming — ZAgents
...
| 124 | `zagents` (rebrand to **ZAgents**) | npm rejected `openharness` at publish time: *"Package name too similar to existing package open-gemini"* (npm anti-typosquatting). User suggested `zagents`; it published cleanly. |
...
1. **`zagents` is etymologically anchored.** ADR-001 introduced the project as "the **meta-gemini** for AI agents" in the first sentence of the first ADR. Every subsequent ADR — including ADR-018 (RVM as deployment target), ADR-023 (repo-to-gemini importer), ADR-031 (Bundle JSON Pattern) — uses "meta-gemini" in the running text. The published npm name now matches the term the codebase has used since day one.

2. **Defensive name protection emerged for free.** Once `zagents` was published, npm's similarity rule began rejecting publish attempts for `meta-gemini` (dashed) by anyone else. The same rule that blocked us from claiming `openharness` (because `open-gemini` already existed) now protects `zagents` from being typo-squatted in the dashed form.
...
**ZAgents is the product brand. `zagents` is the published npm name.**
...
> **Before generation: `zagents`. Inside generated gemini: `gemini`.**
...
- **`npx zagents ...`** — the canonical invocation as of iter 124.
...
- iter-124 publish — `zagents@0.1.0` claimed on npm.
