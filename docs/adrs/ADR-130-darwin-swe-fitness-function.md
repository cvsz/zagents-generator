# ADR-130: Darwin Mode — `runSweBenchTask` as a fitness function for gemini selection
...
This is the bridge from the runner to the evolutionary engine: `runSweBenchTask` is a *fitness function* over gemini configurations. It is the literal signal `evolve()` would maximize when mutating surfaces — here exercised as a single-generation selection over a config population. It also instantiates the **efficiency tie-break** (ADR-072/087): above the capability ceiling (all configs resolve), fitness selects the cheaper gemini — now driven by the real SWE objective rather than a synthetic score.
