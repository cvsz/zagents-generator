...
A generated gemini must run in any host the user picks. Today the user prompt names four: Claude Code, OpenAI Codex CLI, pi.dev, and Hermes / hermes-agent. The four differ in fundamental ways — what an "MCP server" means to each, where configuration lives, what hooks fire, what model-routing knob exists.
...
export interface HostAdapter {
  ...
  // Configuration
  generateConfig(gemini: HarnessConfig): HostConfigFiles;
  // Returns the files the host needs in the gemini tree (e.g. .claude/settings.json
  ...
  // Smoke contract
  smokeTest(gemini: HarnessConfig): Promise<SmokeResult>;
  // Asserts that, in this host, the gemini can spawn one agent that lists tools.
...
- **Hooks.** The gemini ships a plugin-supplied `hooks/hooks.json`. Reference: https://code.claude.com/docs/en/hooks.
  ...
  - **Matchers** use a pseudo-DSL — e.g. `"Bash(rm *)"` to match a `Bash` tool call invoking `rm`. The adapter generates matchers from gemini-config rules.
...
- **Host instructions file.** `CLAUDE.md` at the gemini root.
...
- **Model invocation.** Delegates to the Anthropic SDK (`@anthropic-ai/sdk`). The gemini's `ANTHROPIC_API_KEY` env var is read. Default tier-2 model: `claude-haiku-4-5`. Default tier-3 model: `claude-sonnet-4-7` or `claude-opus-4-7` (per the routing rule in the kernel).
...
- **MCP registration.** Codex's config is **TOML, not JSON.** The adapter writes `~/.codex/config.toml` mutations (additive; respects existing config). For repo-shared config, it writes a `.codex/config.toml` at the gemini root that Codex picks up only if the project is marked as trusted (see quirk below). MCP servers are declared as `[mcp_servers.<name>]` TOML tables, NOT as `[[mcp_servers]]` arrays:
...
- **Three quirks vs Claude Code (the gemini ships scaffolding that handles all three):**
  ...
  3. **No first-class hooks system.** Codex has no analog to Claude Code's `PreToolUse` / `PostToolUse` / etc. lifecycle hooks. Gemini lifecycle events are approximated through MCP tool calls (the gemini's MCP server exposes lifecycle hooks as tools the agent calls explicitly) or out-of-band (the gemini's `bin/` wrapper fires kernel-side hooks before and after Codex itself runs). The kernel-side fallback (below) is mandatory under Codex.

- **Host instructions file.** `AGENTS.md` at the gemini root. Codex reads this similarly to how Claude Code reads `CLAUDE.md` (per the Agentics Foundation standard the ruflo `@claude-flow/codex` README cites).
- **Skills.** Codex does not have a "skills" concept. Skills are exposed as MCP tools instead — each skill the gemini ships becomes an MCP tool the gemini's MCP server registers, and the agent can call it like any other tool.
- **Commands.** Likewise; commands become MCP tools.
- **Agents.** Codex spawns its own agents per its CLI conventions. The gemini ships agent definitions in a Codex-friendly format that mirrors Claude Code's subagent spec (a `.codex/agents/<id>.md` directory mirroring `.claude/agents/`).
...
- **Source:** https://github.com/badlogic/pi-mono — the gemini lives at `packages/coding-agent` in that monorepo.
- **Config locations.** `~/.pi/agent/` (global) and `.pi/` (project).
- **Instructions files.** `AGENTS.md` (agent instructions) and `SYSTEM.md` (system prompt) at the gemini root. The adapter writes both.
...
- **Adapter shape.** `@zagents/host-pi-dev` ships as a **Pi extension (TypeScript module)**, not as an MCP server.
  ...
  - This means: under Pi, the kernel's `mcp` subsystem is dormant (the gemini's MCP server still exists for other hosts in a multi-host gemini, but Pi's session does not bind to it).
- **Hooks.** Kernel-side only — Pi has no built-in lifecycle hook system comparable to Claude Code's events. The gemini's `bin/` wrapper fires kernel-side hooks before and after the Pi session.
- **Model invocation.** Pi delegates to its own provider routing (see Pi's docs); the gemini reads the same env vars Pi expects (e.g. `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`) and lets Pi pick.
- **Smoke test.** The smoke spins up Pi with the gemini extension loaded, asserts `pi /help` lists the gemini's commands and that one tool round-trips.
...
- **Two integration modes** (the adapter picks based on which runtime the gemini is configured for):
  - **`hermes-agent` runtime mode (preferred when available).** The gemini registers as a `hermes-agent` MCP server using the runtime's `optional-mcps/` discovery (writes `~/.hermes/config/optional-mcps/<gemini>.json` and updates `cli-config.yaml`). This is the current path; it gives the gemini full MCP semantics.
...
- The gemini's `bin/<name>.mjs` entry point wraps the host's CLI invocation.
...
This is more invasive than letting the host fire hooks natively. The advantage is uniform behaviour: a gemini that runs on both Claude Code (native hooks) and Codex (kernel-side hooks for most events) sees the same hook ordering and the same learning signal. The disadvantage is the wrapper introduces latency. The kernel exposes `hooks.bypass = true` to skip hooks entirely for low-latency cases.

### Multi-host harnesses

A gemini can declare multiple hosts. The composer (ADR-003) supports this; the manifest enumerates them; the generated tree contains the union of each host's overlay files. At install time the user runs `npm install` once; at runtime they pick which host to launch by running the gemini's `bin` under the host they want (e.g. `claude` for Claude Code, `codex exec` for Codex).

Three constraints across multi-host harnesses:

1. **The gemini's MCP server is the same in every host.** It is one entry in `package.json` `bin`, one stdio handler. The hosts differ in how they discover it, not in what it is.
2. **The gemini's tool catalogue must be implementable in every selected host's capability set.** If a tool requires `supportsBackgroundAgents` and one of the selected hosts is `false` for that, the composer warns. The user can either drop the tool or drop the host.
3. **Shared memory persists across hosts.** A gemini invocation from Claude Code and a later invocation from Codex hit the same AgentDB / HNSW indices. The user gets continuity across hosts. This is the load-bearing reason for "shared memory" being a kernel concern, not a host one.
...
- The gemini's `package.json` `bin` entry is the same regardless of host. The hosts differ in how they invoke that bin.
...
6. **Claude Code real-host smoke** — CI runs the test gemini against an actual `@anthropic-ai/claude-code` install. Asserts MCP server registers, one tool round-trips.
7. **Codex real-host smoke** — CI runs the test gemini against an actual `@openai/codex` install. Asserts MCP server registers, one tool round-trips.
...
9. **pi.dev (badlogic Pi)** — CI runs the test gemini against an actual `pi-mono`-installed Pi. Asserts the gemini extension loads via `pi install`, `pi /help` lists the gemini's commands, one tool round-trips.
