# ADR-050: Gemini intelligence learned from the verified-swarm push (cross-host)
...
everything.* Generalised: **a gemini should never present agent output as done until a
cheap, exact-as-possible verifier passes** — and the verifier is repo-shaped:

| repo signal | the gemini's verify gate |
|---|---|
...
task-router. Prevents the gemini from burning budget on trivial or hopeless work.
...
primitive surfaced when the gemini selects >1 agent.
...
- **Next:** add the `verify` + `decompose` skills to the catalog (CLI `registry.ts` +
  ...
  all behind the existing primitive toggles (ADR-022) so a minimal gemini stays minimal.
