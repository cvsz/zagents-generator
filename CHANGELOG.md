...
- **`gemini audit --bundle`** — JSON snapshot of the iter-51 audit
...
- **Exit code follows audit verdict** — `gemini audit --bundle > b.json`;
...
- **`__tests__/gemini-audit-bundle.test.ts`** — 4 cases:
...
- **iter-100's `--wizard` flag propagated across user-facing docs**
  ...
  - **`docs/USAGE.md` section 1 (Install)** — 3 entrypoint commands
    listed side-by-side:
    ```
    npx create-agent-gemini my-bot     # arg-driven
    npx create-agent-gemini --wizard   # iter 100 — interactive picker
    npx create-agent-gemini --list     # browse all 19 templates
    ```
    Plus a callout: "Don't know what to pick? Run `--wizard` …" with
    the 4-question summary + the "next time skip with" hint mention.
  - **README day-to-day commands** — new row:
    `First scaffold — don't know what to pick?` → `npx create-agent-gemini --wizard`
- **No dev-toolkit / plugin.json updates** — `--wizard` is a flag on
  `npx create-agent-gemini`, not a `gemini` subcommand. Per
...
- **`npx create-agent-gemini --wizard`** — interactive picker for
...
- **One-paragraph design**:
  ...
  - **"Next time, skip the wizard with:"** hint prints the equivalent
    `npx create-agent-gemini ...` command so the user learns the
...
- **`__tests__/gemini-wizard.test.ts`** — 7 cases:
...
- **Section 5 (Test locally)** gained a "Sanity-check before you ship"
  ...
  - `gemini validate` (iter 20 — 6-check umbrella)
  - `gemini diag` (iter 66)
  - `gemini audit` (iter 51)
  - `gemini sbom > sbom.json` (iter 51)
  - `gemini mcp-scan` (iter 55)
...
- **Stale fixes**:
  ...
  - "Check `npx create-agent-gemini` (no args)" → "Check
...
- **iter-97's `export-config` propagated across user-facing surfaces**
  ...
  - **README day-to-day commands** — new row:
    `Share MCP + Bash + claims config for a security review` →
    `gemini export-config <path> > config.json`
...
- **`gemini export-config` — 16th subcommand**. Real new feature
...
- **Single JSON output** containing:
  ...
  - `host` + `hosts` — what this gemini targets
...
- **Exit code contract**: 0 on success, 2 if `.gemini/manifest.json`
  missing (with FAIL message — same shape as iter 66 diag).
- **Use case**: a user asks "is my MCP allowlist reasonable?" or "are
  my Bash permissions too tight?" — they paste `gemini export-config`
  output into a security discussion without zipping their whole
  gemini. Reviewers see the security surface, not the agent prompts.
...
- **`__tests__/gemini-export-config.test.ts`** — 3 cases:
...
- **5 steps documented**:
  ...
  2. **Surface** — README + codex skill + gemini help (iter 91)
...
- **`gemini validate` umbrella FAIL message now recommends
  `gemini diag --bundle`** — symmetric with iter 93's doctor fix.
...
- **HEALTHY output is unchanged** — the bundle suggestion would be
  ...
  paths in the test.
- **`packages/create-agent-gemini/__tests__/validate.test.ts`**
  8 → **10** cases (+2):
...
- **`gemini doctor` failure message now recommends
  `gemini diag --bundle`**. Closes the discovery loop opened by
...
- **New "Next:" block** on FAIL only (HEALTHY output unchanged):
  ```
  Result: N issues (/path/to/gemini)
...
- **iter-90's `diag --bundle` now surfaces across all 4 user-facing
  ...
  - **README day-to-day commands table** — new row:
    ...
    → `gemini diag <path> --bundle > bundle.json`
  - **`.codex/skills/diag-gemini/README.md`** — Equivalent CLI block
  ...
  - **`.claude-plugin/plugin.json`** — `diag-gemini` command's
...
- **`gemini diag --bundle`** — real new user-facing feature. One
...
- **Bundle contents** (`SupportBundle` interface):
  ...
  - `harnessFiles` — `.gemini/*` enumeration (proves which lifecycle
...
- **`scripts/healthcheck.mjs catalogCount` — new 8th check** that
  ...
  1. `packages/create-agent-gemini/templates/catalog.json` →
     `.templates.length`
  2. `packages/create-agent-gemini/__tests__/generated-templates.test.ts`
...
- **`dev-toolkit.mjs` now lists the 4 runnable example demos**. iter 55
  shipped the orientation map listing scripts/, gemini subcommands,
...
- **New section** in default text output:
  ...
  ## Runnable example demos (examples/) — 4 listed
    quickstart  (iter 32, ~50ms ) — Scaffold minimal gemini → validate
...
- **`examples/education/education.mjs`** — 3-step demo:
  ...
  2. surface the 4 agents + 2 commands + 2 skills the iter-80
     catalog promised
  3. run `gemini validate --skip-gcp` and assert HEALTHY
...
- **README templates table surfaces `vertical:education`** with one-line
  ...
  having to run `npx create-agent-gemini --list`.
...
- **`vertical:education` — 17th vertical template**. Real new product
  ...
  vertical-specific gemini end-users can scaffold today:
  ```
  npx create-agent-gemini my-tutor --template vertical:education --host claude-code
  ```
...
- **Generated scaffold** produces 4 agent files + 2 command files +
  ...
  ```
- **Tests** — `packages/create-agent-gemini/__tests__/generated-templates.test.ts`
...
- **ADR-028 — Skew Detection and Liveness — One Probe, Many Surfaces**.
  Iters 66-78 built a complete skew + liveness fabric (`gemini diag`,
...
- **`gemini validate` umbrella now chains `diag` as a 6th
...
- **README day-to-day commands table refreshed** — was frozen at iter 62.
  ...
  - `Is THIS local kernel compatible with this gemini?` → `gemini diag` (iter 66)
  - `Same, for a CI script` → `gemini diag --json` (iter 73)
...
- **Status table** test-suite row 529/529 → **563/563** and 64 → 66
  test files.
- **New row** for "15 `gemini` subcommands" with the full inline list,
  bolding `diag` as the freshly-shipped one (ADR-027).
- The `create-agent-gemini` CLI row now points down to that subcommand
...
- **Validated end-to-end** against the live CHANGELOG:
  ...
  - **Iter 66** — `gemini diag` — 15th subcommand, the ADR-027 diagnostic UX loop
  - **Iter 67** — Shell completions catch up with the dispatcher
  - **Iter 68** — `gemini diag` chained into the iter-52 e2e lifecycle
  - **Iter 69** — `scripts/dev-toolkit.mjs` currency
  - **Iter 70** — 7th Codex skill — `diag-gemini`
  - **Iter 71** — `gemini diag` now also surfaces generator-version skew
  - **Iter 72** — `scripts/healthcheck.mjs --probe-pages`
  - **Iter 73** — `gemini diag --json`
  ```
...
- **`gemini diag --json`** — machine-readable output for CI scripts
...
- **Position-independent flag** — `gemini diag --json ./path` and
  `gemini diag ./path --json` produce identical output.
- **Sample output**:
  ```json
  {
  ...
  ```
- **`__tests__/gemini-diag.test.ts`** 17 → **20** (+3):
...
- **Two-stage probe** (catches more than "is the index 200?"):
  1. Fetch the index HTML, assert `200` + contains the "Agent Gemini
     Generator" title
...
- **`gemini diag` now also surfaces generator-version skew.** iter 66
  reported kernel skew only. The manifest has always recorded
  `generator` (the create-agent-gemini version at scaffold time);
...
- **Implementation in `diag.ts`**:
  ...
  - new `resolveLocalGeneratorVersion()` reads the running
    create-agent-gemini's own `package.json` (workspace + installed
    fallback)
  ...
  - `formatDiagReport()` prints two extra lines (`manifest generator:`,
    ...
    `gemini upgrade` to preview drift".
- **Generator skew is INFORMATIONAL — never changes the exit code.**
  Templates evolve, but the generated gemini is self-contained;
  generator skew doesn't break anything, it just tells the user
  what `gemini upgrade` would surface.
- **CLI smoke output**:
  ```
  surface:              cli
  ...
  ```
- **Tests** — `gemini-diag.test.ts` 14 → 17 (+3):
...
- **7th Codex skill — `diag-gemini`** wrapping the iter-66 `gemini
  diag` subcommand. Closes the "every user-facing subcommand has a
  matching codex skill" parity:
  - `create-gemini`, `publish-gemini`, `validate-gemini`,
    `gemini-secrets` (iter 22)
...
- **`.codex/skills/diag-gemini/README.md`** — exit code table per
  ...
  list (clone someone else's gemini, post-kernel-bump, cryptic doctor
  failures, fail-fast CI).
- **`.claude-plugin/plugin.json`** updated:
  - `skills` array grew 6 → 7 (`diag-gemini` appended)
  - `commands` array gained a `diag-gemini` entry with description
- **`__tests__/codex-skills.test.ts`** new case (7 → 8 tests): "all 7
  codex skills are present (iter 70: + diag-gemini)" — pinned with
...
- **`scripts/dev-toolkit.mjs` currency** — the iter-55 contributor
  ...
  landed since:
  - `mcp-scan` (PR #1 / iter 55) — Security-scan the gemini MCP
    surface (policy + perms + deps)
  - `analyze-repo` (PR #1 / iter 55) — Recommend a gemini from a
...
- **`gemini diag` chained into the iter-52 e2e lifecycle test** as
  ...
  gemini — but iter 66's diag was added after iter 52 and was never
...
- **Shell completions catch up with the dispatcher** — previously the
  ...
  - `sbom` + `audit` (iter 51) — slipped past the iter-48 completions
    when the per-gemini security subcommands landed
...
- **`__tests__/cli-flags-completions.test.ts`** updated:
  - the `gemini help` assertion lists all 15 subcommands (was 10)
...
- **`gemini diag` — 15th subcommand, the ADR-027 diagnostic UX loop**.
  iter 56 + iter 58 wrote `manifest.meta.surface` and
  `manifest.meta.kernel_version`; iter 66 makes them *actionable*
  from the CLI for an end user who downloaded a gemini and wants to
  know if their local kernel install is compatible.
- **`packages/create-agent-gemini/src/diag.ts`** — new module:
  ...
  - `resolveLocalKernelVersion(harnessDir)` — uses `createRequire`
    rooted at the gemini's own `package.json` so it follows real Node
...
- **CLI smoke (end-to-end against a real scaffold)**:
  ```
  gemini diag /tmp/some-gemini
    surface:              cli
    manifest kernel:      0.1.0
    installed kernel:     0.1.0
    PASS kernel versions match exactly
  ```
- **`__tests__/gemini-diag.test.ts`** — 14 cases:
...
- `gemini help` updated to list the new subcommand.
...
- **README.md rewritten end-user-first** — the previous version led with
  "meta-gemini" architecture which is what *contributors* want to read.
  ...
  - Architecture / meta-gemini sections moved BELOW the try-it path
- Status table refreshed:
  - Tests: 412 → **529** (was stale, behind by ~5 iters)
  - 5 Codex skills → 6 (upgrade-gemini from iter 49 added)
...
- **`__tests__/manifest-kernel-version.test.ts`** (2 cases) —
  scaffold a real gemini against a tmpdir, read the manifest back,
...
- **`manifest.meta.surface` + `manifest.meta.kernel_version`** —
  ...
  sets `'web-ui'`.
- **`gemini doctor` reports the surface** — PASS line when present,
...
- **`scripts/dev-toolkit.mjs`** — single-command repo orientation map.
  ...
  the 12 `gemini` subcommands, and the 6 CI matrix job groups.
...
- **`__tests__/dev-toolkit.test.ts`** (8 cases) — default exits 0 with
  4 sections, lists every gemini subcommand, every entry point,
...
- **PR #1 merge** — combined the web-UI Studio (188 files, +15977 /
  ...
  `crates/template-catalog/`, `gemini mcp-scan` + `analyze-repo`
...
- **`examples/host-tour/`** — single script scaffolds and validates
  one gemini for each of all 6 hosts (claude-code, codex, pi-dev,
...
- **`__tests__/e2e-lifecycle.test.ts`** — strongest cross-iter
  regression net we have. Scaffolds ONE gemini and runs every
  `gemini` subcommand against it in a single chain:
...
- If ANY subcommand breaks the contract, this test fires before
  ...
  `create-agent-gemini <name>` and `gemini publish --confirm`.
- TS suite: **467/467** (up from 465).

### Added — Iter 51 (2026-06-13)

- **2 new `gemini` subcommands** brining the binary to **12 total**,
  surfacing the iter-38 + iter-50 security work at the per-gemini layer:
  - **`gemini sbom [path] [--out=<file>] [--include-dev] [--validate-only]`**
    — emits SPDX-2.3 SBOM for the gemini at `<path>`. Reads
  ...
  - **`gemini audit [path] [--level=high|critical|moderate|low|info]
    [--include-dev]`** — wraps `npm audit --json` against the
    gemini's lockfile and reports structured per-severity counts +
    advisory IDs. Exits 1 on advisories ≥ level. Requires
    `package-lock.json` (CLI prompts for `npm install --package-lock-only`
    if missing).
- **`gemini completions` updated** to include `sbom` + `audit` in all
  ...
  forget to update.
- **`__tests__/gemini-sbom-audit.test.ts`** (8 cases):
...
- Full gemini CLI surface: **12 subcommands** (sign / verify / doctor /
...
- **6th Codex skill: `upgrade-gemini`** — wraps the iter-47 `gemini
  upgrade` CLI command. Mirrors the pattern of iter-22 (validate-gemini,
  gemini-secrets) and iter-28 (verify-witness):
...
- Codex skill catalog: **create / publish / validate / secrets /
  verify-witness / upgrade-gemini** — 6 surfaces (was 5).
...
- **CLI conventional flags on the `gemini` binary**:
  - `gemini --help` and `gemini -h` — aliases for `gemini help`
  - `gemini --version` and `gemini -v` — prints `gemini <version>`
    and exits
...
- **`gemini completions <bash|zsh|fish>` subcommand (10th)** —
...
- **`__tests__/cli-flags-completions.test.ts`** (11 cases):
  ...
  - `--version` and `-v` print `gemini <semver>`
  - help text lists all 10 subcommands + the new Flags section
  - bash completion contains `_harness_completion` function + `complete -F`
  - zsh completion starts with `#compdef gemini`
  - fish completion uses `complete -c gemini` + `__fish_use_subcommand`
...
- Full gemini CLI surface: **10 subcommands** + 4 standard flags.
- TS suite: **446/446** (up from 435).

### Added — Iter 47 (2026-06-13)

- **`gemini upgrade [path] [--apply] [--conflict=<inline|rej>]` CLI
  subcommand** — wires the iter-4 `planUpgrade()` + `applyPlan()` into
  the `gemini` binary as the **9th** user-facing subcommand. Closes
  the gemini lifecycle:
  ```
  scaffold (create-agent-gemini)
      ↓
   edit (user)
      ↓
   upgrade (gemini upgrade [--apply])     <- this iter
      ↓
   sign (gemini sign)
      ↓
   verify (gemini verify)
      ↓
   publish (gemini publish [--confirm])
  ```
- Default mode is **dry-run** — re-renders the template that produced
  the gemini with the same vars, computes a 3-bucket plan (added /
...
- **`__tests__/upgrade-cmd.test.ts`** (6 cases):
  - exits 1 if `<dir>` isn't a generated gemini
...
- Full gemini binary surface: **9 subcommands** —
...
- **`gemini publish [path] [--confirm]` CLI subcommand** — wires the
  iter-5 `publishHarness()` function into the `gemini` binary as
  ...
  - `--confirm` actually pins to IPFS via Pinata. Requires
    `PINATA_JWT` env var (CLI prints the `gemini secrets fetch
...
- **`__tests__/publish-cmd.test.ts`** (5 cases):
  ...
  - `--confirm` without `PINATA_JWT` exits 1 with `gemini secrets
...
- **`gemini mcp <ls|invoke>` subcommand** — surfaces the iter-10/13/34
  ...
  - `gemini mcp ls [path]` — list MCP servers + tools declared in
    `<path>/.mcp/servers.json`. Reports cleanly when the file is absent
    or empty.
  - `gemini mcp invoke <server> <tool> [--args=<json>] [path]` —
    dispatches a tool through the kernel's claim-checked dispatcher
    using the gemini's local `.gemini/claims.json`. Prints the
...
- **`examples/federation/federation.mjs`** — second runnable example
  ...
  network:
  1. provision two gemini tmpdirs (host-A, host-B)
...
- **`__tests__/witness-tamper.test.ts`** (16 cases) — pins the TS
  ...
  - missing `gemini` / `version` fields rejected
  - `findWitness` locates both `<dir>/witness.json` and
    `<dir>/.gemini/witness.json`
...
- This locks the publish-time gate that prevents an unsigned or
  shape-malformed gemini from reaching npm. If the wrapper ever
...
- **`examples/quickstart/`** — first RUNNABLE example. Before this iter
  ...
  - `node examples/quickstart/quickstart.mjs` — scaffolds a `demo-bot`
    gemini from the `minimal` template, runs the full `gemini validate`
...
- **e2e validate-per-host sweep** added to
  ...
  runs the full `gemini validate` umbrella against the output of every
...
- **5th Codex skill: `verify-witness`** — distinct from the iter-22
  `validate-gemini` umbrella because it only checks the Ed25519
  signature:
...
- **`__tests__/marketplace-entry.test.ts`** (6 cases) — pins the entry
  ...
  - skills match iter-22 (`create-gemini, publish-gemini,
    validate-gemini, gemini-secrets`) — catches codex↔marketplace
...
- **path-guard scanner was finding itself** — the Node CI jobs had
  ...
  scanning for. Same for `packages/create-agent-gemini/src/validate.ts`
...
- **Fix**: added a `SKIP_FILES` set listing the three known-meta paths:
  ...
  - `packages/create-agent-gemini/src/validate.ts` (the umbrella that
...
- **`.claude-plugin/plugin.json` rewritten** to reflect current state:
  ...
  - skills: dropped non-existent `list-templates`, added `validate-gemini`
    + `gemini-secrets` from iter 22
...
- **End-to-end integration test**
  ...
  - `minimal/claude-code scaffolds, then 'gemini validate' reports
  ...
  - `scaffold output passes path-guard` — pins that the SCAFFOLDER
    ...
    if it did, every user-generated gemini inherits the original
...
- **`__tests__/codex-skills.test.ts`** (6 cases) — schema validation
  ...
  - the 4 expected skills (create / publish / validate / gemini-secrets)
  - create-gemini lists all 6 hosts
...
- **`gemini validate` umbrella command** — single release-readiness
...
- **`gemini secrets` subcommand** — long-requested GCP Secret Manager
  integration delivered as `gemini secrets <check|fetch|validate-token>`:
...
- **`@zagents/vertical-trading` standalone pack** — first concrete pack
  ...
  - `load()` returns `{ manifest, templateRoot }` for the
    create-agent-gemini external-template loader to consume
...
- **External-template loader in CLI**
  (`packages/create-agent-gemini/src/external-template.ts`):
...
- `HOSTS` const in `create-agent-gemini` now lists 6 hosts
- README badge added, host table extended with the hardware-isolated tier
- USAGE.md + create-agent-gemini/README.md host tables updated
...
- `HOSTS` const in `create-agent-gemini` now includes `openclaw` (5 total)
...
- **Cost tracking subsystem in Rust** (`crates/kernel/src/cost.rs`):
  ...
  - 5 new Rust tests
- **AST-aware identifier rename** (`packages/create-agent-gemini/src/
  rename.ts`):
...
- **Tarball builder for IPFS** (`packages/create-agent-gemini/src/
  tarball.ts`):
...
- **`gemini federate` subcommand** (`packages/create-agent-gemini/src/
  federate.ts`):
  - 5 subactions: `init`, `add`, `remove`, `list [--trusted]`, `status`,
    `help`
  - State persisted at `.gemini/federation.json`
...
- **Examples directory** (`examples/`):
  - `multi-host/` walkthrough showing one gemini targeting Claude Code +
    Codex
  - `federation/` walkthrough showing 2-peer trust-tier coordination

### Added — Iter 8 (2026-06-13)

- **`gemini` CLI binary** (`packages/create-agent-gemini/src/gemini-bin.ts`)
  with three subcommands:
  - `gemini sign [path]` — produce/update witness manifest; reads
...
- Package bin map adds `gemini` binary alongside `create-agent-gemini`
...
- **Codex skills** (`.codex/skills/`):
  - `create-gemini/skill.toml` + `README.md` — invoked as `/create-gemini` in Codex
  - `publish-gemini/skill.toml` — smoke-test + witness-sign + publish gate
...
- **Template engine** (`packages/create-agent-gemini/src/renderer.ts`)
...
- **`.gemini/manifest.json` schema** (`packages/create-agent-gemini/src/manifest.ts`)
