...
// harnesses. Every gemini ships with a signed `witness.json` plus a JSONL
...
pub struct WitnessManifest {
    ...
    pub schema: u32,
    /// The gemini this manifest belongs to.
    pub gemini: String,
    /// Semver version of the gemini this manifest signs.
    pub version: String,
...
pub fn sign_manifest(
    ...
    let bytes = canonical_payload(gemini, version, &entries)?;
...
pub fn verify_manifest(m: &WitnessManifest) -> crate::Result<bool> {
    ...
    let payload = canonical_payload(&m.gemini, &m.version, &m.entries)?;
