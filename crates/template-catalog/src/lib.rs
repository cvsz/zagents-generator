...
//! `packages/create-agent-gemini/templates/catalog.def.mjs`, from which
//! `scripts/gen-templates.mjs` emits `templates/catalog.json`. That JSON is
//! `include_str!`'d here so the Rust core, the `create-agent-gemini` CLI, and
...
const CATALOG_JSON: &str =
    include_str!("../../../packages/create-agent-gemini/templates/catalog.json");
