...
1. **GitHub Security Advisory** — preferred. Go to [Security → Report a vulnerability](https://github.com/ruvnet/agent-gemini-generator/security/advisories/new). The form gives us a private repo where we collaborate on the fix without disclosing details before a coordinated release.
2. **Email** — `ruv@ruv.net` with subject `SECURITY: agent-gemini-generator`. Include:
   - The affected package (`@metaharness/kernel`, `@metaharness/host-*`, `create-agent-gemini`)
   - Affected versions
   - Reproduction steps
...
3. **Generated gemini defaults that are insecure** — if `create-agent-gemini` emits a default that lets a gemini's users execute attacker-controlled code, that's our bug, not theirs.
