...
//
// Lists every dev script + gemini subcommand + key CI gate so a new
...
const HARNESS_SUBCOMMANDS = [
  ...
  { name: 'doctor', summary: 'Smoke-check a scaffolded gemini', iter: 8 },
  ...
  { name: 'publish', summary: 'Pin the gemini manifest to IPFS via Pinata (dry-run default)', iter: 46 },
  ...
  { name: 'sbom', summary: 'Emit SPDX-2.3 SBOM for the gemini (npm)', iter: 51 },
  { name: 'audit', summary: 'npm audit per-gemini — text by default, --bundle for JSON snapshot (iter 102)', iter: 51 },
  { name: 'mcp-scan', summary: 'Security-scan the gemini MCP surface (policy + perms + deps)', iter: 55 },
  { name: 'analyze-repo', summary: 'Recommend a gemini from a local repo (--embed for ruvllm)', iter: 55 },
  ...
  { name: 'score', summary: '5-dimension 0–100 gemini scorecard with README-ready badges (iter 111) — target A grade ≥85', iter: 111 },
  { name: 'threat-model', summary: 'MCP threat-model artifact for PR / compliance review (iter 112) — "enterprise gold"', iter: 112 },
  { name: 'oia-manifest', summary: 'Emit/validate .gemini/oia-manifest.json — OIA v0.1 layer alignment (ADR-034, iter 121)', iter: 121 },
];
 ...
 */
const EXAMPLE_DEMOS = [
  { name: 'quickstart', cmd: 'node examples/quickstart/quickstart.mjs', wall: '~50ms', what: 'Scaffold minimal gemini → validate (any of 6 hosts)', iter: 32 },
...
async function main() {
  ...
  log('');
  log(`## gemini subcommands (${HARNESS_SUBCOMMANDS.length} total) — ${subs.length} listed`);
  for (const s of subs) {
    log(`  gemini ${s.name.padEnd(12)} (iter ${String(s.iter).padStart(2)}) — ${s.summary}`);
  }
  log('');
