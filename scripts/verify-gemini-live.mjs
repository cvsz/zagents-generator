// ADR-044 — Live-LLM host gemini verification via OpenRouter.
//
...
// gemini's CONTENT (system prompt + agent roster + tool manifest) is usable
// by a real model — and it can't exercise the 8 non-Anthropic hosts against a
// live provider at all.
//
// This script closes that gap. For each host it extracts the gemini's
...
// and asserts the model can enumerate the gemini's tools/agents back — i.e.
...
//
// Usage:
//   node scripts/verify-gemini-live.mjs --self-test          # built-in fixture
//   node scripts/verify-gemini-live.mjs --dir bot-claude-code # a real scaffold
//   node scripts/verify-gemini-live.mjs --all                 # every bot-<host>/
//
...
/** Pull the gemini's capability surface out of whatever config a host emitted. */
...
async function liveProbe(key, name, cap) {
  ...
  const res = await fetch(`${BASE_URL}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 60,
      messages: [
        { role: 'system', content: `You are validating a generated agent gemini. Given its manifest, reply with ONLY a JSON object {"coherent":true|false,"capabilities":<count of distinct named MCP servers + agents>}. Nothing else.` },
...
const SELF_TEST_FIXTURE = {
  name: 'demo-gemini',
  cap: {
    systemPrompt: 'You are demo-gemini, a repo-aware coding agent for the acme/widgets monorepo.',
...
async function main() {
  ...
  console.log(`Live-LLM gemini verification (model=${MODEL})\n` + '-'.repeat(72));
  ...
  if (targets.length === 0) { console.log('No gemini scaffolds found (bot-<host>/). Nothing to verify.'); process.exit(0); }
