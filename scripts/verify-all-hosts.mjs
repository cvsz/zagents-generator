// iter 130 — per-host gemini verification using real host CLIs where available.
...
// ADR-045: scaffold each host through the REAL `zagents --host <X>` path so
// this gate verifies the CLI's actual output — not adapter-direct fixtures that
// could pass while `npx zagents --host X` silently emits the wrong tree
...
const cliBin = join(repoRoot, 'packages', 'create-agent-gemini', 'dist', 'bin.js');
...
if (existsSync(cliBin)) {
  try {
    ...
    for (const host of HOSTS) {
      execSync(
        `node ${JSON.stringify(cliBin)} ${JSON.stringify('bot-' + host)} --template vertical:coding --host ${host} --description ${JSON.stringify('Verification gemini for ' + host)} --force`,
        { cwd: scaffoldRoot, stdio: 'ignore', timeout: 30000 },
      );
    }
    console.log(`Scaffolded all ${HOSTS.length} hosts via zagents --host into ${scaffoldRoot}`);
  } catch (e) {
...
for (const host of HOSTS) {
  ...
  try {
    if (host === 'claude-code' && CLAUDE_AVAILABLE) {
      ...
      //   A) `claude -p` from inside the gemini (workspace .claude/settings.json scope)
      //   B) `claude -p --plugin-dir <gemini>` (plugin scope, .claude-plugin/plugin.json)
      ...
      const checks = {
        ...
        rvm:     { path: 'rvm.manifest.toml',     test: (s) => /\[gemini/.test(s),                          tool: 'RVM partition TOML' },
        ...
        // real scaffold existed.) Gemini name is `bot-github-actions`, slug =
        // same after slugify.
        ...
        if (host === 'claude-code') {
          ...
          if (existsSync(pluginPath)) {
            try {
              ...
              pluginOk = !!(p.name && p.author?.displayName === 'Generated by zagents');
...
if (REAL) {
  ...
  const realChecks = {
    ...
    opencode: (dir) => {
      ...
      // Disable the (unpublished) gemini MCP server so the spawn doesn't hang;
    ...
    'github-actions': (dir) => {
      ...
      return { ok: /gha-|gemini|Job ID/i.test(out) || /\bgha\b/i.test(out) || out.includes(`bot-github-actions`), proof: 'act parsed + listed the workflow job' };
    },
  };
