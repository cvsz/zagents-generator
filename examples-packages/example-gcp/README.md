...
| **Verification gate** | Verifier re-reads BigQuery job metadata and Storage object count before the gemini reports output as done |
...
Set these before running `npm run doctor` or invoking the gemini.
...
The gemini will display the estimated bytes-processed (at standard BigQuery on-demand pricing of $6.25/TB) and request explicit confirmation before running the live query.
...
When run without `--live`, the gemini returns: the generated SQL, the dry-run bytes-processed estimate, the schema, and a Gemini-authored natural-language interpretation of what the query would return — all without touching your query quota.
...
- **Audit log.** Every MCP tool call is appended to `.gemini/audit/gcp-<timestamp>.jsonl`.
...
The scaffolded `.gemini/mcp-policy.json` grants exactly these tools (all others are denied at the MCP gate):
