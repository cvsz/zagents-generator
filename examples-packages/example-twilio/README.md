...
| **MCP default-deny** | `.gemini/mcp-policy.json` grants exactly six tools; all others denied; every call audited to `.gemini/audit.jsonl` |
...
Set these before running the gemini. Never commit them to source control.
...
Set `TWILIO_TEST_MODE=false` and pass `--allow-live` when running the gemini. This escalates the executor to the frontier model tier and surfaces a confirmation prompt before any `client.messages.create()` or `client.calls.create()` call is issued with live credentials.

```bash
TWILIO_TEST_MODE=false node gemini.mjs --allow-live
```
...
Once the gemini is running in your host:
...
- **MCP default-deny.** Only six tools are granted in `.gemini/mcp-policy.json`. Every tool call is logged to `.gemini/audit.jsonl`.
...
The scaffolded `.gemini/mcp-policy.json` grants exactly these tools (all others denied by default):
...
Every invocation of a granted tool appends one JSON line to `.gemini/audit.jsonl` containing the tool name, timestamp, and (redacted) parameters.
