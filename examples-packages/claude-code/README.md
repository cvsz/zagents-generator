...
`gemini doctor` validates that Node is recent enough, that the `.claude/` tree is well-formed, that the plugin manifest parses, and that any declared MCP servers resolve. From here you open the folder in your editor and start editing `CLAUDE.md` plus the files under `.claude/`.
...
A: Yes. `claude init` writes a single `CLAUDE.md` into an existing repo. This scaffolds a fresh, opinionated workspace: settings, commands, agents, MCP config, and a loadable plugin manifest, all wired together and validated by `gemini doctor`.
...
Full explainer gist: https://gist.github.com/cvsz/79808ffb89eeb080b1c00bf79fba33e1
