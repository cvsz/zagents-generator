...
- **Namespace isolation** — each gemini instance writes to its own namespace (`gemini-dev` by default); queries never cross namespace boundaries
...
`npm run doctor` checks that your Node version is >= 20, that `PINECONE_API_KEY` is set, that the `.gemini/mcp-policy.json` is well-formed, and that the Pinecone index specified by `PINECONE_INDEX_NAME` exists (or reports that it will be created on first run).
...
| `PINECONE_INDEX_NAME` | No | `gemini-memory` | Name of the serverless index. Created on first run if it does not exist. |
| `PINECONE_NAMESPACE` | No | `gemini-dev` | Namespace within the index. Isolates dev data from other namespaces. |
...
Pinecone does not offer a sandbox API endpoint or test keys. Every API call reaches the live Pinecone service. The gemini defaults to **read-only** (no `PINECONE_ALLOW_WRITE`) and targets the `gemini-dev` namespace so that development activity is isolated from any production data you may have in the same project. Use a dedicated development project in the Pinecone console for maximum isolation.
...
"Search the gemini memory for anything related to authentication and summarise the top 3 results."

"How many vectors are currently in the gemini-dev namespace?"

"Run a cascading search across the gemini-memory and product-docs indexes for information about rate limits."
```
...
- **Read-only by default.** Upsert and index-creation calls are guarded by `PINECONE_ALLOW_WRITE=true`. Without that flag the gemini cannot modify your Pinecone data.
- **Namespace isolation.** The default namespace `gemini-dev` keeps scaffolded-gemini data separate from any other namespaces in your index. Change `PINECONE_NAMESPACE` to further isolate.
- **MCP default-deny.** The `.gemini/mcp-policy.json` grants exactly the Pinecone tools needed and nothing else. Shell access, file writes, and arbitrary network calls are all denied.
- **Verification gate.** The gemini does not report a write as successful until the verifier has confirmed the data is retrievable. This catches partial failures and silent errors.
...
**`pinecone-verifier`** re-issues a `searchRecords` call with the original input text after every upsert and asserts that the inserted record IDs appear in the top-K results. It also calls `describeIndexStats` and confirms the namespace vector count has increased. If either assertion fails, the gemini surfaces an explicit verification error rather than silently reporting success.
...
The gemini mounts the official `pinecone-io/pinecone-mcp` server (GA, 2026) and grants exactly these tools:
