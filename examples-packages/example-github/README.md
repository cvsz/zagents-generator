...
| **MCP default-deny** | `.gemini/mcp-policy.json` grants only the 7 read tools needed; all 4 write tools are blocked unless `ALLOW_WRITES=true` is set |
...
- **MCP default-deny.** Only the 11 tools explicitly listed in `.gemini/mcp-policy.json` are accessible. All others — including shell, file-write, and network outside the GitHub API — are denied. Run `gemini mcp-scan` for a static audit.
- **Rate-limit protection.** `@octokit/plugin-throttling` is wired automatically. The gemini backs off at GitHub's secondary rate limit threshold and retries with exponential backoff.
...
- **Not a certified GitHub App.** This gemini is illustrative. It has not been submitted to the GitHub Marketplace, does not implement webhook signature verification, and is not suitable for use as an automated merge bot or CI gate without significant additional hardening.
...
The `.gemini/mcp-policy.json` grants exactly these tools (all others are denied):
