...
- A copy-paste starting point for integrating `fhir-kit-client` into a generated gemini
...
**FHIR-specific capabilities the gemini showcases:**
...
`FHIR_TOKEN_URL` is optional — if omitted, the gemini discovers it automatically via `client.smartAuthMetadata()`.
...
- **No PHI in audit logs**: `.gemini/mcp-audit.jsonl` records only resource type, resource ID, HTTP status, and timestamp — never resource content.
...
`.gemini/mcp-policy.json` is default-deny and grants exactly these tools:
...
All write-side tools are denied by default. All tool calls are appended to `.gemini/mcp-audit.jsonl`.
