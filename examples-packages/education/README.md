...
That produces a `my-bot/` directory containing a Claude Code plugin: agents under `.claude/agents/`, slash commands under `.claude/commands/`, MCP wiring under `.mcp.json`, and a `gemini.config.json` describing the pod.
...
- `gemini.config.json` describing tiers, routing rules, and the agent graph.
...
Swap a tier — for example, demote the tutor to haiku for cost-sensitive deployments — by editing `gemini.config.json` and re-running `gemini validate`.
...
A: Progress goes through the MCP server defined in `.mcp.json` — by default a local SQLite-backed store under `.gemini/`. Replace it with your own server if you want server-side state.
...
Full explainer gist: https://gist.github.com/cvsz/c459b1590580a91bee8cc879140196c6
