...
| **MCP default-deny** | `.gemini/mcp-policy.json` grants exactly 9 Stripe tools; everything else is denied and audited |
...
|---|---|---|---|
| `STRIPE_SECRET_KEY` | Yes | Server-side secret key. **Must start with `sk_test_`** in the default test-mode gemini. | [Stripe Dashboard](https://dashboard.stripe.com/test/apikeys) → Developers → API keys |
...
The generated gemini validates at startup that `STRIPE_SECRET_KEY` starts with `sk_test_` and aborts with a clear error if a live key is detected without an explicit `--live` flag.
...
| Gate | Behavior |
|---|---|
| Test mode enforcement | Gemini aborts at startup if `STRIPE_SECRET_KEY` does not start with `sk_test_` (unless `--live` was passed to the scaffolder) |
...
| MCP default-deny | Only 9 specific Stripe tools are granted; all others are denied and logged to `.gemini/mcp-audit.jsonl` |
| Verification gate | `billing-verifier` reads back every mutated Stripe object before the gemini reports success |
...
The `.gemini/mcp-policy.json` file grants exactly these 9 tools (all others denied):
...
Every tool call is written to `.gemini/mcp-audit.jsonl` with timestamp, agent identity, tool name, and outcome.
