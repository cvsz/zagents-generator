...
**What it IS:** a starting point that shows how to connect an agent gemini to real IoT infrastructure, reason over a live telemetry stream, and send commands with a non-bypassable safety gate.
...
| **MCP default-deny** | `.gemini/mcp-policy.json` grants exactly six tools; publish and ROS service calls are flagged `dangerous` and require both `ALLOW_ACTUATION=true` and operator approval |
...
This scaffolds the gemini into `my-bot/`, installs dependencies, and runs the health check. With default settings (no credentials, public broker, read-only mode) the doctor check should pass immediately.
...
```
The gemini will: validate the command schema → run safety bounds check → publish at QoS 1 → wait for acknowledgement topic → report DONE or UNVERIFIED.
...
MCP tools are flagged `dangerous` in `.gemini/mcp-policy.json` and are unreachable
unless:
...
- **`telemetry-monitor`**: subscribes to MQTT/ROS topics via `iot__mqtt_subscribe`
  ...
  records to the gemini memory namespace. Runs at Tier 1 — extraction only, no
  reasoning.
...
The generated `.gemini/mcp-policy.json` grants exactly:
