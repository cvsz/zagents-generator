...
| **MCP default-deny** | `.gemini/mcp-policy.json` grants only the six web3 tools needed; all others are denied; every call is audit-logged |
...
|---|---|---|
| `WEB3_CHAIN` | `sepolia` | Chain name recognised by viem/chains (`sepolia`, `holesky`, `hoodi`). If set to `mainnet` the gemini emits a prominent warning and `--allow-send` is still required for any write. |
...
- **Testnet by default.** Chain defaults to Sepolia (chain ID 11155111).
  The gemini logs a warning if `WEB3_RPC_URL` resolves to a mainnet node
  (chain ID 1).
...
- **MCP audit log.** Every tool call is appended to `.gemini/audit.jsonl`.
...
`.gemini/mcp-policy.json` applies **default-deny** (ADR-022) and grants
exactly:
...
All calls are appended to `.gemini/audit.jsonl` with timestamp, agent
