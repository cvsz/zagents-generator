...
`gemini doctor` verifies the risk gate is wired, the paper-trading flag is on, and Claude Code can see the bundled MCP servers.
...
```bash
$ gemini validate policies/risk.yaml
...
Drive the gemini headlessly from a CI job or a research notebook:
...
A: Add a markdown spec under `strategies/`, then ask the strategist agent to implement it. The backtester agent will auto-pick it up. No code changes to the gemini itself are needed.
