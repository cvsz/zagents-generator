...
| **MCP default-deny** | `.gemini/mcp-policy.json` grants eight tools; mutations absent by default; every call logged to `.gemini/mcp-audit.jsonl` |
...
`gemini doctor` checks Node version, validates the MCP policy schema, confirms that environment variables are present (or warns if missing), and verifies that the host adapter config is well-formed.
...
1. Open `.gemini/mcp-policy.json` in the scaffolded project.
...
The audit log at `.gemini/mcp-audit.jsonl` records every tool invocation regardless of whether mutations are enabled.
...
- **Audit log.** Every MCP tool call is appended to `.gemini/mcp-audit.jsonl` with timestamp, agent, tool name, and a SHA-256 hash of the parameters.
...
The `.gemini/mcp-policy.json` grants these eight tools by default:
...
Mutating tools are absent. Every call is logged to `.gemini/mcp-audit.jsonl`.
