...
| **MCP default-deny** | `.gemini/mcp-policy.json` grants only `fetch`, `read_file`, `write_file` (cache paths only), and read-only `bash` |
...
|---|---|---|---|
| `NASA_API_KEY` | Recommended | Your NASA Open API key. If unset, the gemini uses `DEMO_KEY` (30 req/hr, 50 req/day). | Free registration at [api.nasa.gov](https://api.nasa.gov/) |
...
Copy `.env.example` to `.env` and fill in your values. `.env` is gitignored and never written by the gemini.
...
NASA Open APIs have no separate sandbox endpoint. Safety in this gemini comes from two sources:

1. **All endpoints are read-only GET requests.** There is nothing to accidentally mutate.
2. **`DEMO_KEY` as a rate-limited default.** If you have not registered a key, the gemini uses `DEMO_KEY` and enforces a soft session cap of 25 requests, warning you to register before running batch scenarios.
...
- **TLE cache.** The gemini will not re-download a CelesTrak group more than once per 2 hours. A staleness warning fires if the cached TLE epoch exceeds 48 hours.
- **MCP default-deny.** Only `fetch`, `read_file`, `write_file` (restricted to cache/log paths), and read-only `bash` are granted. All other tools are denied, and every invocation is appended to `.gemini/audit.jsonl`.

**Not for production aerospace use.** This gemini is illustrative only. Orbital data carries inherent uncertainty and must not be used for flight planning, launch operations, collision avoidance, spectrum management, or regulatory compliance without authoritative sources and domain-certified software.
...
The scaffolded `.gemini/mcp-policy.json` uses default-deny and grants:

- `fetch` — outbound GET requests to `api.nasa.gov`, `eonet.gsfc.nasa.gov`, and `celestrak.org` only
- `read_file` — TLE cache files in `.gemini/cache/tle/`
- `write_file` — TLE cache in `.gemini/cache/tle/` and pass log in `.gemini/logs/passes.jsonl`
- `bash` — read-only (`node --check` syntax validation only)

All other tools are denied. Every grant invocation is appended to `.gemini/audit.jsonl`.
