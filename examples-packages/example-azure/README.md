...
2. **Service principal**: `az ad sp create-for-rbac --name my-gemini-sp --role Reader --scopes /subscriptions/<id>` — copy `appId` (AZURE_CLIENT_ID), `tenant` (AZURE_TENANT_ID), `password` (AZURE_CLIENT_SECRET).
...
The gemini detects `UseDevelopmentStorage=true` and routes all blob operations to Azurite automatically. You still need a real Azure subscription for ARM resource listing and Azure OpenAI calls.
...
1. Generate a dry-run plan (logged to `.gemini/mcp-audit.jsonl`)
...
- **Secrets via ENV only**: no credentials are ever written to scaffolded files. `.env.example` contains only placeholder values. `.gitignore` covers `.env`, `*.pem`, `*.pfx`, and `.gemini/mcp-audit.jsonl`.
- **Read-only by default**: mutation MCP tools (`azure.resourceGroups.createOrUpdate`, `azure.blobs.uploadBlob`) are granted in the policy but carry `requires_flag: allow_mutations`. The gemini sets `HARNESS_ALLOW_MUTATIONS=false` by default.
...
- **MCP audit log**: every tool call is appended to `.gemini/mcp-audit.jsonl` with timestamp, tool name, calling agent, flag state, and whether it was allowed or denied.
- **Verification gate**: the verifier agent always re-checks output against the live API before the gemini reports done. Discrepancies are flagged, not silently dropped.
...
Defined in `.gemini/mcp-policy.json` (default-deny):
...
All other tools: denied. All calls audited to `.gemini/mcp-audit.jsonl`.
