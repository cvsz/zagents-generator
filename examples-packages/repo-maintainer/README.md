...
The `gemini doctor` step verifies your Node version, that the GitHub MCP server can authenticate (`GITHUB_TOKEN` or `gh auth status`), and that Claude Code is on PATH. Once it passes, point Claude Code at the folder and the three agents register automatically.

## What you get

- `agents/triager.md` — Tier 2 (Haiku). Labels new issues, asks for repros, closes duplicates against the existing issue corpus.
- `agents/reviewer.md` — Tier 3 (Sonnet). Reads PR diffs, runs the lint/test commands declared in `gemini.config.json`, leaves inline review comments.
...
- `gemini.config.json` — declares lint/test/build commands, label taxonomy, release branch, and the maintainer handle the bot should @-mention on escalation.
...
Tune model routing in `gemini.config.json` — set `triager.model` to `sonnet` for noisier issue trackers, or bind the reviewer to `opus` on security-sensitive repos. Add custom labels in the `labels` map and they flow into the triager's prompt at next session start.
...
Full explainer gist: https://gist.github.com/cvsz/a1e9a4b3da25e64ccfcc1124515f722d
