...
`gemini doctor` checks Node, the Claude CLI, MCP server reachability, and the agent files. Fix anything red before sending traffic.
...
A: Wherever you point the `memory` MCP server. The default config uses a local AgentDB store at `./data/kb`. Index your KB once with `gemini kb ingest <path>` (the scaffold ships the ingest script) and the kb-searcher agent will find it.
...
Full explainer gist: https://gist.github.com/cvsz/97a386e2c52cf15c6ec6d33386386180
