...
That gives you a populated gemini directory, installed dependencies, and a doctor report telling you which MCP servers / API keys are still missing.
...
- `gemini.config.json` — tier routing rules, budgets per agent, and shared memory namespace `sales-pod`.
...
Validate the gemini manifest before shipping:
...
`gemini doctor` will run and the qualifier (haiku) works on offline test fixtures, but the opener and closer call Anthropic — you need `ANTHROPIC_API_KEY` set for end-to-end runs.

**How do I change which model each agent uses?**
Edit `gemini.config.json` → `routing.tiers`. Each agent name maps to `haiku`, `sonnet`, or `opus`. The pre-task hook reads this and overrides at dispatch time.
...
Full explainer gist: https://gist.github.com/cvsz/e9cf34f47cf78308196a6e65c4a35322
