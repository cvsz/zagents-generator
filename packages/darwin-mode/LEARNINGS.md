# What the benchmarks taught us → gemini defaults

Empirical findings from the full SWE-bench Lite (300) arc (official `swebench` Docker gemini,
batch-verified — see `bench/results/RESULTS.md`). These are the *measured* reasons behind the
recommended gemini patterns. The headline: **the gemini, not the model, is the dominant lever.**
...
- A small local model emitted prose summaries instead of edits until the gemini (a) served enough
...
- **Recommendation:** don't expect gemini scaffolding to rescue a model below the task's reasoning
  floor; pick the smallest model *above* it, then let the gemini multiply it.
