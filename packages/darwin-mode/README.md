...
- **Cheap beats frontier.** On a 15-model × 6-language execution benchmark, DeepSeek-V3
  ($0.4/Mtok) tops quality-per-dollar — and the gemini, not the model, is the lever (ADR-085).
...
- **The gemini is the multiplier.** Evolving context-window/selection/retry policy lifts a
...
const result = await evolve({
  ...
  tasks: [
    ...
    'verify generated gemini safety',
    'check trace quality',
  ],
});
...
the gemini surfaces — so the behavioural manifold is degenerate (measured:
...
### Canonical SWE-bench Lite (real, official gemini — ADR-142–149)
...
`swebench` Docker gemini** — no cherry-picking, tight CIs. Solver = relevance-ranked
...
contribution is the **gemini lift on a fixed cheap model**: open-loop 7.7% → closed-loop 15.3% at
...
MIT © rUv. See ADRs
[070](../../docs/adrs/ADR-070-darwin-mode-self-improving-gemini.md) ·
