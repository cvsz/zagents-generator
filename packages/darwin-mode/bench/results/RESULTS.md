...
The evolve scorer ceilings quality at 0.985, so the gemini-evolution win is a
...
§1–4 are the model/cost frontier. This section is the gemini **evolving and solving real tasks** — the core "evolve it" claim. Every number is a committed result artifact (`bench/results/`, indexed in `bench/REPRODUCE.md`); the architecture synthesis is `docs/adrs/ADR-108`.
...
| SWE resolve-rate as a fitness function | 130 | scores a gemini config population; fitness selects `searchreplace/3` — equal resolve, **~35% cheaper** |
...
| Multi-package cross-package resolve-rate | 132 | **4/4** across kernel-js, create-agent-gemini, vertical-base, darwin-mode — one runner, $0.017 |
| Evolve the gemini vs real SWE fitness | 133 | `(1+λ)` loop over 3 external packages; elite improves gen0→gen1 (3/3 at lower cost) — the "evolve it" loop, end-to-end |
...
The first run on the **public yardstick** (official `swebench` 4.1.0 Docker gemini, Python).
...
fixed-deepseek single-shot, official `swebench` Docker gemini.
...
traceback/apply-rejection back → retry), official `swebench` gemini:
...
## 11. Local $0-inference models — the gemini-lift, and where it stops (ADR-150) — *PROBE (n=25)*
...
Same gemini, pointed at **ruvultra ollama** (localhost, $0 inference) instead of OpenRouter
...
1. **Gemini-lift is real at the apply layer.** qwen-7b went **0/25 → 13/25 applying patches** once the
   gemini (a) served a 32k context (ollama default 4096 truncated the code prompts), (b) carried the
...
deepseek ~doubles (7.7->15.3%, ADR-149). So the gemini repair lift reproduces at $0 **once a local
...
Same gemini (localize + search/replace + ≤3 test-feedback repair), **cheap base model swapped**
