...
//
// ADR-130 — runSweBenchTask as a FITNESS FUNCTION for gemini optimization. The whole SWE
// runner (123–129) exists so the gemini can be SCORED on real tasks. This closes the loop:
...
console.log(JSON.stringify({
  experiment: 'ADR-130 — SWE resolve-rate as a fitness function for gemini selection',
  corpus: CORPUS.map((c) => c.instance_id), configsEvaluated: CONFIGS.length, model,
  landscape: results,
  winner: winner.config,
  verdict: `FITNESS-SELECTED: '${winner.config}' wins (resolveRate ${winner.resolveRate}, $${winner.cost_usd}) — the SWE resolved-criterion ranks gemini configs; evolve() optimizes this signal`,
}, null, 2));
