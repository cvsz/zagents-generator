...
//
// ADR-122 — ADR-098 step 1: the long-horizon Validation Gemini. A synthetic repo
...
// two policies: (a) the gemini's REAL relevance-ranked contextBuilder (buildContext),
...
//
// Run: node --experimental-strip-types --no-warnings bench/experiments/validation-gemini.mjs
...
// Get the gemini's real relevance-ranked contextBuilder (a generated surface).
...
console.log(JSON.stringify({
  experiment: 'ADR-098 step 1 — long-horizon context validation gemini',
  ...
  verdict: rate(rankedHeld) === 1 && rate(recencyHeld) < 1
    ? 'VALIDATED: relevance-ranked gemini holds the thread across all 50 steps; naive recency loses the old load-bearing file as the repo grows'
    : 'inconclusive',
}, null, 2));
