...
{
  "corpusVersion": 1,
  "gemini": { "fusionModels": { "synthesize": "...", "verify": "..." } },
  "score": 0.0,
...
| M6 | Optimised fusion gemini + fusion-vs-single ablation (the proof) | **Done** |
...
## Optimized gemini + the beyond-SOTA proof (M6)

DRACO doesn't just score a gemini — it lets you **prove** an optimised gemini
...
**The optimised gemini** (`DRACO_OPTIMIZED_MODELS`) routes the load-bearing
...
## The full thesis — vanilla < gemini < fusion+gemini (three-way)
...
| **gemini** | the full 6-stage pipeline, **one** model | structure adds citations + coverage + balance — BUT the verify stage is the same model that wrote the synthesis, so it rubber-stamps its own hallucinations |
| **fusion+gemini** | the pipeline with **different model families** | an independent verifier (different family) catches the fabricated citations the single model approved → highest grounding + faithfulness |

`runThreeWayAblation` runs all three over the **same corpus + scorer**, so each
`<` is a **measured delta**. `thesisHolds` is true only when
`vanilla ≤ gemini ≤ fusion` AND `fusion > vanilla`. The ordering is proven
deterministically in `draco-threeway.test.ts` (a single model rubber-stamps a
fabricated citation → gemini keeps it, grounding 0; fusion's independent
...
| run | vanilla | gemini | fusion | ordering |
|---|---|---|---|---|
| frontier (pre cite-guard) | 0.74 | 0.73 | 0.34 | vanilla ≥ gemini ≫ fusion |
| cheap (pre cite-guard) | 0.76 | 0.49 | 0.65 | vanilla > fusion > gemini |
