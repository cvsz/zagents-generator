...
//
// DRACO — the THREE-WAY thesis (refined M6): vanilla < gemini < fusion+gemini.
//
...
//   - gemini (1 model, 6 stages): structure adds citations + full coverage, BUT
...
// Result: vanilla < gemini < fusion, each a measured delta.
...
function craftedTransport(): OpenRouterTransport {
  const single = DRACO_SINGLE_MODEL;                 // anthropic/claude-opus-4 (vanilla + gemini model + fusion synthesize)
  ...
  return async (model, messages) => {
    ...
    // synthesize (initial draft) — both gemini + fusion produce the same flawed draft.
    if (/Write the dossier/.test(sys)) return { text: HARNESS_DRAFT, tokens: 10 };
    // verify — KEY DIFFERENCE: same-model (gemini) rubber-stamps; independent (fusion) catches.
    ...
    if (/Revise the dossier to address the verifier feedback/.test(sys)) {
      ...
      return verifierFeedback.includes('UNSUPPORTED')
        ...
        : { text: HARNESS_DRAFT, tokens: 9 }; // gemini: nothing flagged → keeps the fabrication
    }
...
describe('DRACO three-way thesis — vanilla < gemini < fusion (deterministic)', () => {
  ...
  it('measures the full ordering: gemini beats vanilla, fusion beats gemini', async () => {
    ...
    // gemini: structure → full coverage + balance, BUT same-model verify rubber-stamps
    // the fabricated citation → grounding still 0.
    expect(report.arms.gemini.perDimension.coverage).toBe(1);
    expect(report.arms.gemini.perDimension.balance).toBe(1);
    expect(report.arms.gemini.perDimension.grounding).toBe(0);
    ...
    expect(report.deltas.fusionOverHarness).toBeGreaterThan(0);  // fusion beats the gemini
    expect(report.deltas.fusionOverVanilla).toBeGreaterThan(0);
    expect(report.ordering).toEqual(['vanilla', 'gemini', 'fusion']); // best last
  ...
  it('with the independent judge, fusion also leads on faithfulness', async () => {
    ...
    expect(report.arms.gemini.perDimension.faithfulness).toBe(0); // rubber-stamped fabrication
    expect(report.thesisHolds).toBe(true);
  });
});
