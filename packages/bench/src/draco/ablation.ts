...
//   - fusion: the DRACO-optimised gemini (DRACO_OPTIMIZED_MODELS) — independent
 ...
 * true iff the optimised fusion gemini scores strictly higher than the
...
//
//   vanilla   < gemini   < fusion+gemini
//   (raw chat)  (structure)  (structure + independent fusion)
//
// The claim the benchmark proves: a GEMINI beats vanilla (structure adds
// coverage/balance/citations), and FUSION beats the gemini (an independent
...
export interface ThreeWayReport {
  ...
  judged: boolean;
  judge?: { model: string; promptVersion: number };
  arms: { vanilla: ArmResult; gemini: ArmResult; fusion: ArmResult };
  /** vanilla→gemini and gemini→fusion deltas. */
  deltas: { harnessOverVanilla: number; fusionOverHarness: number; fusionOverVanilla: number };
  /** True iff vanilla <= gemini <= fusion AND fusion strictly beats vanilla. */
  thesisHolds: boolean;
  /** The measured ordering, best last. */
  ordering: Array<'vanilla' | 'gemini' | 'fusion'>;
}
...
export async function runThreeWayAblation(corpus: DracoCorpus, opts: AblationOptions): Promise<ThreeWayReport> {
  ...
  const dims = { vanilla: [] as DimensionScores[], gemini: [] as DimensionScores[], fusion: [] as DimensionScores[] };
  const faith = { vanilla: [] as number[], gemini: [] as number[], fusion: [] as number[] };
  const tokens = { vanilla: 0, gemini: 0, fusion: 0 };
  ...
  for (const r of perQ) {
    ...
    tokens.gemini += r.h.totalTokens; dims.gemini.push(r.hs.d); if (r.hs.f != null) faith.gemini.push(r.hs.f);
  ...
  const gemini = { ...arm('single', dims.gemini, faith.gemini, tokens.gemini), arm: 'single' as const };
  const fusion = arm('fusion', dims.fusion, faith.fusion, tokens.fusion);

  const ordering = ([
    ['vanilla', vanilla.score] as const,
    ['gemini', gemini.score] as const,
  ...
  return {
    ...
    judged,
    ...(judged ? { judge: { model: judgeModel, promptVersion: DRACO_JUDGE.promptVersion } } : {}),
    arms: { vanilla, gemini, fusion },
    deltas: {
      harnessOverVanilla: gemini.score - vanilla.score,
      fusionOverHarness: fusion.score - gemini.score,
      fusionOverVanilla: fusion.score - vanilla.score,
    },
    thesisHolds: vanilla.score <= gemini.score && gemini.score <= fusion.score && fusion.score > vanilla.score,
    ordering,
  };
}
...
// scorer. The baseline showed the rebuild-everything gemini loses to vanilla;
