...
//
// The research gemini routes each pipeline stage to a DIFFERENT model class
...
export interface FusionResult {
  ...
  grounding?: {
    /** Dead citations rescued by swapping in a live source the gemini retrieved. */
...
export async function fuseResearch(
  ...
  opts: {
     ...
     * gemini ALREADY retrieved (from the search/grade stages — its own pool),
  ...
  // verifier). The single-model GEMINI arm of the ablation runs the same
  ...
  // Deterministic grounding pass (ADR-038 arms 5+6): use the gemini's OWN
