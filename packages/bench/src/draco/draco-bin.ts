...
async function main() {
  ...
  // The full thesis: --threeway runs vanilla < gemini < fusion+gemini.
  if (has('threeway')) {
    ...
    process.stdout.write(`\nDRACO ${kind.toUpperCase()} THREE-WAY${grounded ? ' (grounded fusion)' : ''} — vanilla < gemini < fusion+gemini\n`);
    ...
    process.stdout.write(`  gemini (structure, 1 model):  ${r.arms.gemini.score.toFixed(4)}  (+${r.deltas.harnessOverVanilla.toFixed(4)} vs vanilla)\n`);
    process.stdout.write(`  fusion+gemini (independent):  ${r.arms.fusion.score.toFixed(4)}  (+${r.deltas.fusionOverHarness.toFixed(4)} vs gemini)\n`);
    process.stdout.write(`  ordering (best last): ${r.ordering.join(' < ')}\n`);
    process.stdout.write(`  thesis (vanilla ≤ gemini ≤ fusion, fusion > vanilla): ${r.thesisHolds ? 'HOLDS' : 'does not hold'}\n`);
    if (out) {
