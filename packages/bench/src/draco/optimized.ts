...
//
// DRACO M6 — the OPTIMIZED research gemini + the single-model baseline to beat.
//
// "Push beyond SOTA" means: a research gemini TUNED for the DRACO rubric that
 ...
 * vanilla<gemini<fusion ordering reproduces) on inexpensive models BEFORE
...
/**
 * ARM 1 — VANILLA: ask one model the raw question in a single call, no gemini
 * structure at all. This is the floor — what you get from a chat box. No
 * decompose, no source-grading, no verify. The thesis: a GEMINI beats this.
 */
...
/**
 * ARM 2 — GEMINI (single-model): run the full 6-stage pipeline (decompose →
