...
//
// Shared types for the Algorithmic Agent Gemini (ADR-047).
//
// The gemini treats models as interchangeable workers and algorithms as the
...
// pluggable workers the gemini conducts.

/** A unit of work for the gemini to satisfy. */
...
/** Hard ceilings the gemini must never exceed in a single run. */
 ...
 * skills) are functions; the gemini owns everything else. Every field feeds the
