...
import { scrubOllamaBlocks, optionalMcpYaml, cliConfigYaml } from '../src/index.js';

describe('@zagents/host-ollama — Ollama-4 quirk handling', () => {
  describe('scrubOllamaBlocks', () => {
    it('strips well-formed <think>...</think>', () => {
      const r = scrubOllamaBlocks('keep me <think>drop me</think> keep me too');
      expect(r).toBe('keep me  keep me too');
    });

    it('strips stray <tool_call>...</tool_call> (Ollama #741 quirk)', () => {
      const r = scrubOllamaBlocks(
    ...
    it('strips <thinking> and <reasoning> (extended-thinking variants)', () => {
      expect(scrubOllamaBlocks('a <thinking>b</thinking> c')).toBe('a  c');
      expect(scrubOllamaBlocks('a <reasoning>b</reasoning> c')).toBe('a  c');
    });

    it('leaves prose mentioning the tag names alone', () => {
      // No paired tags -> nothing to strip.
      const r = scrubOllamaBlocks('We can use the <think> tag to express reasoning.');
    ...
    it('handles non-string input by returning it unchanged', () => {
      ...
      expect(scrubOllamaBlocks(null as unknown as string)).toBe(null);
    });

    it('handles strings with no < at all by short-circuiting', () => {
      expect(scrubOllamaBlocks('no tags here')).toBe('no tags here');
    });
  });
  ...
  // ADR-046 — verified against the authoritative ollama cli-config.yaml.example.
  describe('cliConfigYaml', () => {
    it('emits the real ollama schema: model + agent.personalities (no invented keys)', () => {
      ...
      expect(c).toContain('h: "Be terse."'); // gemini identity → default personality
      // The previously-invented keys are NOT in the real ollama schema.
  ...
  describe('ReDoS hardening', () => {
    it('handles a pathological UNCLOSED <think> in linear time', () => {
      ...
      const out = scrubOllamaBlocks(evil);
    ...
    it('still strips a well-formed block after a long prefix (linear)', () => {
      ...
      const out = scrubOllamaBlocks(`${big}<think>drop</think>${big}`);
      expect(out).toBe(big + big);
    });

    it('strips only up to the FIRST close tag (tempered token correctness)', () => {
      const out = scrubOllamaBlocks('a<think>one</think>b<think>two</think>c');
      expect(out).toBe('abc');
    });
  });
});
