...
//
// @zagents/host-ollama — Ollama Agent (NousResearch) host adapter.
//
// TWO DISTINCT PROJECTS — do not conflate:
//   1. https://github.com/NousResearch/Ollama-Function-Calling — OLDER
//      function-calling reference for Ollama 2/3 models. Parses
//      <tool_call>{"name":...,"arguments":{...}}</tool_call> ChatML tags.
//      No <think> block handling documented.
//   2. https://github.com/NousResearch/ollama-agent — CURRENT (v0.2+)
...
//   - Docs: https://ollama-agent.nousresearch.com/docs/
//   - Install: curl -fsSL https://ollama-agent.nousresearch.com/install.sh | bash
//   - Config: `ollama config set` and cli-config.yaml
//
// CRITICAL QUIRK: Ollama-4 models (e.g. NousResearch/Ollama-4-14B) emit
...
// https://github.com/NousResearch/ollama-agent/issues/741.
//
...
export const HOST_NAME = 'ollama' as const;

/**
 * Strip <think>...</think> and stray <tool_call>...</tool_call> blocks from
 * Ollama assistant content. Boundary-gated: only well-formed paired tags
 * are stripped, prose that merely mentions the tag names is left alone.
 *
 * Mirrors ruflo's scrubReasoningBlocks(). Per Ollama issue #741.
 */
export function scrubOllamaBlocks(text: string): string {
...
/**
 * Ollama optional-mcps/ directory layout: one YAML file per MCP server
 ...
 * `cli-config.yaml.example` in NousResearch/ollama-agent (ADR-046).
 *
 * The real ollama config is a nested schema (`model:`, `agent:`, `skills:`,
 * `memory:`, …). It has NO `name`/`description`/`system_prompt`/`scrub_*`
 * top-level keys (those were assumed, never real). The gemini identity maps
 * onto:
 *   - `model.provider: "auto"`  — auto-detect from credentials (OpenRouter,
 *     Anthropic, …); a generated gemini leaves the choice to the user's keys.
 *   - `agent.personalities.<name>` — a name→prompt map. The gemini system
 *     prompt + each agent's prompt become named personalities (selectable with
 *     `/personality`).
 * Ollama-4 <think>/<tool_call> scrubbing (scrubOllamaBlocks) is RUNTIME logic,
...
export function cliConfigYaml(spec: HarnessSpec): string {
  ...
  const lines: string[] = [
    `# Ollama Agent config for ${spec.name} — subset of cli-config.yaml.example.`,
    'model:',
    '  provider: "auto"  # auto-detect from credentials (OPENROUTER_API_KEY, ANTHROPIC_API_KEY, …)',
    'agent:',
    '  personalities:',
  ];
  // Gemini identity → the default personality.
