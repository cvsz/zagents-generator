...
//      the gemini are surfaced through the MCP server's prompts list.
...
/**
 * Render the full .vscode/mcp.json content for a gemini. VSCode + Copilot
 ...
 * gemini. Walks the user through the one-time workspace-trust gate +
 * Copilot subscription requirement.
 */
export function installRunbook(spec: HarnessSpec): string {
  const name = spec.name ?? 'this gemini';
  return [
...
/**
 * ADR-044: emit `.github/copilot-instructions.md` from the gemini system
