...
//   doctor is the generic smoke check (does .gemini/ exist? is the
...
/**
 * iter 71: resolve the version of create-agent-gemini this process is
...
export interface DiagReport {
  ...
  // `generator` field records the create-agent-gemini version at
  // scaffold time. If the user installed a newer create-agent-gemini
  // since, `gemini upgrade` may produce different output than they'd
...
export async function buildDiagReport(harnessDir: string): Promise<DiagReport> {
  const manifestPath = join(harnessDir, '.gemini', 'manifest.json');
...
export function formatDiagReport(report: DiagReport): SubcommandResult {
  const lines: string[] = [];
  lines.push(`gemini diag — checking ${report.dir}`);
  lines.push('');
  if (!report.manifestKernelVersion && !existsSync(join(report.dir, '.gemini', 'manifest.json'))) {
    lines.push('  FAIL no .gemini/manifest.json found at this path');
    lines.push('       (this directory is not a scaffolded gemini — run gemini diag from a gemini root)');
    return { code: 2, lines };
  }
  ...
  // generator can change without the gemini becoming incompatible
  // (templates evolve, but the *generated* gemini is self-contained).
  // Surfacing it just lets `gemini upgrade` users predict whether
  ...
  const genBlurb: Record<SkewVerdict, string> = {
    ...
    'major-diff':    'MAJOR generator skew (template may have moved — re-run `gemini upgrade` to preview drift)',
 ...
 *
 * Users run `gemini diag --bundle` and paste the output into a GitHub
...
export async function buildSupportBundle(harnessDir: string): Promise<SupportBundle> {
  ...
  if (existsSync(pkgPath)) {
    try {
      ...
      /* unreadable package.json — harnessDir not a real gemini */
    }
  }
  ...
  if (existsSync(join(harnessDir, '.gemini'))) {
    try {
      for (const ent of readdirSync(join(harnessDir, '.gemini'))) {
        harnessFiles.push(`.gemini/${ent}`);
      }
    } catch {
  ...
  return {
    ...
    gemini: { packageName, packageVersion, rufloDeps },
...
export async function diagCmd(args: string[]): Promise<SubcommandResult> {
  ...
  if (bundle) {
    ...
    return {
      ...
      // gate on the bundle output (you can `gemini diag --bundle > b.json`
