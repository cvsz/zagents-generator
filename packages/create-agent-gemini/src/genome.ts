...
//
// `gemini genome <repo>` — 18th subcommand (iter 110). The "strongest next
...
function verdictLine(v: GenomeReport['verdict']): string {
  switch (v) {
    case 'ready':
      return 'READY — this repo is in shape to scaffold a gemini for';
    case 'needs-work':
      return 'NEEDS WORK — gemini will scaffold; expect some manual cleanup';
    case 'blocked':
...
export function formatGenomeReport(r: GenomeReport): string[] {
  ...
  // 6. Recommended gemini plan
  out.push('6. Recommended gemini plan');
  ...
  out.push(`Scaffold it: npx zagents ${r.plan.name} --template ${r.plan.template}`);
  return out;
}
...
function usage(): string[] {
  return [
    'Usage: gemini genome <path> [--out <file>] [--json] [--bundle]',
    '',
...
export async function genomeCmd(args: string[]): Promise<SubcommandResult> {
  ...
  if (!existsSync(dir) || !statSync(dir).isDirectory()) {
    ...
    return { code: 2, lines: [`gemini genome: not a directory: ${dir}`] };
  }
  ...
  if (outPath) {
    try {
      ...
      return { code: 2, lines: [bundle || json ? JSON.stringify(err, null, 2) : `gemini genome: failed to write --out: ${String(e)}`] };
    }
  }
