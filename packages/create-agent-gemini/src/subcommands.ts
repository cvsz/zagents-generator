...
//
// `gemini` CLI subcommands: sign, verify, doctor.
//
// The create-agent-gemini package ships TWO binaries:
//   - create-agent-gemini <name>   (the scaffolder)
//   - gemini <subcommand>          (the per-gemini tooling)
//
// This file implements the subcommands the `gemini` binary dispatches to.
...
/**
 * `gemini verify [path]` — verify the witness manifest of a scaffolded
 * gemini. Defaults to cwd.
 */
export async function verify(args: string[]): Promise<SubcommandResult> {
  ...
  if (!wp) {
    pushLines(lines,
      ...
      `Looked at: ${dir}/witness.json and ${dir}/.gemini/witness.json`,
      `Run 'gemini sign' first to produce one.`,
    );
    return { code: 1, lines };
  }
  try {
    ...
    pushLines(lines, `  gemini: ${manifest.gemini}`);
...
/**
 * `gemini doctor [path]` — local smoke check on a scaffolded gemini.
 * Checks for the markers that indicate a well-formed gemini.
 */
export async function doctor(args: string[]): Promise<SubcommandResult> {
  ...
  pushLines(lines, `gemini doctor — checking ${dir}`);
  ...
  check(existsSync(join(dir, '.gemini', 'manifest.json')), '.gemini/manifest.json exists');
  check(existsSync(join(dir, '.gemini', 'manifest.sha256')), '.gemini/manifest.sha256 exists');
  ...
  if (existsSync(join(dir, '.gemini', 'manifest.json'))) {
    try {
      const m = JSON.parse(await readFile(join(dir, '.gemini', 'manifest.json'), 'utf-8'));
      if (m.meta?.surface) {
  ...
  if (existsSync(join(dir, '.gemini', 'manifest.json')) && existsSync(join(dir, '.gemini', 'manifest.sha256'))) {
    try {
      const m = await readFile(join(dir, '.gemini', 'manifest.json'), 'utf-8');
      const expected = (await readFile(join(dir, '.gemini', 'manifest.sha256'), 'utf-8')).trim();
      const actual = createHash('sha256').update(m, 'utf-8').digest('hex');
      check(actual === expected, '.gemini/manifest.json hash matches .gemini/manifest.sha256');
    } catch {
  ...
  // Common host-specific artifacts (any one is enough — multi-host gemini
  // ships multiple).
...
export async function sign(args: string[]): Promise<SubcommandResult> {
  ...
  if (!existsSync(manifestPath)) {
    pushLines(lines, `No .gemini/manifest.json at ${dir}.`);
    return { code: 1, lines };
  }
  ...
  try {
    ...
    try {
      ...
      if (typeof k.witnessSign === 'function') {
        const payload = JSON.stringify({ schema: 1, gemini: name, version, entries });
    ...
    const out = join(dir, '.gemini', 'witness.json');
...
export async function dispatch(subcommand: string, args: string[]): Promise<SubcommandResult> {
  ...
  if (subcommand === '--version' || subcommand === '-v') {
    return { code: 0, lines: [`gemini ${PACKAGE_VERSION}`] };
  }
  switch (subcommand) {
    case 'verify':
      return verify(args);
    ...
    case undefined:
      return {
        code: 0,
        lines: [
          'Usage: gemini <subcommand> [args]',
          '',
          'Subcommands:',
          '  sign      — produce or update the witness manifest for a gemini',
          '  verify    — verify the witness manifest of a gemini',
          ...
          '  score         — 5-dimension gemini scorecard (0–100, grade A/B/C/F) (iter 111)',
          '  threat-model  — MCP threat-model artifact (enterprise review) (iter 112)',
          '  oia-manifest  — emit .gemini/oia-manifest.json (ADR-034 OIA v0.1) (iter 121)',
          '  plugin-init   — backfill .claude-plugin/plugin.json for legacy harnesses (iter 135)',
          '  help      — show this message',
          '',
          'Flags:',
          '  --help, -h    — same as `gemini help`',
    ...
    default:
      return {
        code: 2,
        lines: [`Unknown subcommand: ${subcommand}`, `Run 'gemini help' for usage.`],
      };
  }
}
