...
//
// `gemini compare <a> <b>` — diff two gemini manifests + per-file
...
async function readManifest(dir: string): Promise<Manifest | undefined> {
  const path = join(dir, '.gemini', 'manifest.json');
...
function formatReport(r: CompareReport): string[] {
  const lines: string[] = [];
  lines.push(`gemini compare`);
  lines.push(`  A: ${r.a.dir}`);
  lines.push(`  B: ${r.b.dir}`);
  lines.push('');
  if (!r.a.manifest || !r.b.manifest) {
    if (!r.a.manifest) lines.push(`  FAIL no .gemini/manifest.json at A (${r.a.dir})`);
    if (!r.b.manifest) lines.push(`  FAIL no .gemini/manifest.json at B (${r.b.dir})`);
    return lines;
  }
  lines.push('  meta:');
...
export async function compareCmd(args: string[]): Promise<SubcommandResult> {
  ...
  if (positional.length < 2) {
    const lines = [
      'Usage: gemini compare <a> <b> [--bundle]',
      '',
