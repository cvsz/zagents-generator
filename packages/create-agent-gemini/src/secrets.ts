...
//
// `gemini secrets` — GCP Secret Manager integration for publish-time
...
//
// Commands:
//   gemini secrets check               validate full GCP setup
//   gemini secrets fetch <name>        fetch a secret value (stdout)
//   gemini secrets validate-token      fetch NPM_TOKEN + npm whoami
//
// Auth path:
...
export async function check(args: string[], runner: GcloudRunner = defaultRunner): Promise<SubcommandResult> {
  const lines: string[] = ['gemini secrets check'];
  let problems = 0;
...
export async function fetch(args: string[], runner: GcloudRunner = defaultRunner): Promise<SubcommandResult> {
  ...
  if (!name) {
    return { code: 2, lines: ['Usage: gemini secrets fetch <secret-name> [--project=<id>] [--version=latest]'] };
  }
  ...
  if (r.code !== 0) {
    return {
      ...
      lines: [
        ...
        'Try: gemini secrets check --secret=' + name,
      ],
    };
  }
...
export async function validateToken(args: string[], runner: GcloudRunner = defaultRunner): Promise<SubcommandResult> {
  const lines: string[] = ['gemini secrets validate-token'];
  const project = args.find(a => a.startsWith('--project='))?.slice('--project='.length);
...
/** Top-level dispatcher for `gemini secrets <subsub> ...`. */
  ...
  switch (subsub) {
    ...
    case 'help':
      return {
        code: 0,
        lines: [
          'Usage: gemini secrets <subcommand> [options]',
          '',
          'Subcommands:',
    ...
    default:
      return {
        code: 2,
        lines: [`Unknown secrets subcommand: ${subsub}`, `Run 'gemini secrets help' for usage.`],
      };
  }
}
