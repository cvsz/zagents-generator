...
// — the scorecard's job is to tell the truth about whether a generated gemini
...
export interface RepoScorecard {
  ...
  /** Estimated USD per gemini run (cheap-tier routing, DRACO finding). */
...
/** Rough per-agent token budget for one gemini run (analysis estimate). */
...
export function buildRepoScorecard(dir: string, generatedAt: string = new Date().toISOString()): RepoScorecard {
  ...
  // Gemini fit — how confidently the recommended archetype matches this repo.
  const harnessFit = clamp100(plan.confidence * 100);

  // Compile confidence — signals a generated gemini will build/run cleanly:
...
/** One candidate gemini design (ADR-041 candidate-generation / beam stage). */
...
/**
 * The top-N candidate gemini designs for a repo, each scored — the beam-search
...
export function formatCandidates(repo: string, cands: CandidateScore[]): string[] {
  return [
    `Top ${cands.length} gemini designs — ${repo}`,
    ``,
...
function usage(): string[] {
  return [
    ...
    'coverage, tool safety, memory usefulness, est. $/run, recommended mode.',
    '--top N lists the N best-fit gemini designs (candidate generation).',
