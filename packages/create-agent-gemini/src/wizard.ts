...
//
// iter 100 (MILESTONE) — interactive wizard for `npx create-agent-gemini`.
//
...
// pickable form:
//
//   1. Gemini name (validated against validateHarnessName())
...
export async function runWizard(catalog: WizardCatalog, ask: Asker): Promise<WizardAnswers> {
  ...
  let name = '';
  while (true) {
    const raw = await ask('Gemini name (kebab-case, e.g. my-coding-bot): ');
  ...
  const description = descRaw.trim() || `${name} — an AI agent gemini`;

  return { name, template, host, description };
}

/**
 * Build the equivalent `npx create-agent-gemini ...` invocation for
...
export function answersToInvocation(answers: WizardAnswers): string {
  const parts = ['npx create-agent-gemini', answers.name];
