...
//
// ADR-041 validation stage — hard/soft constraint checks over a candidate gemini
...
/**
 * Check a recommended gemini design against the constraint set. Pure +
...
export function checkConstraints(profile: RepoProfile, plan: HarnessPlan): ConstraintResult[] {
  ...
  // HARD — a generated gemini must be able to target this repo + stay safe.
  add('language-detected', 'A language was detected (compile target)', 'hard', profile.languages.length > 0,
    profile.languages.length ? undefined : 'no language signal — gemini cannot reliably compile-target');
  ...
  add('buildable', 'A build command was found', 'soft', !needsBuild || profile.buildCommands.length > 0,
    needsBuild && profile.buildCommands.length === 0 ? 'no build command detected — verify the gemini builds' : undefined);
