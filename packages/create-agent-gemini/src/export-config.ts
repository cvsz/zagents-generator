...
//
// `gemini export-config` — emit the gemini's security-relevant config
...
export async function exportConfigCmd(args: string[]): Promise<SubcommandResult> {
  ...
  if (!existsSync(join(dir, '.gemini', 'manifest.json'))) {
    return {
      code: 2,
      lines: [
        `gemini export-config — checking ${dir}`,
        '',
        `  FAIL no .gemini/manifest.json found at this path`,
        `       (this directory is not a scaffolded gemini — run`,
        `       gemini export-config from a gemini root)`,
      ],
    };
  }
