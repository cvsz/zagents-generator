...
//
// `gemini mcp <ls|invoke>` subcommand.
//
...
async function readClaims(harnessDir: string): Promise<Array<{ capability: string; resource?: string; expires_at: number }>> {
  const path = join(harnessDir, '.gemini', 'claims.json');
...
/**
 * `gemini mcp invoke <server> <tool> --args=<json> [path]` — dispatch
 * a tool through the kernel-style claim check and print the outcome.
 *
 * We re-use the TS ToolDispatcher here. For a real-world gemini this
...
export async function mcpInvoke(args: string[]): Promise<SubcommandResult> {
  ...
  if (!server || !tool) {
    return {
      code: 2,
      lines: ['Usage: gemini mcp invoke <server> <tool> [--args=<json>] [path]'],
    };
  }
  ...
  const lines: string[] = [`gemini mcp invoke ${server}/${tool}`];
...
/** Top-level dispatcher for `gemini mcp <subsub> ...`. */
  ...
  switch (subsub) {
    ...
    case 'help':
      return {
        code: 0,
        lines: [
          'Usage: gemini mcp <subcommand> [args]',
          '',
          'Subcommands:',
          ...
          '  - `invoke` reads <path>/.gemini/claims.json for the local claim set',
    ...
    default:
      return {
        code: 2,
        lines: [`Unknown mcp subcommand: ${subsub}`, `Run 'gemini mcp help' for usage.`],
      };
  }
}
