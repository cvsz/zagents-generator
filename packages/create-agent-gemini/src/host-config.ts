...
//
// Until now `npx zagents <name> --host <X>` recorded the host in the
// manifest but emitted only the claude-shaped template tree — the
// `@zagents/host-*` adapters were never reached. This module closes that
// gap. It is intentionally DEPENDENCY-FREE (no import of the adapter packages)
// so the published `zagents` CLI stays standalone, and it mirrors the
...
export interface HostConfigInput {
  name: string;
  description: string;
  /** Whether the gemini ships an MCP surface (templates default to local). */
...
function ghaSlug(name: string): string {
  return (name || 'gemini').trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'gemini';
}
