...
//
// `gemini threat-model <path>` — 20th subcommand (iter 112). Priority 5 from
...
export function buildThreatModel(dir: string, generatedAt: string = new Date().toISOString()): ThreatModel {
  ...
  const policy = safeReadJson(join(root, '.gemini', 'mcp-policy.json'));
...
export function formatThreatModel(tm: ThreatModel): string[] {
  ...
  out.push('');
  if (!tm.mcpInUse) {
    out.push('  mcp surface:         off (no .mcp.json, no .gemini/mcp-policy.json)');
  ...
  if (tm.verdict !== 'clean') {
    out.push('Run: gemini mcp-scan ' + tm.dir + '   # for the full finding list');
  }
  return out;
}
...
function usage(): string[] {
  return [
    'Usage: gemini threat-model <path> [--out <file>] [--json] [--bundle]',
    '',
...
export async function threatModelCmd(args: string[]): Promise<SubcommandResult> {
  ...
  if (!existsSync(dir) || !statSync(dir).isDirectory()) {
    ...
    return { code: 2, lines: [`gemini threat-model: not a directory: ${dir}`] };
  }
  ...
  if (outPath) {
    try {
      ...
      return { code: 2, lines: [bundle || json ? JSON.stringify(err, null, 2) : `gemini threat-model: failed to write --out: ${String(e)}`] };
    }
  }
