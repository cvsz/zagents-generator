...
//
// `gemini analyze-repo [path]` — the Node-side counterpart to the Studio's
// Repo → Gemini importer (ADR-023/026). It inventories a LOCAL repo, builds a
// deterministic profile, scores archetypes, and emits repo-profile.json +
// gemini-plan.json (optionally scaffolding from the plan).
//
...
export const ARCHETYPES: Archetype[] = [
  { id: 'ai-agent-framework-gemini', label: 'AI agent framework', description: 'multi agent orchestration framework swarm planner worker tools llm', keywords: ['agent', 'agents', 'mcp', 'llm', 'orchestration', 'swarm', 'tool', 'autonomous'], manifestHints: ['@modelcontextprotocol', 'openai', 'anthropic', 'langchain'], template: 'vertical:agentics', agents: ['orchestrator', 'planner', 'worker', 'critic'], skills: ['run-swarm', 'memory-inspect'], commands: ['doctor'], mcp: 'local' },
  { id: 'mcp-server-gemini', label: 'MCP server', description: 'mcp server tools protocol json rpc stdio streamable http resources prompts', requiredSignal: 'hasMcp', keywords: ['mcp', 'tool', 'server', 'protocol', 'stdio', 'resource', 'prompt'], manifestHints: ['@modelcontextprotocol', 'mcp'], template: 'vertical:coding', agents: ['reviewer', 'test-writer'], skills: ['plan-change'], commands: ['doctor', 'review-diff'], mcp: 'remote' },
  { id: 'rust-crate-gemini', label: 'Rust crate', description: 'rust crate cargo wasm systems performance library', requiredLang: 'rust', keywords: ['rust', 'cargo', 'crate', 'wasm', 'no_std', 'clippy'], manifestHints: ['[package]', '[dependencies]', 'edition'], template: 'vertical:coding', agents: ['architect', 'implementer', 'reviewer', 'test-writer'], skills: ['plan-change'], commands: ['doctor', 'review-diff'], mcp: 'local' },
  { id: 'typescript-sdk-gemini', label: 'TypeScript SDK', description: 'typescript javascript sdk npm package library node esm', requiredLang: 'typescript', keywords: ['typescript', 'sdk', 'npm', 'node', 'library', 'api', 'client'], manifestHints: ['typescript', 'tsc', 'vitest', 'jest'], template: 'vertical:coding', agents: ['architect', 'implementer', 'reviewer', 'test-writer'], skills: ['plan-change'], commands: ['doctor', 'review-diff'], mcp: 'local' },
  { id: 'data-pipeline-gemini', label: 'Data / ML pipeline', description: 'data pipeline machine learning training model evaluation dataset python', requiredLang: 'python', keywords: ['data', 'ml', 'model', 'training', 'dataset', 'pipeline', 'pandas', 'torch', 'sklearn'], manifestHints: ['numpy', 'pandas', 'torch', 'scikit', 'tensorflow'], template: 'vertical:ai', agents: ['data-curator', 'trainer', 'evaluator', 'deployer'], skills: ['eval-report'], commands: ['doctor'], mcp: 'local' },
  { id: 'research-gemini', label: 'Research / docs', description: 'research documentation knowledge synthesis citations literature review', keywords: ['research', 'docs', 'documentation', 'paper', 'knowledge', 'citation', 'wiki'], manifestHints: ['mkdocs', 'docusaurus', 'sphinx'], template: 'vertical:research', agents: ['scout', 'synthesizer', 'fact-checker', 'citer'], skills: [], commands: ['doctor'], mcp: 'local' },
  { id: 'devops-gemini', label: 'DevOps / infra', description: 'devops infrastructure kubernetes terraform ci incident on call deploy', keywords: ['devops', 'kubernetes', 'k8s', 'terraform', 'docker', 'ci', 'deploy', 'infra', 'helm'], manifestHints: ['dockerfile', 'helm', 'terraform'], template: 'vertical:devops', agents: ['responder', 'runbook-runner', 'escalator', 'postmortem'], skills: [], commands: ['doctor'], mcp: 'local' },
  { id: 'consulting-gemini', label: 'Business / consulting', description: 'business strategy consulting product analytics metrics roadmap operations', keywords: ['business', 'strategy', 'product', 'analytics', 'metrics', 'roadmap', 'consulting'], manifestHints: [], template: 'vertical:business', agents: ['analyst', 'strategist', 'ops-coordinator'], skills: [], commands: ['doctor'], mcp: 'local' },
];
...
export function scoreArchetypes(profile: RepoProfile, semantic?: Record<string, number>): Scored[] {
  ...
  return ARCHETYPES.map((a) => {
    ...
    const structure = Math.min(1, (profile.hasClaude ? 0.34 : 0) + (profile.hasMcp && a.id === 'mcp-server-gemini' ? 0.5 : 0) + (profile.languages.length ? 0.33 : 0));
...
export function recommendPlan(profile: RepoProfile, semantic?: Record<string, number>): HarnessPlan {
  ...
  return {
    name: kebab(`${profile.name}-gemini`),
    hosts,
    template: a.template,
    archetypeId: a.id,
...
export async function analyzeRepoCmd(args: string[]): Promise<{ code: number; lines: string[] }> {
  ...
  const root = resolve(dir);
  const lines: string[] = [`gemini analyze-repo — ${root}`, ''];
  ...
  const planPath = join(out, 'gemini-plan.json');
  ...
  lines.push(`Gemini:  ${plan.name}  ·  template ${plan.template}  ·  hosts ${plan.hosts.join(', ')}`);
  ...
  if (scaffoldName) {
    ...
    const r = await scaffold({ name: scaffoldName, template: plan.template, host, targetDir: target, description: `Gemini for ${profile.name}`, generatorVersion: '0.1.0' });
    lines.push('', `Scaffolded ${scaffoldName} (${r.paths.length} files) from the plan into ${target}`);
  } else {
    lines.push('', `Scaffold it:  gemini analyze-repo ${dir} --scaffold ${plan.name}`);
  }
