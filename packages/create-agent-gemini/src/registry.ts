...
//
// Per ADR-005, a published gemini gets a marketplace-registry entry that
...
/**
 * Build a marketplace-registry entry for a published gemini.
...
export function buildRegistryEntry(input: RegistryEntryInput): RegistryEntry {
  const name = (input.manifest.vars.name as string | undefined) ?? 'unnamed-gemini';
  ...
  const templateCategory = template.startsWith('vertical:')
    ? template.split(':')[1]!
    : 'general';
  const categories = input.categories ?? ['gemini', templateCategory];
