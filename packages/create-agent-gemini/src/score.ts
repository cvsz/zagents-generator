...
//
// `gemini score <path>` — 19th subcommand (iter 111). Priority 2 from the
...
function scoreRepoUnderstanding(dir: string, manifest: any): DimensionScore {
  ...
  if (manifest) {
    ...
    signals.push('no manifest — cannot identify what this gemini is');
  }
...
function scoreMcpSafety(dir: string): McpScore {
  const signals: string[] = [];
  const policy = safeReadJson(join(dir, '.gemini', 'mcp-policy.json'));
...
function scorePublishReadiness(dir: string, manifest: any): PublishScore {
  const signals: string[] = [];
  let s = 0;
  const witnessSigned = fileExists(dir, '.gemini/witness.json');
  if (witnessSigned) {
    s += 40;
    signals.push('witness.json present');
  }
  // sbom.json is emitted by `gemini sbom`; presence indicates the operator
  // has at least run it once. Even if it's not committed, presence is the
  // strong publish-readiness signal.
  const sbom = fileExists(dir, 'sbom.json') || fileExists(dir, '.gemini/sbom.json');
  if (sbom) {
    s += 20;
  ...
  if (s === 0) signals.push('no publish signals — run gemini validate first');
...
export function buildScorecard(dir: string, generatedAt: string = new Date().toISOString()): Scorecard {
  const manifest = safeReadJson(join(dir, '.gemini', 'manifest.json'));
...
export function formatScorecard(sc: Scorecard): string[] {
  ...
  out.push('');
  out.push('Badges:');
  out.push(`  Gemini Score: ${sc.badges.score}`);
...
function usage(): string[] {
  return [
    'Usage: gemini score <path> [--out <file>] [--json] [--bundle]',
    '',
    '  5-dimension scorecard for a generated gemini. 0–100, grade A/B/C/F.',
...
export async function scoreCmd(args: string[]): Promise<SubcommandResult> {
  ...
  if (!existsSync(dir) || !statSync(dir).isDirectory()) {
    ...
    return { code: 2, lines: [`gemini score: not a directory: ${dir}`] };
  }
  ...
  if (outPath) {
    try {
      ...
      return { code: 2, lines: [bundle || json ? JSON.stringify(err, null, 2) : `gemini score: failed to write --out: ${String(e)}`] };
    }
  }
