...
//
// `gemini publish [path] [--confirm]` CLI subcommand.
//
...
//   PINATA_JWT — fetch via `gemini secrets fetch PINATA_JWT`
//                or your local .env
...
/**
 * `gemini publish [path] [--confirm] [--name=<override>]`
 */
export async function publishCmd(args: string[]): Promise<SubcommandResult> {
  ...
  const lines: string[] = [`gemini publish — ${dir} ${confirm ? '(CONFIRMED)' : '(DRY-RUN)'}`];
  ...
  if (confirm) {
    ...
    if (!jwt) {
      lines.push(
        ...
        '  Fetch with: gemini secrets fetch PINATA_JWT',
