...
//
// `gemini upgrade [path] [--apply] [--conflict=inline|rej]` CLI subcommand.
//
// Re-renders the gemini's template against the current generator
...
/**
 * `gemini upgrade [path] [--apply] [--conflict=inline|rej]`
 */
export async function upgradeCmd(args: string[]): Promise<SubcommandResult> {
  ...
  const lines: string[] = [`gemini upgrade — ${dir} ${apply ? '(APPLY)' : '(DRY-RUN)'}`];
  ...
  if (!existsSync(manifestPath)) {
    lines.push(`  no .gemini/manifest.json at ${dir} — not a generated gemini`);
    return { code: 1, lines };
  }
  ...
  // Re-render the template that produced this gemini, with the same vars,
  ...
  if (!existsSync(tdir)) {
    ...
    lines.push(`  (the create-agent-gemini package may have evolved the template id)`);
    return { code: 1, lines };
  }
  ...
  if (total === 0) {
    lines.push('', '  No drift — gemini is up-to-date with the template.');
    return { code: 0, lines };
  }
