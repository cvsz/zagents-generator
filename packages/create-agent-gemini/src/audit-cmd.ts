...
//
// `gemini audit [path] [--level=high|critical|moderate]` CLI subcommand.
//
// Wraps `npm audit --omit=dev --audit-level=<level>` against a gemini's
// package-lock.json + reports the per-severity advisory count. Like
// scripts/audit-deps.mjs but per-gemini.
...
export async function auditCmd(args: string[]): Promise<SubcommandResult> {
  ...
  // gemini metadata) so users can paste it into a security review or
  // attach to a GitHub issue. Same pattern as iter-90 diag --bundle.
  const bundle = args.includes('--bundle');

  const lines: string[] = [`gemini audit — ${dir} (level=${level})`];
