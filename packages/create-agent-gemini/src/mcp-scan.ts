...
//
// `gemini mcp-scan [path]` — a security scanner for a scaffolded gemini's MCP
// surface. "npm audit for agent tools" (ADR-022): it reads the gemini's MCP
...
/**
 * Scan a gemini directory for MCP security issues. Never executes anything —
...
export function scanMcp(dir: string): ScanReport {
  ...
  const policy = readJson(join(root, '.gemini', 'mcp-policy.json')) as Record<string, unknown> | undefined;
  ...
  // --- policy checks -------------------------------------------------------
  if (!policy) {
    add({ id: 'no-policy', severity: 'high', title: 'MCP server with no policy', detail: 'An MCP server is registered but .gemini/mcp-policy.json is missing — execution is ungoverned.' });
  } else {
...
export function mcpScanCmd(args: string[]): { code: number; lines: string[] } {
  ...
  const lines: string[] = [`gemini mcp-scan — ${report.dir}`, ''];
