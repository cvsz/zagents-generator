...
//
// Per ADR-011, every gemini ships with a `witness.json` next to its
// `.gemini/manifest.json`. The publish gate verifies that signature
// BEFORE pushing the gemini to npm or pinning it to IPFS — there is
// no path to publish an unsigned or tampered gemini.
//
...
export async function verifyWitness(manifest: unknown): Promise<VerificationResult> {
  ...
  if (!m.gemini || typeof m.gemini !== 'string') {
    return { valid: false, reason: 'gemini must be a string' };
  }
...
/**
 * Find a gemini's witness.json. Convention: <harnessDir>/witness.json
 * or <harnessDir>/.gemini/witness.json.
 */
export function findWitness(harnessDir: string): string | null {
  for (const cand of [
    join(harnessDir, 'witness.json'),
    join(harnessDir, '.gemini', 'witness.json'),
  ]) {
