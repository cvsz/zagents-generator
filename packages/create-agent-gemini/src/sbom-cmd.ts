...
//
// `gemini sbom [path] [--out=<file>] [--include-dev]` CLI subcommand.
//
// Generates a SPDX-2.3 Software Bill of Materials for the scaffolded
// gemini at <path>. Mirrors scripts/sbom.mjs at the meta-repo level,
// but scoped to ONE gemini so the user gets a per-package artifact.
//
// Reads the gemini's package-lock.json if present; falls back to
...
export async function sbomCmd(args: string[]): Promise<SubcommandResult> {
  ...
  const lines: string[] = [`gemini sbom — ${dir}`];

  const pkgPath = join(dir, 'package.json');
  if (!existsSync(pkgPath)) {
    lines.push(`  no package.json at ${dir} — not a gemini or npm package`);
    return { code: 1, lines };
  }
  ...
  const doc = {
    ...
    documentNamespace: `https://example.invalid/gemini/${pkg.name}/sbom-${createHash('sha1').update(`p=${packages.length}`).digest('hex').slice(0, 12)}`,
    creationInfo: {
      created: new Date(0).toISOString(),
      creators: [`Tool: gemini sbom (iter 51)`],
