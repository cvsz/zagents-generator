...
async function makeHarnessDir(): Promise<string> {
  ...
  const m = await import('node:fs/promises').then(m =>
    m.readFile(join(dir, '.gemini', 'manifest.json'), 'utf-8')
  );
  const hash = createHash('sha256').update(m, 'utf-8').digest('hex');
  await writeFile(join(dir, '.gemini', 'manifest.sha256'), hash);
...
describe('gemini validate', () => {
  ...
  it('runs all 5 checks and returns HEALTHY on a clean gemini', async () => {
    const dir = await makeHarnessDir();
    try {
  ...
  it('umbrella FAIL message recommends diag --bundle (iter 94)', async () => {
    ...
    try {
      // Empty dir → doctor fails (no package.json, no .gemini),
