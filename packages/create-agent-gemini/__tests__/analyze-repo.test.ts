...
describe('recommendPlan (lexical default)', () => {
  it('routes a Rust crate to rust-crate-gemini + vertical:coding', async () => {
    const dir = await rustRepoDir();
    const plan = recommendPlan(analyzeFiles('ruvector', inventory(dir)));
    expect(plan.archetypeId).toBe('rust-crate-gemini');
...
describe('analyzeRepoCmd', () => {
  it('writes repo-profile.json + gemini-plan.json and prints the plan', async () => {
    const dir = await rustRepoDir();
    const r = await analyzeRepoCmd([dir]);
    expect(r.code).toBe(0);
    const profile = JSON.parse(await readFile(join(dir, 'repo-profile.json'), 'utf-8'));
    const plan = JSON.parse(await readFile(join(dir, 'gemini-plan.json'), 'utf-8'));
    expect(profile.languages).toContain('rust');
    expect(plan.archetypeId).toBe('rust-crate-gemini');
    expect(r.lines.join('\n')).toContain('Best archetype: rust-crate-gemini');
  });

  it('--scaffold materialises the recommended gemini', async () => {
    const dir = await rustRepoDir();
