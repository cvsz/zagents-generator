...
//
// Run from packages/create-agent-gemini:  node scripts/gen-templates.mjs
...
function manifestJson(t) {
  const files = [
    ...
    // manifest so `claude -p --plugin-dir <gemini>` works out of the box.
    ...
    JSON.stringify(
      {
        ...
        files,
        vars: [
          { name: 'name', prompt: 'Gemini name (kebab-case)', validate: '^[a-z0-9-]+$' },
...
// iter 132 — Claude Code plugin manifest for `claude -p --plugin-dir <gemini>`.
// Same shape as packages/create-agent-gemini/templates/minimal/.claude-plugin/
// plugin.json.tmpl (iter 131) but with per-vertical categories + tags so the
// plugin shows up correctly when the gemini is registered as a plugin in
// the Claude Code marketplace.
function pluginJsonTmpl(t) {
  const cats = ['agent-gemini', 'metaharness-scaffold'];
  if (t.category) cats.push(t.category);
  if (t.domain) cats.push(t.domain);
  const tags = ['metaharness', 'agent-gemini'];
...
// Generated by create-agent-gemini — your gemini's \`{{name}} init\` entry.
...
export const SYSTEM_PROMPT = \`${a.systemPrompt} You operate inside the {{name}} gemini; defer destructive actions to the user.\`;
...
> ${t.name} gemini · domain: \`${t.domain}\`. Generated with [create-agent-gemini](https://github.com/ruvnet/agent-gemini-generator).
...
This gemini uses [@metaharness/kernel](https://www.npmjs.com/package/@metaharness/kernel) — a Rust-compiled WASM module with a NAPI-RS native fallback — so the same code runs identically on every platform.
`;
}
...
> **${t.name}** — ${t.quickStart}
>
> Generated with [\`create-agent-gemini\`](https://github.com/ruvnet/agent-gemini-generator). WASM kernel, multi-host support, witness-signed releases.
...
${t.agents.length ? `## Agents\n\n| Agent | Role |\n|---|---|\n${agentRows}\n` : ''}
This gemini ships with the **{{host}}** adapter.
