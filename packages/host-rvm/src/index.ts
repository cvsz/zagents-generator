...
//
// This adapter emits the per-gemini RVM artefacts:
//   - rvm-partition.toml  partition manifest
//   - capability-table.json  derived from gemini claims (kernel claims
...
/** Build the RVM capability table from the gemini's kernel-side claims. */
...
/**
 * ADR-044 fix: derive RVM capability *rights* from a gemini permission
 ...
 *   2. otherwise derive tokens from `spec.permissions.allow` (the gemini's
...
export interface RvmPartitionSpec {
  /** Gemini name; doubles as the partition's coherence domain seed. */
  name: string;
...
export function partitionToml(spec: HarnessSpec, partition: RvmPartitionSpec = defaultPartitionSpec(spec.name)): string {
  const lines: string[] = [];
  lines.push('# Generated by @zagents/host-rvm');
  lines.push('# RVM partition manifest for gemini: ' + spec.name);
  lines.push('');
  ...
  lines.push(`package = "@zagents/kernel"`);
  ...
  if (spec.description || spec.systemPrompt) {
    ...
    // ADR-044: carry the gemini system prompt so the wasm guest can load it.
...
export function installScript(spec: HarnessSpec): string {
  return [
    '#!/usr/bin/env bash',
    '# RVM install runbook for gemini: ' + spec.name,
    'set -euo pipefail',
    '',
...
export const adapter: HostAdapter = {
  ...
  generateConfig: (spec: HarnessSpec) => ({
    ...
    // gemini's permission posture (or an explicit spec.claims extension).
