...
const base: HarnessSpec = {
  name: 'my-pi-gemini',
  description: 'a pi gemini',
  systemPrompt: 'You are my-pi-gemini.',
...
describe('@zagents/host-pi-dev', () => {
  ...
  describe('agentsMarkdown', () => {
    it('carries name, description, and agents', () => {
      ...
      expect(md).toContain('# my-pi-gemini');
  ...
  describe('trustJson (ADR-044)', () => {
    it('trusts the gemini extension by package name', () => {
      const parsed = JSON.parse(trustJson(base));
      expect(parsed.schema).toBe(1);
      expect(parsed.trusted_extensions).toHaveLength(1);
      expect(parsed.trusted_extensions[0].name).toBe('my-pi-gemini');
      expect(parsed.trusted_extensions[0].source).toBe('npm:my-pi-gemini');
    });
