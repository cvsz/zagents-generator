...
Two-gemini diff. Useful when you've forked an upstream template, when a
...
| `no-manifest-in-a` | 2 | `a/.gemini/manifest.json` missing |
| `no-manifest-in-b` | 2 | `b/.gemini/manifest.json` missing |
...
```bash
gemini compare ./my-fork ./upstream                  # text output
gemini compare ./my-fork ./upstream --bundle         # ADR-031 schema-1 JSON
```
...
shape is shared with `gemini diag --bundle`, `gemini export-config`, and
`gemini audit --bundle`:
...
```
gemini compare — diffing /tmp/a /tmp/b
...
- A user files a "this doesn't work" bug; you run `gemini compare
...
- `diag-gemini` — single-gemini kernel-version skew check (iter 66)
- `validate-gemini` — release-readiness umbrella (iter 20)
