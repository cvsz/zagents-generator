...
```
/threat-model path=./my-gemini
/threat-model path=./my-gemini bundle=true
```

## Equivalent CLI

```bash
gemini threat-model ./my-gemini                # text artifact
gemini threat-model ./my-gemini --json         # full envelope
gemini threat-model ./my-gemini --bundle       # ADR-031 schema-1
gemini threat-model ./my-gemini --out tm.json  # write to file
```
...
- Any change to `.gemini/mcp-policy.json` or `.claude/settings.json`
  permissions
...
- `validate-gemini` (iter 22) — release-readiness umbrella (includes
  mcp-scan)
- `score-gemini` (iter 114) — broader 0-100 scorecard; MCP safety is
