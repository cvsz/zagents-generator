...
- **repo-genome** — "is this repo READY for that gemini, and what's the risk?"
...
6. **Recommended gemini plan** — template, archetype, hosts, agents, skills
...
/repo-genome path=./my-repo out=./gemini-genome.json
```

## Equivalent CLI

```bash
gemini genome ./my-repo                          # text report
gemini genome ./my-repo --json                   # 6-field scorecard JSON
gemini genome ./my-repo --bundle                 # ADR-031 schema-1 envelope
gemini genome ./my-repo --out gemini-genome.json # write scorecard to file
```
...
- Before scaffolding a gemini — run genome first, decide whether the
...
- `diag-gemini` (iter 70) — kernel-version skew check
- `score-gemini` (iter 114) — post-scaffold gemini scorecard
