...
Emits `.gemini/oia-manifest.json` — the gemini's self-assessment against
...
```
/oia-manifest path=./my-gemini
/oia-manifest path=./my-gemini check=true
/oia-manifest path=./my-gemini dry-run=true
```

## Equivalent CLI

```bash
gemini oia-manifest ./my-gemini                # write .gemini/oia-manifest.json
gemini oia-manifest ./my-gemini --check        # validate existing manifest
gemini oia-manifest ./my-gemini --dry-run      # stdout, no file write
gemini oia-manifest ./my-gemini --json         # stdout JSON
```
...
| `FAIL no manifest` | 2 | `.gemini/oia-manifest.json` missing |
...
- `validate-gemini` (iter 22) — release-readiness umbrella
