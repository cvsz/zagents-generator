# Agent Gemini Studio — Web UI

A **100% client-side** Studio for the agent-gemini supply chain: turn any GitHub repo (or a blank slate) into a governed, branded, multi-host AI agent gemini — recommend, build, verify, ship. No backend, no install, nothing leaves your browser. Deployable to GitHub Pages; desktop- and mobile-friendly.

> Design: [ADR-020](../../docs/adrs/ADR-020-web-generator-ui.md) · [ADR-021](../../docs/adrs/ADR-021-client-side-packaging-and-pages-deploy.md) · [ADR-022 (MCP)](../../docs/adrs/ADR-022-mcp-primitive.md) · [ADR-023 (Repo→Gemini)](../../docs/adrs/ADR-023-repo-to-gemini-importer.md) · [ADR-024 (Studio+Verify)](../../docs/adrs/ADR-024-studio-and-verify.md)
...
| Tab | What you get |
|---|---|
| **Repo → Gemini** | Paste a GitHub URL → deterministic analysis (languages, build/test commands, MCP/host/CI signals) → archetype scoring → an editable gemini plan with a confidence panel. No repo code executed; suggested commands are `trust: inferred · execution: disabled`. Semantic engine: **Lexical** (default) or optional **MiniLM** embeddings (Transformers.js, WebGPU/WASM, lazy-loaded, lexical fallback). |
| **Create gemini** | 16 quick-start verticals, composable agents/skills/commands, kernel options, and the **Primitives** panel (CLI · MCP · memory · learning · witness · release gates). Live file tree + `<name>.zip`. |
...
<p align="center">
  <img src="../../docs/web-ui/screenshot-repo.png" width="32%" alt="Repo → Gemini" />
...
- **Generator parity** — `src/generator/render.ts` is a behaviour-for-behaviour port of the CLI renderer; templates/agents/skills/commands come from the generated `src/generated/catalog.ts` (single source of truth in `packages/create-agent-gemini/templates/catalog.def.mjs`).
...
│  │  ├─ scaffold.ts        # full gemini file tree
│  │  ├─ mcp.ts             # MCP primitive (ADR-022)
│  │  ├─ repo.ts            # Repo → Gemini analyzer + archetypes (ADR-023)
│  │  ├─ verify.ts          # in-browser gemini verifier (ADR-024)
