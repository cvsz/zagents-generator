...
const INITIAL: HarnessConfig = {
  name: 'my-gemini',
  description: findTemplate(DEFAULT_TEMPLATE)?.harnessDesc ?? 'My AI agent gemini',
  hosts: ['claude-code'],
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
      ...
      <div className="space-y-5">
        <Section title="Identity" desc="Your gemini ships to npm under this name.">
          <div className="space-y-4">
            <Field label="Gemini name" hint={nameCheck.valid ? 'kebab-case, npm-publishable' : nameCheck.reason}>
              <input
                className={`input ${nameCheck.valid ? '' : 'border-red-500/70 focus:border-red-500 focus:ring-red-500/30'}`}
                value={cfg.name}
                spellCheck={false}
                aria-invalid={!nameCheck.valid}
                aria-label="Gemini name"
        ...
        <Section title="Hosts" desc="Where the gemini runs. Pick one or more.">
        ...
        <Section
          title="Primitives"
          desc="MCP is one selectable primitive — not the whole product. Toggle what the gemini ships."
        >
          <div className="space-y-4">
              ...
              {cfg.primitives.mcp !== 'off' && (
                <div className="mt-3 border-t border-ink-700 pt-3">
                  ...
                  <p className="mt-2 text-xs text-slate-500">
                    ...
                    <code className="text-slate-300">.gemini/mcp-policy.json</code>.
                  </p>
                </div>
              )}
            </div>
      ...
      <div className="space-y-5 lg:sticky lg:top-5 lg:self-start">
        <Section
          title="Generated gemini"
          desc={`${files.length} files · ${(totalBytes(files) / 1024).toFixed(1)} KB uncompressed`}
          right={
            <button data-testid="download-gemini" className="btn btn-primary" onClick={download} disabled={!nameCheck.valid || busy}>
        ...
        >
          {nameCheck.valid ? (
            ...
            <div className="flex items-center gap-2 rounded-lg border border-red-500/40 bg-red-500/10 px-3 py-2 text-sm text-red-200">
              <Package size={16} /> Fix the gemini name to preview the scaffold.
            </div>
          )}
        </Section>
