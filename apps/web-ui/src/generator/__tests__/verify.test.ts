...
describe('verifyFileMap', () => {
  it('passes a freshly generated, secure gemini', () => {
  ...
  it('flags an MCP server whose policy was weakened to allow shell', () => {
    ...
    const policy = files.find((f) => f.path === '.gemini/mcp-policy.json')!;
  ...
  it('treats an MCP-off gemini as clean (info, not fail)', () => {
  ...
  it('reports host capability coverage (system-prompt + agents + mcp + permissions)', () => {
    ...
    expect(cov!.detail).toContain('verify-gemini-live');
  });

  it('detects an opencode gemini host artifact + permissions coverage', () => {
