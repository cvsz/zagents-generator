...
describe('injected semantic scores', () => {
  it('an injected map can override the ranking deterministically', () => {
    ...
    const semantic: Record<string, number> = Object.fromEntries(ARCHETYPES.map((a) => [a.id, a.id === 'research-gemini' ? 1 : 0]));
    const ranked = scoreArchetypes(profile, semantic);
    // research-gemini should now outrank with a near-max semantic contribution.
    const research = ranked.find((r) => r.archetype.id === 'research-gemini')!;
  ...
  it('profileText feeds embeddings; lexical default still works without it', () => {
    ...
    expect(recommendPlan(profile).archetypeId).toBe('rust-crate-gemini');
  });
});
