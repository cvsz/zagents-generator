...
//
// Repo → Gemini importer (ADR-023). Paste a GitHub URL; get a recommended,
// editable gemini plan; emit a deterministic scaffold.
//
...
export function scoreArchetypes(profile: RepoProfile, semanticScores?: Record<string, number>): ScoredArchetype[] {
  ...
  const scored = ARCHETYPES.map((a) => {
    ...
    const structure =
      (profile.hasClaude ? 0.34 : 0) + (profile.hasMcp && a.id === 'mcp-server-gemini' ? 0.5 : 0) + (profile.languages.length ? 0.33 : 0);
...
export function recommendPlan(profile: RepoProfile, semanticScores?: Record<string, number>): HarnessPlan {
  ...
  return {
    name: toKebabCase(`${profile.name}-gemini`),
    hosts,
    template: a.template,
    archetypeId: a.id,
...
export function planToConfig(plan: HarnessPlan): HarnessConfig {
  ...
  return {
    name: plan.name,
    description: tmpl?.harnessDesc ?? `Agent gemini for ${plan.name}`,
    hosts: plan.hosts,
