...
//
// Shared types for the browser-side gemini generator. Mirrors the data model
// of packages/create-agent-gemini so the UI emits artifacts that the CLI and
 ...
 * field defaults to the safe option; the gemini author opts INTO capability,
 * never out of safety. Emitted as both enforced TS (policy.ts) and inert data
 * (.gemini/mcp-policy.json) so it can be scanned, audited, and witnessed.
 */
...
/** The composable primitives a gemini can switch on (ADR-022). */
