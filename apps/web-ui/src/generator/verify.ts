...
//
// Browser-side gemini verifier — the engine behind the Studio's "Verify" tab.
// Takes a file map (either the live scaffold or an unzipped download) and runs
// the same class of checks the CLI's `gemini validate` + `gemini mcp-scan`
...
export function verifyFileMap(files: GenFile[]): VerifyReport {
  ...
  if (!pkg) {
    fail('pkg-missing', 'high', 'package.json present', 'No package.json — not a publishable gemini.');
  } else {
    try {
  ...
  if (find(files, '.gemini/manifest.json')) pass('manifest', '.gemini/manifest.json present');
  else fail('manifest', 'medium', '.gemini/manifest.json present', 'Missing generator manifest (drift detection).');
  ...
  checks.push({
    ...
    detail: covered.length ? `Represented: ${covered.join(', ')}. Live-verify with \`node scripts/verify-gemini-live.mjs --dir <gemini>\` (OpenRouter).` : 'No spec capabilities detected in the emitted tree.',
  });
  ...
  const policyFile = find(files, '.gemini/mcp-policy.json');
  ...
  if (policyFile || mcpServer) {
    if (!policyFile) {
      fail('mcp-no-policy', 'high', 'MCP server has a policy', 'MCP registered but no .gemini/mcp-policy.json — ungoverned.');
    } else {
      try {
